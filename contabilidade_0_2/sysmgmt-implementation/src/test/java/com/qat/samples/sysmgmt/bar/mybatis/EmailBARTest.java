...
import com.qat.samples.sysmgmt.bar.IEmailBAR;
import com.qat.samples.sysmgmt.model.Email;
...
@Test
	...
	{
		Email email = new Email(999, "Email_999");
	...
	Email email = new Email();
		List<Email> response = getEmailBAR().fetchAllEmails().getResultsList();
	...
	{
		Email email = new Email(1234, "NATIVE INSERT UPDATE");
		FetchByIdRequest request = new FetchByIdRequest();
		request.setFetchId(1234);
		Email emailResponse = getEmailBAR().fetchEmailById(request);
		Assert.assertEquals(emailResponse.getDescription(), "NATIVE INSERT");
		getEmailBAR().updateEmail(email);
		emailResponse = getEmailBAR().fetchEmailById(request);
		Assert.assertEquals(emailResponse.getDescription(), "NATIVE INSERT UPDATE");
	}
