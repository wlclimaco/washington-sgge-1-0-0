...
function TFrmConSrv.ConectaServidorConexao(pSnConIni : Boolean) : Boolean;
var lDsErro : String;
begin
      Result  := False;
      lDsErro := '';
...
function TFrmConSrv.RecebeEnderecoServidor : Boolean;
begin
      FrmAtuEnd := TFrmAtuEnd.Create(Self);

      try
          CarregarEnderecoDeConexao;
          FrmAtuEnd.EdtNoPorta.Text  := IntToStr(gNoPorta);
          ...
          begin
                ...
                gNoPorta  := StrToInt(FrmAtuEnd.EdtNoPorta.Text);
...
procedure TFrmConSrv.EnviaMensagemDicionarioAtualizado;
begin
...
procedure TFrmConSrv.EnviaMensagemClienteConectado(pNrClient: Integer; pDsClient: String);
begin
      if TCPClient.Connected then
      begin
            if uFrmLogos.cTpServer = 'A' then
...
procedure TFrmConSrv.EnviaMensagemConexaoEstabelecida;
begin
...
procedure TFrmConSrv.EnviaMensagemClienteAutenticado(pCdUsuari : Integer;
...
procedure TFrmConSrv.CarregarEnderecoDeConexao;
...
procedure TFrmConSrv.GravaEnderecoDeConexao;
var lIniFile  : TIniFile;
    lDsCaminh : String;
begin
      try
...
procedure TFrmConSrv.TCPClientExecute(Sender: TObject; pDsMensag: string);
...
procedure TFrmConSrv.TimerTimer(Sender: TObject);
begin
      PgbProgre.StepIt;
      PgbProgre.Refresh;
end;

procedure TFrmConSrv.TmrConexaTimer(Sender: TObject);
begin
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
procedure TFrmConSrv.IniciarProcessoReconexao(pQtSegund : Integer);
begin
...
procedure TFrmConSrv.ProcessarMensagemServerConexao(pDsMensag : String);
...
procedure TFrmConSrv.AtualizarDicionarioDados(pSnMensag : Boolean);
begin
