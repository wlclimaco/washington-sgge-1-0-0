...
function CompressGZip(var Data: AnsiString; Compress: boolean): AnsiString;

/// (un)compress a data content using the Deflate algorithm
// - as expected by THttpSocket.RegisterCompress
// - will use internaly a level compression of 1, i.e. fastest available (content
// of 4803 bytes is compressed into 700, and time is 440 us instead of 220 us)
function CompressDeflate(var Data: AnsiString; Compress: boolean): AnsiString;
...
{$endif Linux}
  ...
  protected
    ...
    fFileName: TFileName;
    fMagic: cardinal;
    procedure InternalAdd(const zipName: TFileName; Buf: pointer; Size: integer);
  public
...
procedure TZipWrite.InternalAdd(const zipName: TFileName; Buf: pointer; Size: integer);
begin
  with Entry[Count] do begin
    ...
    fHr.localHeadOff := SetFilePointer(Handle,0,nil,FILE_CURRENT)-fAppendOffset;
...
begin
  ...
  i1 := CompressMem(pointer(data), pointer(integer(result)+12), length(data),
...
begin
  ...
  if Length(data) > 12 then begin
    ...
    SetLength(result, UncompressMem(pointer(integer(data)+12), pointer(result),
...
begin
  if FInitialized then begin
    while (Check(deflate(FStrm, Z_FINISH), [Z_OK, Z_STREAM_END])
       <> Z_STREAM_END) and (FStrm.avail_out = 0) do
      FlushBufferOut;
    FlushBufferOut;
    FStrm.next_out := nil;
    FStrm.avail_out := 0;
