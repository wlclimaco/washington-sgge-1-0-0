sensus.commons.modules.content.lightConfigurations = {
	...
	fnLoadLightConfigurations : function(aData) {
		...
		var sDateAddedToFormat = aData.dateAdded;
		var sHour = ' ' + $.sc.dateFormat( sDateAddedToFormat, 'h:i:s.fff A');

		sensus.commons.modules.util.fnFillData($('#update-date-added'), [ $.sc.dateFormat( sDateAddedToFormat, sensus.settings.dateFormatMask), sHour ]);
...
$('#configuration').on('click', '.update-light-protected', function() {
	...
	var oRequest = new lightRequest(id,null,$(this).attr('id'));

	oRequest.selectionPaginationIds =  [parseInt(id)];

	$.sc.getJson("api/lighttop/updateprotected", oRequest, false, sensus.commons.modules.content.lightConfigurations.setProtect, $.sc.locale("action.longRunningProcessDialog.confirm"));
	return false;
});
...
$('.save-pole-id', oUpdatePoleId).click(function(e) {
	...
	if (poleId.length === 0) {
		...
		var oRequest = new lightRequest(id, null, false, null, null, true, null, null, null, null, null, null, null, aParameters);

		oRequest.selectionPaginationIds =  [parseInt(id)];

		$.ajaxValidator.fnMonitor("api/lighttop/upsertproperty", oRequest, false, sensus.commons.modules.content.lightConfigurations.fnUpdateConfigurations, $.sc.locale("action.longRunningProcessDialog.confirm"), true);
