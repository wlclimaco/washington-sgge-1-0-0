...
public class BeneficiosDACImpl extends SqlSessionDaoSupport implements IBeneficiosDAC
{
	...
	public InternalResultsResponse<Beneficios> insertBeneficios(Beneficios beneficios)
	{
		...
		insertCount += maintainBeneficiosAssociationsCnae(beneficios, response);

		insertCount += maintainBeneficiosAssociationsEmail(beneficios, response);

		insertCount += maintainBeneficiosAssociationsTelefone(beneficios, response);
	...
	public InternalResultsResponse<Beneficios> updateBeneficios(Beneficios beneficios)
	{
		...
		updateCount += maintainBeneficiosAssociationsCnae(beneficios, response);

		updateCount += maintainBeneficiosAssociationsEmail(beneficios, response);

		updateCount += maintainBeneficiosAssociationsTelefone(beneficios, response);
	...
	{
		Integer count = 0;
		// First Maintain Beneficios
		if (ValidationUtil.isNullOrEmpty(beneficios.getEnderecos()))
		{
			return count;
		}
		// For Each Contact...
		for (Endereco endereco : beneficios.getEnderecos())
		{
			// Make sure we set the parent key
			endereco.setParentKey(beneficios.getId());
			...
			switch (endereco.getModelAction())
			{
				...
				default:
					...
					{
						LOG.debug("ModelAction for Beneficios missing!");
					}
					break;
			}
		}
	...
	private Integer maintainBeneficiosAssociationsCnae(Beneficios beneficios,
			InternalResultsResponse<Beneficios> response)
	{
		Integer count = 0;
		// First Maintain Beneficios
		if (ValidationUtil.isNullOrEmpty(beneficios.getCnaes()))
		{
			return count;
		}
		// For Each Contact...
		for (Cnae cnae : beneficios.getCnaes())
		{
			// Make sure we set the parent key
			cnae.setParentKey(beneficios.getId());
	...
	private Integer maintainBeneficiosAssociationsTelefone(Beneficios beneficios,
			InternalResultsResponse<Beneficios> response)
	{
		Integer count = 0;
		// First Maintain Beneficios
		...
		// For Each Contact...
		for (Telefone telefone : beneficios.getTelefones())
		{
			// Make sure we set the parent key
			// contact.setParentKey(beneficios.getId());
			//
			// if (ValidationUtil.isNull(beneficios.getModelAction()))
			// {
			// continue;
			// }
			// switch (beneficios.getModelAction())
			// {
			// case INSERT:
	...
	private Integer maintainBeneficiosAssociationsEmail(Beneficios beneficios,
			InternalResultsResponse<Beneficios> response)
	{
		Integer count = 0;
		// First Maintain Beneficios
		if (ValidationUtil.isNullOrEmpty(beneficios.getEmails()))
		{
			return count;
		}
		// For Each Contact...
		for (Email email : beneficios.getEmails())
		{
			// Make sure we set the parent key
			email.setParentKey(beneficios.getId());
			...
			InternalResultsResponse<Beneficios> response)
	{
		Integer count = 0;
		// First Maintain Beneficios
		if (ValidationUtil.isNullOrEmpty(beneficios.getDocumentos()))
		{
			return count;
		}
		// For Each Contact...
		for (Documento documentos : beneficios.getDocumentos())
		{
			// Make sure we set the parent key
			documentos.setParentKey(beneficios.getId());
