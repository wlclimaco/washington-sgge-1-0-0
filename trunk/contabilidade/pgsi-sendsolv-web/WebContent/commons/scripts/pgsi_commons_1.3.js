	...
	$.pgsi = (function ()
	{
		...
		var _ajax = (function()
		{
			...
			var _call = function(oCallConfig)
			{
				...
				if(pgsi.settings.appRoot)
				{
					oCallConfig.sUrl = pgsi.settings.appRoot + "/" + oCallConfig.sUrl;
		...
		var _util = (function ()
		{
			...
			{
				return pgsi.settings.userContext.userRole.indexOf(sRole) !== -1;
			};
			...
			var _savePropertyProfile = function (oParam, id)
			{
				...
				{
					pgsi.pages.dm.fnReloadUserSettings(oResponse);
...
//					oRequest[_oConfig.monitor.requestAttribute] = new ProcessRequest();
				...
				var fnCheckRniCallBack = function(oRniResponse)
				{
					...
					{
						$rniOffline.show();
						pgsi.pages.longrunningprocess.toggleSystemMessaging(true);
					}
					...
					if (pgsi.pages.longrunningprocess.lrpSize == 0 && oRniResponse.linkStatus)
					{
						pgsi.pages.longrunningprocess.toggleSystemMessaging(false);
					}
...
//				oRequest.processCriteria.monitored = isMonitored;
			...
			var _monitorDialogEvent = function(e)
			{
				...
				pgsi.pages.longrunningprocess.dialogSettingsProcess.longRunningProcessDialog.iMonitor = bMonitor;
			...
			var _setMonitorAjax = function(oMonitorDefaults)
			{
				...
				switch (parseInt(pgsi.settings.user.monitorRequest, 10))
				{
					...
					case 3: // Prompt
						...
						// Open dialog
						_util.launchActionDialog(_oConfig.monitor.longRunninProcessDialog , pgsi.pages[_oConfig.monitor.monitorDialogSettingsNamespace].dialogSettingsProcess[_oConfig.monitor.longRunninProcessDialog], "monitor-dialog");
						break;
			...
			var _start = function(oMonitorDefaults)
			{
				...
				var fnCheckRniCallBack = function(IsRniIsOn)
				{
					...
					{
						fnDefineMonitor( oMonitorDefaults, pgsi.pages.longrunningprocess.dialogSettingsProcess.longRunningProcessDialog.iMonitor );
						pgsi.pages.longrunningprocess.dialogSettingsProcess.longRunningProcessDialog.iMonitor = null;
...
//				oTimeZoneJS.setMinutes(oTimeZoneJS.getMinutes() - (iBrowserOffset - iCurrentOffset));
			...
			var _format = function (date, sFormat, oTimeZone)
			{
				...
				if (!sFormat)
				{
					sFormat = pgsi.settings.user.dateFormat;
				}

				// Set only Today or Yesterday
				if (sFormat == "simpleDay")
				{
					sFormat = _getValidDateFormat(pgsi.settings.user.dateFormat) + " wt";
				}

				// Set date with Today or Yesterday Value
				if (sFormat == "day")
				{
					sFormat = _getValidDateFormat(pgsi.settings.user.dateFormat) + " ww";
				}
				else
				{
		...
		var _locale = (function()
		{
			var _init = function (oMessageProperties)
			{
				jQuery.i18n.properties(
				{
					...
					language			: pgsi.settings.language,
		...
		var _pageLoader = (function()
		{
			...
			{
				return "/" + pgsi.settings.serviceType.toLowerCase() + "/";
			};
			...
			var _title = function (currentPage)
			{
				...
				var _titleService		= _locale.get(_oConfig.pageLoader.serviceLabel + pgsi.settings.serviceType.toLowerCase());
				var _titleSeparator		= " - ";
