...
pgsi.pages.transportador.view = {
	...
	fnFillTransportador : function(oResponse) {

		var oTransportador = oResponse.transportadorList[0];

console.log(oTransportador)

	//	<span class="label">Nome/Fantasia</span>
				$("#nome-field").text(oTransportador.nome);

	//			<span class="label">CNPJ</span>

				for(var i=0;i<oTransportador.documentos.length;i++){
					if(oTransportador.documentos[i].type == "CNPJ"){
						$("#cnpj-field").text(oTransportador.documentos[i].numero);
					}else if(oTransportador.documentos[i].type == "IM") {
						$("#im-field").text(oTransportador.documentos[i].numero);
					}else if(oTransportador.documentos[i].type == "IE") {
						$("#IE-field").text(oTransportador.documentos[i].numero);
					}
				}

				$("#regime-field").text(oTransportador.regime);

				pgsi.pages.phone.view.fillFields(oTransportador.telefones);

				pgsi.pages.address.view.fillFields(oTransportador.enderecos);
				var sEmail = "";

				for (var i = 0; i < oTransportador.emails.length; i++) {
					sEmail = sEmail + oTransportador.emails[i].description + " " +oTransportador.emails[i].email +"<br>"
				}
				$('#phone-container').append(sEmail);

				pgsi.pages.transportador.view.fillCnae(oTransportador.cnaes)
