...
/**
 * @namespace pgsi.pages.pedidoCompras
...
$(document).ready(function()
{
	...
	$.pgsi.listener.wait({
		eventName 	: "pedidoComprasList",
		arguments 	: ["table"],
		fnCallback 	: $.pgsi.progressBar.stopGlobal
	});

	/** * jQuery dataTable setup ** */
	pgsi.pages.pedidoCompras.pedidoComprasTable = $('#data_list').dataTable($.pgsi.table.setTable(
	{
		id 			: "#data_list",
		...
		rowCallback : function(nRow, aData, iDisplayIndex, oColumn) {
			...
			oActionSummary = $('<div><div><a href="pedidoCompras/view?tab=info&pedidoComprasId=' + aData.id + '" title="View '+aData.name+'" id="update" class="icon-nav icon-pencil alist icon-small-button"></a>'
				+ sButtonStatus
				+ sButtonDelete
				+pgsi.util.page.fnInsertButtonSDNSAR(aData,"pedidoCompras")+"</div></div>");
			...
			oActionSummary.find('a.deleteDialog, a.active, a.deactivate ,a.sarDialog').click(function (e) {
				...
				var fnCallBack = function(oResponse) {
					...
					if (oResponse.operationSuccess == true) {
						...
						var iStart;
						var oSettings = pgsi.pages.pedidoCompras.pedidoComprasTable.fnSettings();
						...
						$.pgsi.table.reloadTable({
							table 		: pgsi.pages.pedidoCompras.pedidoComprasTable,
							iStart 		: iStart
						});
					}else{
	...
	$("#clear-all").on("click", function(e)
	{
		...
		$.address.parameter("pedidoCompras","");
		pgsi.util.page.fnReloadTable(pgsi.pages.pedidoCompras.pedidoComprasTable);
	});
