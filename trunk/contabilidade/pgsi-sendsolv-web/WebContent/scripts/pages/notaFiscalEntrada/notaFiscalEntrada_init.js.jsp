...
/**
 * @namespace pgsi.pages.notaFiscalEntrada
...
$(document).ready(function()
{
	...
	$.pgsi.listener.wait({
		eventName 	: "notaFiscalEntradaList",
		arguments 	: ["table"],
		fnCallback 	: $.pgsi.progressBar.stopGlobal
	});

	/** * jQuery dataTable setup ** */
	pgsi.pages.notaFiscalEntrada.notaFiscalEntradaTable = $('#data_list').dataTable($.pgsi.table.setTable(
	{
		id 			: "#data_list",
		...
		rowCallback : function(nRow, aData, iDisplayIndex, oColumn) {
			...
			oActionSummary = $('<div><div><a href="notaFiscalEntrada/view?tab=info&notaFiscalEntradaId=' + aData.id + '" title="View '+aData.name+'" id="update" class="icon-nav icon-pencil alist icon-small-button"></a>'
				+ sButtonStatus
				+ sButtonDelete
				+pgsi.util.page.fnInsertButtonSDNSAR(aData,"notaFiscalEntrada")+"</div></div>");
			...
			oActionSummary.find('a.deleteDialog, a.active, a.deactivate ,a.sarDialog').click(function (e) {
				...
				var fnCallBack = function(oResponse) {
					...
					if (oResponse.operationSuccess == true) {
						...
						var iStart;
						var oSettings = pgsi.pages.notaFiscalEntrada.notaFiscalEntradaTable.fnSettings();
						...
						$.pgsi.table.reloadTable({
							table 		: pgsi.pages.notaFiscalEntrada.notaFiscalEntradaTable,
							iStart 		: iStart
						});
					}else{
	...
	$("#clear-all").on("click", function(e)
	{
		...
		$.address.parameter("notaFiscalEntrada","");
		pgsi.util.page.fnReloadTable(pgsi.pages.notaFiscalEntrada.notaFiscalEntradaTable);
	});
