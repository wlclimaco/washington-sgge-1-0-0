(function($) {
	$.widget("ui.combobox", {
		_create : function() {
			...
			var input = $("<input>").attr('id', $(this.element).attr('id') + "_input").insertAfter(select).autocomplete(
					{
						...
						// as it didn't match
						// anything
						//$(this).val($.pgsi.locale.get("widgets.combobox.prompt"));
						return false;
					}
				...
				minLength : 0
						...
						if ($(this).attr('value') == $.pgsi.locale.get("widgets.combobox.prompt2")) {
							$(this).attr('value', '');
						}
			...
			$("<button class='ui-icon-list-ipt-check'>&nbsp;</button>").attr("tabIndex", -1).attr("title", $.pgsi.locale.get("widgets.combobox.title")).insertAfter(input).button( {
				icons : {
