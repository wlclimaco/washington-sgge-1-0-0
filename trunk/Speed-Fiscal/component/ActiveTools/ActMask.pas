...
{ Alterações efetuadas no arquivo Mask.pas

  Mudei a herança do TCustomMaskEdit de TCustomEdit para TActCustomEdit;
  Foi trocado o nome da classe TCustomMaskEdit para TActCustomMask;
  Foi trocado o nome da classe TMaskEdit para TActMask;
}
...
uses Windows, SysUtils, Classes, StdCtrls, Controls, Messages,
  Forms, Graphics, Menus, MaskUtils, ActCustomEdit;
...
{ TActCustomMask }
  ...
  TActCustomMask = class(TActCustomEdit)
  private
    FEditMask: TEditMask;
    FMaskBlank: Char;
    FMaxChars: Integer;
...
constructor TActCustomMask.Create(AOwner: TComponent);
begin
...
procedure TActCustomMask.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TActCustomMask.KeyUp(var Key: Word; Shift: TShiftState);
begin
...
procedure TActCustomMask.KeyPress(var Key: Char);
begin
...
procedure TActCustomMask.WMLButtonDown(var Message: TWMLButtonDown);
begin
  inherited;
  FBtnDownX := Message.XPos;
end;

procedure TActCustomMask.WMLButtonUp(var Message: TWMLButtonUp);
var
...
procedure TActCustomMask.WMSetFocus(var Message: TWMSetFocus);
begin
  inherited;
  if (IsMasked) then
    CheckCursor;
...
procedure TActCustomMask.SetEditText(const Value: string);
begin
...
function TActCustomMask.GetEditText: string;
begin
  Result := inherited Text;
end;

function TActCustomMask.GetTextLen: Integer;
begin
  Result := Length(Text);
end;

function TActCustomMask.GetText: TMaskedText;
begin
...
procedure TActCustomMask.SetText(const Value: TMaskedText);
var
  OldText: string;
  Pos: Integer;
begin
...
procedure TActCustomMask.WMCut(var Message: TMessage);
begin
  if not (IsMasked) then
    inherited
  else
...
procedure TActCustomMask.WMPaste(var Message: TMessage);
var
  Value: string;
  Str: string;
  SelStart, SelStop : Integer;
...
function TActCustomMask.GetMasked: Boolean;
begin
  Result := EditMask <> '';
end;

function TActCustomMask.GetMaxChars: Integer;
begin
  if IsMasked then
    Result := FMaxChars
  else
...
procedure TActCustomMask.ReformatText(const NewMask: string);
var
  OldText: string;
begin
...
procedure TActCustomMask.SetEditMask(const Value: TEditMask);
var
  SelStart, SelStop: Integer;
begin
...
function TActCustomMask.GetMaxLength: Integer;
begin
  Result := inherited MaxLength;
end;

procedure TActCustomMask.SetMaxLength(Value: Integer);
begin
  if not IsMasked then
...
procedure TActCustomMask.GetSel(var SelStart: Integer; var SelStop: Integer);
begin
  SendMessage(Handle, EM_GETSEL, Integer(@SelStart), Integer(@SelStop));
end;

procedure TActCustomMask.SetSel(SelStart: Integer; SelStop: Integer);
begin
  SendMessage(Handle, EM_SETSEL, SelStart, SelStop);
end;

procedure TActCustomMask.SetCursor(Pos: Integer);
const
...
procedure TActCustomMask.CheckCursor;
var
  SelStart, SelStop: Integer;
begin
...
procedure TActCustomMask.Clear;
begin
  Text := '';
end;

function TActCustomMask.EditCanModify: Boolean;
begin
  Result := True;
end;

procedure TActCustomMask.Reset;
begin
  if Modified then
  begin
    EditText := FOldValue;
...
function TActCustomMask.CharKeys(var CharCode: Char): Boolean;
var
  SelStart, SelStop : Integer;
  Txt: string;
  CharMsg: TMsg;
...
procedure TActCustomMask.ArrowKeys(CharCode: Word; Shift: TShiftState);
var
  SelStart, SelStop : Integer;
begin
...
procedure TActCustomMask.CursorInc(CursorPos: Integer; Incr: Integer);
var
  NuPos: Integer;
begin
...
procedure TActCustomMask.CursorDec(CursorPos: Integer);
var
  nuPos: Integer;
begin
  nuPos := CursorPos;
...
function TActCustomMask.GetFirstEditChar: Integer;
begin
  Result := 0;
  if IsMasked then
...
function TActCustomMask.GetLastEditChar: Integer;
begin
  Result := GetMaxChars;
  if IsMasked then
...
function TActCustomMask.GetNextEditChar(Offset: Integer): Integer;
begin
  Result := Offset;
...
function TActCustomMask.GetPriorEditChar(Offset: Integer): Integer;
begin
  Result := Offset;
...
procedure TActCustomMask.HomeEndKeys(CharCode: Word; Shift: TShiftState);
var
  SelStart, SelStop : Integer;
begin
  GetSel(SelStart, SelStop);
...
procedure TActCustomMask.DeleteKeys(CharCode: Word);
var
  SelStart, SelStop : Integer;
  NuSelStart: Integer;
  Str: string;
...
procedure TActCustomMask.CMEnter(var Message: TCMEnter);
begin
...
procedure TActCustomMask.CMTextChanged(var Message: TMessage);
var
  SelStart, SelStop : Integer;
  Temp: Integer;
begin
...
procedure TActCustomMask.CMWantSpecialKey(var Message: TCMWantSpecialKey);
begin
  inherited;
...
procedure TActCustomMask.CMExit(var Message: TCMExit);
begin
  if IsMasked and not (csDesigning in ComponentState) then
  begin
    ValidateEdit;
...
procedure TActCustomMask.ValidateEdit;
var
  Str: string;
  Pos: Integer;
begin
...
procedure TActCustomMask.ValidateError;
begin
  MessageBeep(0);
  raise EDBEditError.CreateResFmt(@SMaskEditErr, [EditMask]);
end;

function TActCustomMask.AddEditFormat(const Value: string; Active: Boolean): string;
begin
  if not Active then
...
function TActCustomMask.RemoveEditFormat(const Value: string): string;
var
  I: Integer;
  OldLen: Integer;
...
function TActCustomMask.InputChar(var NewChar: Char; Offset: Integer): Boolean;
var
  MaskOffset: Integer;
  CType: TMaskCharType;
  InChar: Char;
...
function TActCustomMask.DoInputChar(var NewChar: Char; MaskOffset: Integer): Boolean;
var
  Dir: TMaskDirectives;
  Str: string;
  CType: TMaskCharType;
...
function TActCustomMask.Validate(const Value: string; var Pos: Integer): Boolean;
var
  Offset, MaskOffset: Integer;
  CType: TMaskCharType;
begin
...
function TActCustomMask.DeleteSelection(var Value: string; Offset: Integer;
  Len: Integer): Boolean;
var
  EndDel: Integer;
...
function TActCustomMask.InputString(var Value: string; const NewValue: string;
  Offset: Integer): Integer;
var
...
function TActCustomMask.FindLiteralChar(MaskOffset: Integer; InChar: Char): Integer;
var
  CType: TMaskCharType;
  LitChar: Char;
begin
