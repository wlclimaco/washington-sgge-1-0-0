...
uses IniFiles, ShellAPI,
  ...
  ACBrNFe, pcnConversao, ACBrNFeDANFEClass, ACBrNFeDANFERave, ACBrUtil, DB,
  DBTables, IBCustomDataSet, IBDatabase, IBQuery, IBTable ;
...
type
  TForm1 = class(TForm)
    ...
    Entradas: TIBTable;
    EntradasItens: TIBTable;
    Query1: TIBQuery;
    IBDatabase1: TIBDatabase;
    IBTransaction1: TIBTransaction;
    EntradasDCNUMERO: TIntegerField;
    EntradasDCSERIE: TIBStringField;
    EntradasDCORDEM: TIBStringField;
    EntradasDCTIPO: TIBStringField;
    EntradasNATUREZA: TIntegerField;
    EntradasVLNOTA: TFloatField;
    EntradasVLICMS: TFloatField;
    EntradasVLIPI: TFloatField;
    EntradasVLFRETE: TFloatField;
    EntradasCFOP: TFloatField;
    EntradasDTENTRADA: TDateField;
    EntradasDTEMISSAO: TDateField;
    EntradasTRANSPORTADO: TFloatField;
    EntradasCDPEDIDO: TFloatField;
    EntradasIDNFENTRADAS: TIntegerField;
    EntradasVLST: TFloatField;
    EntradasTPSITUACAO: TIBStringField;
    EntradasItensDCNUMERO: TIntegerField;
    EntradasItensDCSERIE: TIBStringField;
    EntradasItensDCORDEM: TIBStringField;
    EntradasItensDCTIPO: TIBStringField;
    EntradasItensCODPRO: TIntegerField;
    EntradasItensQUANTIDADE: TFloatField;
    EntradasItensIPI: TFloatField;
    EntradasItensST: TFloatField;
    EntradasItensICMS: TFloatField;
    EntradasItensVLUNITARIO: TFloatField;
    EntradasItensVLDESCONTO: TFloatField;
    EntradasCDFORNECEDOR: TIntegerField;
    EntradasItensCDFORNECEDOR: TIntegerField;
    EntradasItensCDPRODUTO: TIntegerField;
  ...
  begin
    ...
    with ACBrNFe1.NotasFiscais.Items[n].NFe do
     begin
      ...
      {
        rvwNFe.Items.AddChild(Node,'vBC='     +FloatToStr(Total.ICMSTot.vBC));
       ...
       Entradas.Insert;
       try
         EntradasDcnumero.AsInteger      := Ide.nNF;
         EntradasDcserie.AsString        := IntToStr(Ide.serie);
         EntradasDcordem.AsString        := 'A';
         EntradasDctipo.AsString         := 'NF';
         EntradasVlnota.AsFloat          := Total.ICMSTot.vNF;
       //IcmsBase                        := Total.ICMSTot.vBC;
         EntradasVlicms.AsFloat          := Total.ICMSTot.vICMS;
         EntradasVlipi.AsFloat           := Total.ICMSTot.vIPI;
         EntradasVlfrete.AsFloat         := Total.ICMSTot.vFrete ;
         query1.Active := False;
         query1.SQL.Text := 'Select *from fornecedores where cgc = '+Emit.CNPJCPF;
         query1.Active := True;
         while(not query1.Eof )do
         begin
         EntradasCdfornecedor.AsInteger  := query1.fieldbyname('codcli').AsInteger;
         query1.Next;
         end;
      //   EntradasCfop.AsInteger          := Prod.CFOP ;
         EntradasDtentrada.AsDateTime    := StrToDate(FormatDateTime('dd/MM/YYYY',now ));
         EntradasDtemissao.AsDateTime    :=  Ide.dEmi;
         EntradasVlst.AsFloat            :=  Total.ICMSTot.vST;
         EntradasTpsituacao.AsString     := 'F';
        ShowMessage ('Nota Importada com sucesso');
        Entradas.Post;
        Entradas.ApplyUpdates();
        Entradas.Transaction.Commit;
     Except
        ShowMEssage('problema na gravação da nota');
        Entradas.Close;
     end;
        ...
        begin
          with Det.Items[I] do
           begin
             EntradasItens.Close;
             EntradasItens.Open;
             EntradasItens.Insert;
             try

              EntradasItensDcnumero.AsInteger      := Ide.nNF;
              EntradasItensDcserie.AsString        := IntToStr(Ide.serie);
              EntradasItensDcordem.AsString        := 'A';
              EntradasItensDctipo.AsString         := 'NF';
            //  EntradasItensCdfornecedor.AsInteger  := query1.fieldbyname('cdfornecedor').AsInteger;
              EntradasItensCdproduto.AsInteger     := StrToInt(Prod.cProd) ;
              //cfop                               := Prod.CFOP
              EntradasItensQuantidade.AsFloat      := Prod.qCom ;
              EntradasItensIpi.AsFloat             := 0;
              EntradasItensSt.AsFloat              := 0;
              EntradasItensIcms.AsFloat            := 0 ;
              EntradasItensVlunitario.AsFloat      := Prod.vUnCom ;
              EntradasItensVldesconto.AsFloat      := 0;
              EntradasItens.Post;
              EntradasItens.ApplyUpdates();
              EntradasItens.Transaction.Commit;
              Except
                ShowMEssage('Erro na gravação dos Itens');
               end ;
             EntradasItens.Close;
