...
$layout->blocks["top"][] = "search";$page_layouts["C000009_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "C000009_search.htm";
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("CNPJ"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("CNPJ")] = GetTableURL($pageObject->pSet->getLookupTable("CNPJ"));
	
	$pageObject->fillFieldToolTips("CNPJ");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("CNPJ");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("CNPJ_label","<label for=\"".GetInputElementId("CNPJ", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("CNPJ_label", true);
	
	$xt->assign("CNPJ_fieldblock", true);
	$xt->assignbyref("CNPJ_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("CNPJ_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("CNPJ_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_CNPJ", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("CNPJ");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CNPJ", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CNPJ", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if($pageObject->pSet->getLookupTable("IE"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("IE")] = GetTableURL($pageObject->pSet->getLookupTable("IE"));
	
	$pageObject->fillFieldToolTips("IE");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("IE");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("IE_label","<label for=\"".GetInputElementId("IE", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("IE_label", true);
	
	$xt->assign("IE_fieldblock", true);
	$xt->assignbyref("IE_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("IE_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("IE_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_IE", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("IE");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"IE", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"IE", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
