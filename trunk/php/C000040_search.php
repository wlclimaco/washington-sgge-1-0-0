...
$layout->blocks["top"][] = "search";$page_layouts["C000040_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "C000040_search.htm";
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("TITULAR"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("TITULAR")] = GetTableURL($pageObject->pSet->getLookupTable("TITULAR"));
	
	$pageObject->fillFieldToolTips("TITULAR");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("TITULAR");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "TITULAR";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("TITULAR_label","<label for=\"".GetInputElementId("TITULAR", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("TITULAR_label", true);
	
	$xt->assign("TITULAR_fieldblock", true);
	$xt->assignbyref("TITULAR_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("TITULAR_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("TITULAR_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_TITULAR", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("TITULAR");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"TITULAR", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"TITULAR", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
