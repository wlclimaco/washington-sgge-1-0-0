...
add_nocache_headers();
include("include/WEB_variables.php");
...
$layout = new TLayout("add3","BoldGreenTea","MobileGreenTea");
...
$layout->blocks["top"][] = "details";$page_layouts["WEB_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "WEB_inline_add.htm";
else
	$templatefile = "WEB_add.htm";
...
//	processing SENHA - start
	...
	if($inlineAddOption)
	{
		...
		$control_SENHA->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing OBSERVACAO - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_OBSERVACAO = $pageObject->getControl("OBSERVACAO", $id);
		$control_OBSERVACAO->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing SENHA - start
			...
			if($inlineAddOption)
			{
				$control_SENHA->afterSuccessfulSave();
			}
//	processing SENHA - end
//	processing EMAIL - start
			$inlineAddOption = true;
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'WEB_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'WEB_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: WEB_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	...
	$defvalues["OBSERVACAO"]=@$avalues["OBSERVACAO"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("SENHA_label","<label for=\"".GetInputElementId("SENHA", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("OBSERVACAO_label","<label for=\"".GetInputElementId("OBSERVACAO", $id, PAGE_ADD)."\">","</label>");
