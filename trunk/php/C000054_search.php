...
$layout->blocks["top"][] = "search";$page_layouts["C000054_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "C000054_search.htm";
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("PLACA"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("PLACA")] = GetTableURL($pageObject->pSet->getLookupTable("PLACA"));
	
	$pageObject->fillFieldToolTips("PLACA");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("PLACA");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("PLACA_label","<label for=\"".GetInputElementId("PLACA", $id, PAGE_SEARCH)."\">","</label>");
	else 
	...
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("PLACA");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"PLACA", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"PLACA", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
