	...
	processLightStatus : function(oStatusSetup){
		...
		if (oStatusSetup) {
			...
			if(oStatusSetup.lightStatusEnum == "ACTIVE"){
				...
				sStatus = sensus.locale.get("smartpoint.status."+oStatusSetup.lightStatusEnum)+sOverrride+"|"+oStatusSetup.lightStatusEnum.toLowerCase();
				
			} else if(oStatusSetup.lightStatusEnum == "MAINTENANCE" || oStatusSetup.lightStatusEnum == "DEACTIVATED"){
			
				sStatus = sensus.locale.get("smartpoint.status."+oStatusSetup.lightStatusEnum)+"<br>" + $.date.dateFormat(oStatusSetup.date, sensus.settings.dateFormatMask+' h:i:s.fff A') +sOverrride+"|"+oStatusSetup.lightStatusEnum.toLowerCase();
			
			} else {
			
				if (oStatusSetup.statusExceptions.length) {
				
					sStatus = sensus.locale.get("smartpoint.status."+oStatusSetup.statusExceptions[0].statusExceptionTypeEnum)+"<br>" + $.date.dateFormat(oStatusSetup.date, sensus.settings.dateFormatMask+' h:i:s.fff A') +sOverrride+"|"+oStatusSetup.lightStatusEnum.toLowerCase();
