...
sensus.pages.decorator = { 
	...
	sendAction : function() {
		...
		var oRequest = {'inquiryLightRequest' : new inquiryLightRequest(oSearch)};
		//TODO
		...
		var fnCallback = function(response){
			...
			if(iTotal == 0 || iTotal > 1){
				...
				sensus.commons.lib.ajax.do_load($.extend({}, sensus.commons.lib.ajax.param, {
					sUrl : "smartpoint/ajax.smartpoint.detail.main.action?id="+nId,
					bFilter : true
		...
		$.ajaxValidator.fnDoCall ("smartpoint/search.action", oRequest, false, fnCallback);
