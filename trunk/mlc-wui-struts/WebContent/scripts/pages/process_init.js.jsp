...
$(document).ready(function() {	
	...
	sensus.pages.process.eventHistoryTable = $('#event-history-table').dataTable(sensus.widgets.datatable.setTable({
		...
		sAjaxSource  : sensus.settings.searchProcessUrl,
		aColumns     : aColumns,
		oSettings    : {
		...
		fnRowCallback : function(nRow, aData, iDisplayIndex, col) {
			...
			if (nSmartpointCount < 1) {
				...
				if (nSmartpointCount > 1) {
					$("td:eq(" +col.smartpoint+ ")", nRow).html('<a href="smartpoint/ajax.list.action?lights='+ sActionValue +'" class="afilter launch">' + nSmartpointCount + '</a>');
				} else {
	   	...
	   	fnDrawCallback : function(setting, col) {
			...
			$(this).find(".row-header h4").each(function(i) {
				...
				try {
					...
					if ($.date.dateFormat(sOrderDate,'simpleDay') === sensus.locale.get("commons.pages.today")
						|| $.date.dateFormat(sOrderDate,'simpleDay') === sensus.locale.get("commons.pages.yesterday")) {

						$(this).html($.date.dateFormat(sOrderDate) + ": " + "<strong>" + $.date.dateFormat(sOrderDate,'simpleDay') + "</strong>");
					...
					success : function(response) {
						...
						for (i in aColumns) {
							...
							if (aColumns.hasOwnProperty(i)) {
								...
								oRow += "<td class='"+ sClass +"'><span>" + oSpan + sensus.locale.get('sensus.process.'+response.firstProcess.lcAction.actionType.toLowerCase()) + "</span></td>";
						...
						var sFormatDate = $.date.dateFormat(sDate, 'h:i:s.fff A');
						
						$("td:eq(" + iCreateUser + ")", oRow).text(response.firstProcess.createUser);
						
						$("td:eq(" + iStartTimeCol + ")", oRow).text(sFormatDate);

						$("td:eq(" + iStatusCol + ")", oRow).text(sensus.locale.get("table.type.complete"));

						if (bCompleted === true && bSubmited === true) {

							$("td:eq(13)", oRow).html('<a class="button summary-button" href="#">' + sensus.locale.get("table.summary") + '</a>');
	...
	$("#w-filters").wFilters(sFilters, sensus.pages.process.eventHistoryTable, wFn);	
