	...
	function processFilter(dStartDate, dEndDate, aActionCategoryList, sLightTextSearch, aUserIds)
	{
		...
		this.requestType = 'processFilter';
	}
...
//	}
	...
	function inquiryLightRequest (oSearch, aCurrentColumns) {
		...
		if(oSearch.requestType == 'processFilter'){
	...
	function inquiryPaginationRequest (oSearch, aCurrentColumns) {
		...
		this.listColumn		 = aCurrentColumns;
		this.dicionarios          = [{'id':$.address.parameter("id")}];
		this.startRow        = 0;
		this.endRow          = 0;
		this.pageSize        = 25;
		this.search          = oNewSearch;
	...
	function userRequest (sUserId, sUserName, sFirstName, aRole, sLastName, sPassword, sEmail, bAllLightsAuth, aGroups, paginationAllSelected, selectionPaginationIds) {
	...
	function analyticsRequest (statusExceptionTypeEnum) {

		try {
			// Convert String to object date
			var dStartDate = $.datepicker.parseDate(sensus.settings.dateFormatMask, $.address.parameter("di"));
			dStartDate.setHours(0);
			dStartDate.setMinutes(0);
			dStartDate.setSeconds(0);
			
			var dEndDate = $.datepicker.parseDate(sensus.settings.dateFormatMask, $.address.parameter("de"));
	...
	function dicionarioRequest (telaId,action, isAllRows, aIdsSelected, oSearchLight, aAtributoList, nBottomLeftLat, nBottomLeftLon, nTopRightLat, nTopRightLon, nMaxSmartpointCount) {

		this.tela               = {};
		this.tela.id_tela       = telaId;


		if (action) {

			this.group.modelAtion = action;
	...
	function groupRequest (groupName, groupDescription, groupOldName, groupId, action, isAllRows, aIdsSelected, oSearchLight, aGroupList, nBottomLeftLat, nBottomLeftLon, nTopRightLat, nTopRightLon, nMaxSmartpointCount) {
