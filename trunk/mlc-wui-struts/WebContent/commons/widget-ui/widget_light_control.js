...
$.fn.wLightControl = function (bLightList) {
	...
	var lightControlDialog = {
		 ...
		 * The dialog title.
		 */
		title : sensus.locale.get("action.switchlight.title"),
		/**
		...
		buttons : [{
				id     : "turn-on-submit", 
				text   : sensus.locale.get("action.switchlight.submit"),
				click  : function() {
					...
					if (bValidValue && bValidDate) {
						...
						$.ajaxValidator.fnMonitor("/slc/util/ajax.controlLights.action", oRequest, false, fnCallBack, sensus.locale.get("action.longRunningProcessDialog.confirm"), bMonitor);
						...
						if (!bValidValue) {
							...
							$('#radio3').validationEngine('showPrompt', sensus.locale.get("validation.multipleFields"), 'red', '', true);
...
*/
			...
			{
				id     : "turn-on-cancel", 
				text   : sensus.locale.get("action.switchlight.cancel"), 
				click  : function() {
					$(this).dialog('close');
				}
			}
						...
						if(oResponse.firstLight.lightStateEnum == 'ON'){
							...
							if(oResponse.firstLight.lightBlinkEnumValue != 0){
								...
								if (oResponse.firstLight.lightBlinkEnumValue == 2) {
									...
									sMsg = ' <strong>' + sensus.locale.get('smartpoint.actions.controlLights.slow') + '</strong>';

								} else if (oResponse.firstLight.lightBlinkEnumValue == 1) {

									sMsg = ' <strong>' + sensus.locale.get('smartpoint.actions.controlLights.fast') + '</strong>';
	...
	objThat.on("click", function() {
		...
		sensus.util.actiondialog.launchActionDialog("onOffLight", lightControlDialog, "control-light-dialog");
