		...
		FieldSetMapper<ModelItemWrapper<Drug>>
{
	...
	public ModelItemWrapper<Drug> mapFieldSet(FieldSet fieldSet)
	{
		...
		try
		{
			...
			drug.setModelAction(PersistanceActionEnum.INSERT);
		...
		catch (Exception e)
		{
			...
			wrapper.getModelObject().setModelAction(PersistanceActionEnum.ERROR);
	...
	private DrugPrice createDrugPrice(FieldSet fieldSet, String index)
	{
		...
		drugPrice.setModelAction(PersistanceActionEnum.INSERT);
		...
		try
		{
			drugPrice.setEffectiveDate(fieldSet.readDate("ndcEffDate" + index, "MM/dd/yyyy"));
		}
		catch (Exception ex)
		{
