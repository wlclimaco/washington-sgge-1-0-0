...
if [ $? -eq 0 ] ; then
  shout "### Postgresql Server is already installed and conflicts with new postgresql91 install"
  exit 1
fi
...
if [[ "$RC" == "1" ]]; then
  ...
  if yesno "Would you like to use external servers to install $PG_RPM_NAME?" ; then 
    ...
    wget -O /tmp/$PGDG_LATEST  http://yum.postgresql.org/9.1/redhat/rhel-5-x86_64/$PGDG_LATEST
...
runuser postgres -c "/usr/bin/patch -N -ub -i 9.1/patch.pg_hba.conf $PG_DIR/data/pg_hba.conf"
runuser postgres -c "/usr/bin/patch -N -ub -i 9.1/patch.postgresql.conf $PG_DIR/data/postgresql.conf"
