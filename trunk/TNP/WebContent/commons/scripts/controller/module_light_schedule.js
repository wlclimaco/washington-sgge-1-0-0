sensus.commons.modules.content.lightSchedule = {
	...
	fnReloadSchedule: function(aData) {
			...
			if(sensus.settings.timezone != aData.timeZone){
				...
				sensus.commons.modules.util.fnFillData($('.missing_data'), [$.sc.locale('smartpointdetail.differenttimezone')+ ' '+sensus.settings.timezone]);
				$('.missing_data').show();
			...
			sensus.commons.modules.util.fnFillData($('#sunrise-time'), [ $.sc.dateFormat( sSunRiseValue, 'h:i A')]);

			sensus.commons.modules.util.fnFillData($('#sunset-time'), [ $.sc.dateFormat( sSunSetValue, 'h:i A')]);
		...
		loadSchedules : function(){
			...
			var oRequest 	= new inquiryScheduleRequest(null);
			...
			$.sc.getJson("api/schedule/fetchAll", oRequest, false, fnCallback);
...
$('#tabs-content').on('click', '.remove-offset-schedule', function() {
	...
	$.ajaxValidator.fnMonitor("api/schedule/deletelights", eventScheduleRequest, false, fnCallBack, $.sc.locale("action.longRunningProcessDialog.confirm"), true);
	return false;
});
...
$('#tabs-content').on('click', '.apply-offset-schedule-action', function() {
	...
	if (sensus.util.combobox.getOption("#offset-schedule-add-select") != undefined) {
		...
		$.ajaxValidator.fnMonitor("api/schedule/insertlights", eventScheduleRequest, false, sensus.commons.modules.content.lightSchedule.fnUpdateSchedule, $.sc.locale("action.longRunningProcessDialog.confirm"), true);
...
$('#tabs-content').on('click', '.apply-event-schedule-action', function() {
	...
	if (sensus.util.combobox.getOption("#event-schedule-add-select") != undefined) {
		...
		$.ajaxValidator.fnMonitor("api/schedule/insertlights", offsetScheduleRequest, false, sensus.commons.modules.content.lightSchedule.fnUpdateSchedule, $.sc.locale("action.longRunningProcessDialog.confirm"), true);
