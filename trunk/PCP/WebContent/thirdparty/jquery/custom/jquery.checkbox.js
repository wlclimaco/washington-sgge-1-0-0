(function($) {
	$.widget("ui.checkbox", {
		_create : function() {
			...
			var input = $("<input>").val($.sc.locale("widgets.combobox.prompt3"))
									...
									.autocomplete({
										...
										change : function(event, ui) {
											if (!ui.item) {
												//$(this).val($.sc.locale("widgets.combobox.prompt3"));
			...
			$("<button class='button_filter ui-icon-list-ipt'>&nbsp;</button>").attr("tabIndex", -1).attr("title", $.sc.locale("widgets.combobox.title")).insertAfter(input).button( {
				icons : {
