...
$.fn.extend($.sc , function(){
	...
	var _filters = function (arrayObj, objTable, fnCallBack, sPage) {
		...
		var oRequest = {action : "build", filters : arrayObj};
/*
		for (l in arrayObj) {

			oRequest.filters.push({filterEnum : arrayObj[l]})
				...
				for (l in arrayObj) {
					...
					if (oConfig) {
						...
						if (oConfig.type == 'checkBox') {
							...
							objFilters.push({name : $.sc.locale('commons.pages.' + sFilterName), type : 'checkBox',	session : sFilterName, boxes : data[oConfig.sJava]});
		...
		*/
					...
					var objText = '';
						...
						if($('.filter-container').length) {
							...
							if (sType == 'text') {
								...
								if (tagName.length) {
									/** tag html template **/
									oTag = $('<li id="'+sFilterId+'"><span class="type remove"><a href="">' + tagName + '</a></span> <span class="title">' + unescape(sValue) + '</span></li>');
								}
							...
							if (sType == 'drawing') {
								...
								//sValue = $('#query').val();
								/** tag html template **/
								oTag = $('<li id="'+sFilterId+'"><span class="type remove"><a href="">'+tagName+'</a></span><span class="title"><span class="shape-icon" style="background:'+sensus.util.mapopen.vector.color+';opacity:0.6"></span>Shape '+ sensus.util.mapopen.vector.index +' ('+sValue+'km<sup>2</sup>)</span></li>');
		...
		var template = {
			...
			checkbox  : function(aData, id, toggle, all){
				...
				var iSelectSize = (aData.filterValue) ? aData.filterValue.length : 0;
				var sLabel = '';
				var aLi = '';
				var aOptions = '';
				...
				var i = 0;
				for (k in aData.filterValue) {

					if (aData.filterValue.hasOwnProperty(k)) {

						if (i < 6) {

							if(aData.filterValue[k]){

								if(aData.filterValue[k].length > 17) {

									sLabel = aData.filterValue[k].substr(0,19)+ '...';

								} else {

									sLabel = aData.filterValue[k];

								}

							aLi += '<li class="checkbox" title="'+aData.filterValue[k]+'"><input type="checkbox" value="'+[k]+'"> '+sLabel+'</li>';

							}

						} else {

							aOptions += '<option value="'+[k]+'">'+aData.filterValue[k]+'</option>';
		...
		var _buildFilters = function(response){

			for (x in response) {

				if (response.hasOwnProperty(x)) {

					if($.isPlainObject(response[x])){

						var sDivToClone = '#' + response[x].filterType;
						...
						if ((sDivToClone == '#searchFilter')||(sDivToClone == '#dateFilter')) {
							...
							if(template[response[x].filterType]){

								var sSession = x.toUpperCase();

								$('#filters').append(template[response[x].filterType](response[x], sSession,$.sc.locale('commons.pages.'+x),$.sc.locale('filter.all.'+x)));
		...
		$.sc.getJson("api/filters/fetch", oRequest, false, _buildFilters, null, true);
