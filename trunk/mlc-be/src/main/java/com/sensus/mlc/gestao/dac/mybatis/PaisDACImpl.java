...
public class PaisDACImpl extends SqlSessionDaoSupport implements IPaisDAC
{
	...
	private static final String UPDATE_PAIS = null;

	private static final String SENSUS_MLC_PAISVALIDATOR_PAIS_ALREADY_EXISTS = null;
	...
	public InternalResultsResponse<Pais> fetchAllPais(InquiryPaisRequest inquiryPaisRequest)
	{
		...
		if (!ValidationUtil.isNull(inquiryPaisRequest.getPais()))
		{
			paramMap.put(PAIS_ID, inquiryPaisRequest.getPais().get(0).getCodpais());
		}
	...
	public InternalResultsResponse<Pais> insertPais(PaisRequest paisRequest)
	{
		...
		pais.setCodpais((Integer)doQueryForObject(getSqlSession(), INSERT_PAIS, paisRequest));
        pais.setListinsalt((List<Auditoria>)doQueryForObject(getSqlSession(), INSERT_AUDITORIA, paisRequest));
		InternalResultsResponse<Pais> response = new InternalResultsResponse<Pais>();
