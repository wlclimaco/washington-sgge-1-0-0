...
public class MapDataValidator implements IValidator
{
	...
	/** The Constant SENSUS_MLC_VALIDATOR_MAX_SMARTPOINT_COUNT_REQUIRED. */
	private static final String SENSUS_MLC_VALIDATOR_MAX_SMARTPOINT_COUNT_REQUIRED =
			"sensus.mlc.validator.max.smartpoint.count.required";
	...
	private void validationMapData(LightSelectionRequest request, List<MessageInfo> list)
	{
		...
		validateMaxDeviceCount(list, request.getMaxSmartpointCount());
	...
	/**
	 * Validate max device count.
	 * 
	 * @param list the list
	 * @param maxSmartpointCount the max smartpoint count
	 */
	private void validateMaxDeviceCount(List<MessageInfo> list, Integer maxSmartpointCount)
	{
		ValidationUtil.isNull(maxSmartpointCount, SENSUS_MLC_VALIDATOR_MAX_SMARTPOINT_COUNT_REQUIRED, list);
	}
