package com.sensus.mlc.user.dac.mybatis;
...
public class UserDACImpl extends SqlSessionDaoSupport implements IUserDAC
{
	...
	public InternalResultsResponse<User> fetchAllUsers(InquiryPaginationRequest inquiryPaginationRequest)
	{
		...
		Integer totalRows =
				(Integer)doQueryForObject(this.getSqlSession(), USER_PAGINATION_TOTAL_ROWS, paramMap);

		InternalResultsResponse<User> response = new InternalResultsResponse<User>();
		doQueryForList(this.getSqlSession(), USER_FETCH_ALL_USERS, paramMap, response);
	...
	public InternalResultsResponse<User> insertUser(UserRequest userRequest)
	{
		...
		try
		{
			user.setId((Integer)doQueryForObject(this.getSqlSession(), USER_INSERT_USER, paramMap));
		}
	...
	public InternalResultsResponse<User> insertGroupsToUser(UserRequest userRequest)
	{
		...
		for (Group group : userRequest.getUser().getGroups())
		{
			...
			Integer inserted =
					(Integer)doQueryForObject(this.getSqlSession(), USER_INSERT_GROUPS_TO_USER, paramMap);
	...
	{
		InternalResultsResponse<User> response = new InternalResultsResponse<User>();
		doQueryForList(this.getSqlSession(), USER_FETCH_USER_BY_ID, userRequest.getUser(), response);
		return response;
	}
	...
	public InternalResponse updateUser(UserRequest userRequest)
	{
		...
		try
		{
			doQueryForObject(this.getSqlSession(), USER_UPDATE_USER, paramMap);
		}
		catch (DuplicateKeyException ex)
		{
			response.setStatus(Status.ExceptionError);
	...
	public InternalResponse deleteUser(UserRequest userRequest)
	{
		...
		for (Integer id : userRequest.getSelectionPaginationIds())
		{
			...
			doRemove(this.getSqlSession(), DELETE_USER, paramMap, response);
		}
	...
	{
		InternalResponse response = new InternalResponse();
		doRemove(this.getSqlSession(), DELETE_GROUPS_USER, userRequest.getUser(), response);
		return response;
	}
	...
	{
		InternalResponse response = new InternalResponse();
		doRemove(this.getSqlSession(), DELETE_ROLES_USER, userRequest.getUser(), response);
		return response;
	}
	...
	public InternalResultsResponse<User> insertRolesToUser(UserRequest userRequest)
	{
		...
		Integer inserted =
				(Integer)doQueryForObject(this.getSqlSession(), USER_INSERT_ROLES_TO_USER, paramMap);
	...
	public InternalResultsResponse<User> fetchUserByName(UserRequest userRequest)
	{
		...
		doQueryForList(this.getSqlSession(), USER_FETCH_USER_BY_NAME, paramMap, response);
		return response;
	}
