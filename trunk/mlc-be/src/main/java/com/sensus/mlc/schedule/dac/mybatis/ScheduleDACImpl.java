package com.sensus.mlc.schedule.dac.mybatis;
...
public class ScheduleDACImpl extends SqlSessionDaoSupport implements IScheduleDAC
{
	...
	/** The Constant FETCH_SMARTPOINT_BY_SCHEDULE. */
	private static final String FETCH_SMARTPOINT_BY_SCHEDULE = SCHEDULE_NAMESPACE + "fetchSmartPointBySchedule";
	...
	private static final String FETCH_SCHEDULE_BI_ID = SCHEDULE_NAMESPACE + "fetchScheduleById";
	...
	public InternalResultsResponse<Schedule> insertSchedule(ScheduleRequest scheduleRequest)
	{
		...
		for (Event eventData : eventSchedule.getEvents())
		{
			...
			paramMap.put(SCHEDULE_ID, idSchedule);
			paramMap.put(TENANT_ID, scheduleRequest.getTenant().getId());
	...
	public InternalResultsResponse<Schedule> fetchAllSchedules(InquiryScheduleRequest inquiryScheduleRequest)
	{
		...
		paramMap.put(TENANT_ID, inquiryScheduleRequest.getTenant().getId());
	...
	public InternalResponse updateSchedule(ScheduleRequest scheduleRequest)
	{
		...
		for (Event eventData : eventSchedule.getEvents())
		{
			...
			Integer eventId =
					(Integer)doQueryForObject(getSqlSession(), INSERT_SCHEDULE_EVENT, paramMap);
			for (DaysOfWeekEnum dayOfWeek : eventData.getDays())
			{
				paramMap.put(DAY_OF_WEEK_ID, dayOfWeek.getValue());
				paramMap.put(SCHEDULE_EVENT_ID, eventId);
				paramMap.put(CREATE_USER, scheduleRequest.getUserContext().getUserId());
				doInsert(getSqlSession(), INSERT_SCHEDULE_EVENT_DAYOFWEEK,
						paramMap, response);
				paramMap.clear();
			}
		}
	...
	{
		InternalResultsResponse<Schedule> response = new InternalResultsResponse<Schedule>();
		doQueryForList(getSqlSession(), FETCH_SCHEDULE_BI_ID, schedule, response);
	...
	/*
	 * (non-Javadoc)
	 * @see com.sensus.mlc.schedule.dao.IScheduleDAO#deleteSmartpointFromSchedule(com.sensus.mlc.schedule.model.request.
	 * ScheduleRequest)
	 */
	@Override
	public InternalResponse deleteSmartpointFromSchedule(ScheduleRequest scheduleRequest)
	{
		...
		for (Light scheduleLight : schedule.getLights())
		{
			paramMap.put(RNI_ID, scheduleLight.getRniId());
	...
	/*
	 * (non-Javadoc)
	 * @see com.sensus.mlc.schedule.dao.IScheduleDAO#applySmartPointToSchedule(com.sensus.mlc.schedule.model.request.
	 * ScheduleRequest)
	 */
	@Override
	public InternalResponse applySmartPointToSchedule(ScheduleRequest scheduleRequest)
	{
		...
		{
			paramMap.put(SMARTPOINT_ID, light.getSmartPointId());
	...
	public InternalResponse applyUnknownOffsetSchedule(ScheduleRequest scheduleRequest)
	{
		...
		for (Light scheduleLight : scheduleRequest.getSchedule().getLights())
		{
			paramMap.put(RNI_ID, scheduleLight.getRniId());
	...
	public InternalResultsResponse<Light> fetchSmartpointBySchedule(ScheduleRequest scheduleRequest)
	{
		...
		doQueryForList(getSqlSession(), FETCH_SMARTPOINT_BY_SCHEDULE, paramMap, response);
