...
public class UserDACImplTest extends AbstractTestBaseDAC
{
	...
	/* (non-Javadoc)
	 * @see com.sensus.mlc.base.AbstractTestBaseDAC#getUserDAC()
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.sensus.mlc.base.AbstractTestBaseDAC#setUserDAC(com.sensus.mlc.user.dac.IUserDAC)
	 */
	@Override
	@Resource
	...
	public void testUpdateUser()
	{
		...
		userRequestUpdate.setUser(user);
		...
		userRequest.setUser(createUser());
		userRequest.setUserContext(TestBaseUtil.createUserContext());
		userRequest.getUser().setUserName("TestUserUpd1");
		getUserDAC().insertUser(userRequest);

		user = getUserDefault();
		userRequestUpdate = createUserRequest(user);

		user.setUserName("TestUserUpd");
		user.setEmail("emailUserUpd");
		user.setModifyUser("QAT test");

		userRequest.setUser(user);

		response = getUserDAC().updateUser(userRequestUpdate);
		assertTrue("User already exists", SENSUS_MLC_USERVALIDATOR_USER_ALREADY_EXISTS == response.getMessageInfoList()
				.get(0).getCode());
	}
