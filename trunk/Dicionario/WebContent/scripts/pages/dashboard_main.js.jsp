...
sensus.pages.dashboard = {
	...
	header : {
			...
			for (var iId in aColumns) {
				...
				if (aColumns.hasOwnProperty(iId)) {
					...
					if(sId == 'totalconsumption' || sId == 'totalcarboncredits') {
						...
						var sDate = Globalize.format(new Date(aColumns[iId].date),'D');
		...
		*/
		init : function() {
			$.sc.getJson("api/dashboard/fetch", new analyticsRequest(null, "header"), false, this.fnHeaderCallback);
	...
	lightingControl : {
		...
		init : function () {
			$.sc.getJson("api/dashboard/fetch", new analyticsRequest(null, "control"), false, this.fnLightingControlCallback);
		}
	},
...
*/
		...
		init : function () {
			$.sc.getJson("api/dashboard/fetch", new analyticsRequest(null, "chart"), false, this.fnAlertsByTypeCallback);
		}
	},
			...
			var oDataTableProperties = {
					...
					fnRowCallback : function(nRow, aData, iDisplayIndex, col) {
						...
						if (aData[col.Date]) {
							...
							$("td:eq(" +col.Date+ ")", nRow).html($.sc.dateFormat(aData[col.Date], sensus.settings.DATE_FORMAT) + ' ' + $.sc.dateFormat(aData[col.Date], 'h:i:s.fff A'));
