		...
		util : {
			...
			ajaxData : {
				...
				fetchDeviceById : function () {
					var	obj = {
						url : "api/light/fetch",
						data : {"id": $.address.parameter('id'), "action": "id"}
					};
		...
		summaryData : {
			...
			lightInformation : {
				...
				fnSetOverride : function(aData){
					...
					if(parseInt(aData.overrideEnumValue) != 0){
						...
						$($.sc.locale('smartpointdetail.page.lightoverridetext.'+aData.overrideEnumValue, sMsg, $.date.dateFormat(sMsgDate, sensus.settings.dateFormatMask + " at h:i a"))).insertAfter(oCommunication);
			...
			lightActions : {
				...
				getDataFromLight : function() {
					...
					$.ajaxValidator.fnMonitor(sensus.settings.forceLightStatus, { 'lightRequest': new lightRequest($.address.parameter('id')) }, false, sensus.commons.modules.reloadData, $.sc.locale("action.longRunningProcessDialog.confirm"), true);
			...
			networkStatus : {
				...
				fillNetworkStatus : function (oResponse) {
					...
					if (oHanDevice) {
						$("#network-status-value").text($.sc.locale("commons.pages."+oHanDevice.hanDeviceStatusEnum));
					}
			...
			parent : {
				...
				fillParent : function (oResponse) {
					...
					if (oHanDevice && oHanDevice.baseFlexNetId) {
						$("#parent-value").html("<a href='lightDetail?id="+ oHanDevice.baseFlexNetId +"&deviceType=Meter' class='alist'>" + oHanDevice.baseFlexNetId + "</a>");
					} else {
						$("#parent-value").text("--");
					}
				},
			...
			actions : {
				...
				connectFunctions : {
					fnApplyHanDevice : function (oData) {
						...
						sensus.pages.longrunningprocess.monitor(sUrl, sData, $.sc.locale("action.connectHanDevice.confirm"), fnCallback);
					},
					...
					fnValidateInsertHanDevice : function (oData) {
						...
						var sSelect = $.sc.locale('commons.pages.select'),
						...
						if ((oData.sDeviceType === sSelect) || (oData.sDeviceId === "") || (oData.sManufacture === sSelect) || (oData.sModelNumber === sSelect))  {
							...
							if (oData.sDeviceType === sSelect) {
								...
								oDeviceType.validationEngine('showPrompt', $.sc.locale('commons.pages.fieldRequired'), 'red', '', true);

							}

							if (oData.sDeviceId === "") {

								oDeviceId.validationEngine('showPrompt', $.sc.locale('commons.pages.fieldRequired'), 'red', '', true);
								oDeviceId.val("");
							}

							if (oData.sManufacture === sSelect) {

								oManufacture.validationEngine('showPrompt', $.sc.locale('commons.pages.fieldRequired'), 'red', '', true);

							}

							if (oData.sModelNumber === sSelect) {

								oModelNumber.validationEngine('showPrompt', $.sc.locale('commons.pages.fieldRequired'), 'red', '', true);
					...
					fnValidateConnectToHanFields : function (oData) {
						...
						if ((oData.sInstallCode === "") || (oData.sInstallCode.search(regEx) === 0)
							...
							if ((oData.sInstallCode === "") || (oData.sInstallCode.search(regEx) === 0)) {
								...
								oInstallCode.validationEngine('showPrompt', $.sc.locale('commons.pages.fieldRequired'), 'red', '', true);
								oInstallCode.val("");
							}
					...
					fnValidateConnectHanFields : function (oData) {
						...
						if ((oData.sInstallCode === "") || (oData.sInstallCode.search(regExInstall) === 0) || (oData.sMacAddress.search(regExMac) === -1)) {
							...
							if (oData.sMacAddress.search(regExMac) === -1) {
								...
								oMacAddress.validationEngine('showPrompt', $.sc.locale('commons.pages.fieldRequired'), 'red', '', true);
				...
				connectHANDevice : {
					...
					fnFillManufactureByType : function (typeId, oDom) {
						...
						if (oDom.oSelectDeviceTypes.val() != $.sc.locale("commons.pages.select")) {
					...
					fnFillModelNumberByManufacture : function (typeId, sManufacture, oDom ) {
						oDom.oSelectModelNumber.empty();
						if (oDom.oSelectManufacture.val() != $.sc.locale("commons.pages.select")) {
		...
		content : {
			...
			group : {
				...
				dialogDeleteGroup:{
					 ...
					 * The dialog title.
					 */
					title : $.sc.locale("commons.pages.removeConfirm"),
					...
					buttons : (function() {
						...
						buttons[$.sc.locale("commons.pages.remove")] = function() {
							...
							if (oResponse) {
								...
								if (oResponse.operationSuccess) {
									...
									sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("smartpointdetail.tabs.about.deleteGroup", sensus.commons.modules.content.group.sNameGroup), "confirm");
					...
					action : function(actionDialog) {
						...
						$('#action-dialog').text($.sc.locale("smartpointdetail.tabs.about.deleteGroupPermanently",$('#meter-id').text(),sensus.commons.modules.content.group.sNameGroup));
				...
				fnLoadGroups : function(sId){
					var i = 0,
						...
						for( ; i < iGroupByMeterSize; i = i + 1){
							...
							sTableGroupHtml +='<tr>';
							sTableGroupHtml +='<td class="remove-group" id="removeGroup"><a href="#"  id="'+ sGroupId +'">'+ $.sc.locale("commons.pages.remove") +'</a></td>';
				...
				fnValidateGroup: function(sGroupName){
					...
					*/
					...
					if (!bHasGroup) {
						...
						oComboGroupInput.validationEngine(sShowPrompt, $.sc.locale('commons.pages.selectGroup'), 'red', '', true);

					} else if ( sGroupName != '' &&  sGroupName != $.sc.locale('smartpointdetail.tabs.about.selectGroup')) {
						...
						if ((sGroupName.length <= 100) && (sGroupName.match(/^(?:(?:[a-z0-9#]*(?:(?:(?:\s?-\s?|'s\s?|[(?:s)]'\s?|\s)[a-z0-9#]*)?))\s?)*$/i) != null)) {
							...
							oComboGroupInput.validationEngine(sShowPrompt, $.sc.locale('commons.pages.characterInvalid'), 'red', '', true);
							...
							oComboGroupInput.validationEngine(sShowPrompt, $.sc.locale('commons.pages.max.characterJs','100'), 'red', '', true);
						...
						oComboGroupInput.validationEngine(sShowPrompt, $.sc.locale('groupcreate.form.name.required'), 'red', '', true);
				...
				init : function() {
					...
					*/
					oAddGroups.click(function(e) {
						e.preventDefault();
						oBlankslateGroup.hide();
						oComboGroupInput.val($.sc.locale('smartpointdetail.tabs.about.selectGroup'));
						oDivGroups.show();
					});
						...
						*/
						if(sensus.commons.modules.content.group.fnValidateGroup(sGroupName)){
							...
							if (oResponse) {
								...
								if(oResponse.operationSuccess){
									/** Show message Sucess */
									sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("smartpointdetail.tabs.about.sucessAddToGroup", sGroupName), "confirm");
									oDivGroups.hide();
									sensus.commons.modules.content.group.fnLoadGroups(sDeviceId);
			...
			tag : {
				dialogDeleteTag:{
					...
					action : function(actionDialog) {
						...
						$('#action-dialog').text($.sc.locale("smartpointdetail.tabs.about.deleteTagPermanently",$('#meter-id').text(),sensus.commons.modules.content.tag.sNameTag));
				...
				fnValidateTag: function(sTagName){
					...
					if ( sTagName != '') {
						...
						if ((sTagName.length <= 100) && (sTagName != $.sc.locale('smartpointdetail.tabs.about.selectTag')) && ((sTagName.match(/^(?:(?:[a-z0-9#]*(?:(?:(?:\s?-\s?|'s\s?|[(?:s)]'\s?|\s)[a-z0-9#]*)?))\s?)*$/i) != null) && (sTagName.replace(/\s/g,'').length != 0))) {
						...
						} else if (sTagName === $.sc.locale('smartpointdetail.tabs.about.selectTag')) {

							oComboTagInput.validationEngine('showPrompt', $.sc.locale('tag.page.error.invalidTagName'), 'red', '', true);
							return false;

						} else if ((sTagName.match(/^(?:(?:[a-z0-9#]*(?:(?:(?:\s?-\s?|'s\s?|[(?:s)]'\s?|\s)[a-z0-9#]*)?))\s?)*$/i) == null) || (sTagName.replace(/\s/g,'').length == 0)) {

							/** Show error in case of invalid character */
							oComboTagInput.validationEngine('showPrompt', $.sc.locale('commons.pages.characterInvalid'), 'red', '', true);
							return false;

						} else {

							/** Show error in case of limit character */
							oComboTagInput.validationEngine('showPrompt', $.sc.locale('commons.pages.max.characterJs','100'), 'red', '', true);
							return false;
						...
						oComboTagInput.validationEngine('showPrompt', $.sc.locale('tag.page.error.required'), 'red', '', true);
						return false;
				...
				init : function() {
					...
					*/
					oAddTags.click(function(e) {
						e.preventDefault();
						oBlankslateTag.hide();
						oComboTagInput.val($.sc.locale('smartpointdetail.tabs.about.selectTag'));
						oDivTags.show();
					});
						...
						*/
						if(sensus.commons.modules.content.tag.fnValidateTag(sTagName)){
							...
							if (oResponse) {
								...
								if(oResponse.operationSuccess){
									/** Show message Sucess */
									sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("smartpointdetail.tabs.about.sucessAddToTag", sTagName), "confirm");
									var oDivTags = $('#div-tags');
									oDivTags.hide();
									sensus.commons.modules.content.tag.fnLoadTags(sDeviceId);
									...
									if($.isArray(oResponse.messageList)){
										...
										sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("commons.pages.error"), "error");
									}
				...
				fnLoadPostNote : function(sId){
					...
					var i = 0,
						...
						for( ; i < iPostNote; i = i + 1) {
							...
							sPostNoteHtml += '<span class="what start rounded">'+$.sc.locale('smartpointdetail.tabs.about.note')+'</span>';
							...
							sPostNoteHtml += '<small> | <a href="#" id="'+sIdPostNote+'" class="text-button delete-dialog" title="Delete Note for Meter">'+$.sc.locale('commons.pages.delete')+'</a></small>';
							sPostNoteHtml += '</dt>';
				...
				fnValidatePostNote: function(sPostNoteName){
					...
					if (sPostNoteName.length > 0) {
					    ...
					    if (sPostNoteName.length < 1001 || sPostNoteName.length > 0) {
							...
							/** Show error in case of limit character */
							sensus.util.page.showMessage(sMessagingSmartpoint, $.sc.locale('commons.pages.max.characterJs','1000'), "error");
							return false;
						}
						...
						sensus.util.page.showMessage(sMessagingSmartpoint, $.sc.locale('smartpointdetail.tabs.about.noteEmpty'), "error");
						return false;
				...
				init : function() {
						...
						if (sensus.commons.modules.content.postNote.fnValidatePostNote(sPostNoteName)) {
							...
							if (sPostNoteName.length < 1001) {
								...
								if (oResponse) {
									...
									if(oResponse.operationSuccess){
										/** Show message Sucess */
										sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("smartpointdetail.tabs.about.sucessAddToNote", sPostNoteName), "confirm");
										oNewNote.val('');
										sensus.commons.modules.content.postNote.fnLoadPostNote(sDeviceId);
								...
								sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("smartpointdetail.tabs.about.postNote.maxLength"), "error");
					...
					oNotes.delegate(sDeleteNotes, "click", function(e) {
						...
						if(oResponse){
							...
							if(oResponse.operationSuccess){
								/** Show message Sucess */
								sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("smartpointdetail.tabs.about.sucessDeletedNote"), "confirm");
								sensus.commons.modules.content.postNote.fnLoadPostNote(sDeviceId);
								sensus.util.page.stopProgressBar(0);
							} else {
								sensus.util.page.showMessage(sMessagingSmartpointDetail, $.sc.locale("commons.pages.error"), "error");
				...
				fnLoadDeviceInformation : function(){
					...
					if(aMeter){
						...
						if (aMeter.deviceType){
							...
							sDeviceType = $.sc.locale("commons.pages."+aMeter.deviceType.type.toUpperCase());
						}

						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.tabs.about.meterType")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.LCM") || sDeviceType == $.sc.locale("commons.pages.THERMOSTAT") || sDeviceType == $.sc.locale("commons.pages.IHD")){


							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.tabs.about.deviceType")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
							...
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.dialog.deviceInformation.manufacture");
							sTableHtml += '</td>';
							sTableHtml += '<td>';
							...
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.dialog.deviceInformation.modelNumber");
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.tabs.about.meterFirmWare")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						sTableHtml += '<tr>';
						sTableHtml += '<td>';
						sTableHtml += $.sc.locale("commons.pages.networkAddress")+':';
						sTableHtml += '</td>';
						sTableHtml += '<td>';
						...
						sTableHtml += '<tr>';
						sTableHtml += '<td>';
						sTableHtml += $.sc.locale("commons.pages.premiseId")+':';
						sTableHtml += '</td>';
						sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.LCM") || sDeviceType == $.sc.locale("commons.pages.THERMOSTAT") || sDeviceType == $.sc.locale("commons.pages.IHD")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.dialog.deviceInformation.installCode");
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("commons.pages.ipAddress")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.tabs.about.state")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.tabs.about.installedDate")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.tabs.about.timeZone")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("smartpointdetail.tabs.about.encryption")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(sDeviceType == $.sc.locale("commons.pages.METER")){
							sTableHtml += '<tr>';
							sTableHtml += '<td>';
							sTableHtml += $.sc.locale("commons.pages.connectStatus")+':';
							sTableHtml += '</td>';
							sTableHtml += '<td>';
						...
						if(iSchedulesSize == 0){
							...
							for( ; i < iSchedulesSize; i = i + 1){
								...
								var sId                 = aSchedules[i].id,
									...
									sFrequencyTypeEnum  = $.sc.locale("commons.pages."+aSchedules[i].frequency.frequencyTypeEnum);
								...
								sTableHtml += '<td>'+$.sc.locale(sActionName)+'</td>';
					...
					if(iLcmSize == 0){
						...
						for( ; i < iLcmSize; i = i + 1){
							...
							var iRelay = aLcm[i].relay,
								...
								bUsed = aLcm[i].used,
								sUsed = $.sc.locale("commons.pages.yes");


							if(!bUsed){
								sUsed = $.sc.locale("commons.pages.no");
							}

							sTbodyHtml += '<tr class="">';
							sTbodyHtml += '<td>'+iRelay+'</td>';
							sTbodyHtml += '<td>'+sAmp+' <small>'+$.sc.locale("smartpointdetail.tabs.about.amp")+'</small></td>';
						...
						if(iDemandResponseProgramParticipation == 0){
							...
							for( ; i < iDemandResponseProgramParticipation; i = i + 1){
								...
								var sEventTitle = aDemandResponseProgramParticipation[i].eventTitle,
									...
									sStatus = $.sc.locale("commons.pages."+aDemandResponseProgramParticipation[i].executionStatusEnum);
			...
			hanDevices : {
				...
				hanDeviceInformationDialog:{
					 ...
					 * The dialog title.
					 */
					title : $.sc.locale("smartpointdetail.dialog.deviceInformation.hanDeviceDetails"),
							...
							$.ajax({
								...
								success : function (data) {
									...
									var oHanDevice = sensus.commons.modules.util.fnGetFirstDeviceResponse(data);
										...
										sStatus = $.sc.locale("commons.pages."+oHanDevice.hanDeviceStatusEnum),
									...
									if (oHanDevice.deviceType && oHanDevice.deviceType.type) {
										sDeviceType = $.sc.locale("commons.pages." + oHanDevice.deviceType.type);
									}

									oViewDetail.attr('href', 'lightDetail?id='+sFlexNetId + '&deviceType=' + oHanDevice.deviceType.type).addClass('alist').click(function(){
										actionDialogId.dialog('close');
									});
				...
				dialogDeleteHanDevices : {
					...
					buttons : (function() {
						...
						buttons[$.sc.locale("commons.pages.remove")] = function() {
							...
							sensus.pages.longrunningprocess.monitor(sensus.settings.removeMeterFromHanDevice, oData, $.sc.locale("smartpointdetail.tabs.about.sucessRemoveHan"), fnCallback);
					...
					action : function(actionDialog) {
						...
						actionDialogId.text($.sc.locale("smartpointdetail.tabs.about.removeHanDevice",sensus.commons.modules.content.hanDevices.sNameHanDevice, sensus.commons.modules.content.hanDevices.idDevice));
					...
					var i = 0,
						...
						if(iHanDeviceSize == 0){
							...
							for( ; i < iHanDeviceSize; i = i + 1){
								...
								var sType = $.sc.locale("commons.pages."+aHanDevice[i].hanDeviceTypeEnum),
									sFlexNetId = aHanDevice[i].clientEndPointId,
									sStatus = '',
									SLastDateStatus = aHanDevice[i].lastDateStatus,
									oRemoveHAN = '<td><a href="'+sFlexNetId+'" class="icn-delete icon-small remove-dialog" rel="Remove" title="'+$.sc.locale("smartpointdetail.tabs.about.removeHanDevice", sType, sId)+'"></a></td>';

								if(aHanDevice[i].hanDeviceStatusEnum){
									sStatus = $.sc.locale("commons.pages."+aHanDevice[i].hanDeviceStatusEnum);
								}
