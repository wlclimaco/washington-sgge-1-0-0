...
function TForm7.POPULANFENTRADAS():Boolean;
 BEGIN
         ...
         NFFISCAL.VL_SERV_NT := 0; //(Valor total dos serviços não-tributados pelo ICMS)
         NFFISCAL.VL_TERC    := 0; // (Valor total cobrado em nome de terceiros)
         NFFISCAL.VL_DA      := 0; //(Valor total de despesas acessórias indicadas no documento fiscal)
         ...
         NFFISCAL.TP_LIGACAO     := 2 ; //(Código da informação complementar do documento fiscal (campo 02 do Registro 0450))
         NFFISCAL.COD_GRUPO_TENSAO := '1';
         NFFISCAL.TP_CT    :=   '0';
         NFFISCAL.CHV_CTE_REF := TXTCHACESSO.Text;
         NFFISCAL.IND_FRT     := TXTFRETE.Text;   // '2'; // (Indicador do tipo do frete:){0- Por conta de terceiros;1- Por conta do emitente;2- Por conta do destinatário;9- Sem cobrança de frete.}
         NFFISCAL.CODNFE      := 0;
         if (TXTEMPRESA.Text = '')then
...
if( MessageDlg(' Deseja fechar a NOTA', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
 ...
 NFFISCAL.COD_SIT := 'A';
      if (GRAVARNFENTRADAS = True ) then
          ...
          ShowMessage('ERRO ao gravar a nota verifique as informações e tente novamente');

 // else
 // ShowMEssage('Valor do TITULO e ITENS não bate com o valor da nota');
end;
...
begin
  ...
  ELSE
      ShowMEssage('ERRO ao gravar o TITULO verifique as informações e tente novamente');
...
if MessageDlg('Deseja fazer RATEIO', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  ...
  ELSE
      ShowMEssage('ERRO ao gravar o ITENS verifique as informações e tente novamente');
...
begin
  ...
  ELSE
      ShowMEssage('ERRO ao ALTERAR ITEM verifique as informações e tente novamente');
...
begin
  if (DBGrid1.Fields[1].asString = 'A')then
     ...
     ShowMessage('Titulo encontra -se pago e não pode ser excluso !!');
end;
    ...
    with ACBrNFe1.NotasFiscais.Items[n].NFe do
     begin
        ...
        NFFISCAL := TNFISCAL.Create;
         sai := False;
         NFFISCAL.IND_EMIT  := 1;
         NFFISCAL.COD_PART  := 2; //(Indicador do emitente do documento fiscal:0- Emissão própria;1- 2Terceiros)
         repeat
         ...
         sai := True;
          ...
          ELSE
           ...
           ELSE
             if MessageDlg('Fornecedor não encontrado deseja cadastrar !!', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
             begin
              Form10.limpar;
              Form10.POPULARTXT();
              ...
              Form10.ShowModal;
               ...
               begin
                 ShowMessage('Nota não importada favor lançar o fornecedor');
                 sai := True;
                 Form7.Close;
         ...
         NFFISCAL.IND_FRT     := '2'; // (Indicador do tipo do frete:){0- Por conta de terceiros;1- Por conta do emitente;2- Por conta do destinatário;9- Sem cobrança de frete.}
           ...
           repeat
             if (ValidaProdutos(StrToInt(ajustaSpaco(Prod.cProd)),ajustaSpaco(Prod.NCM)))then
              begin
                ...
                if MessageDlg('PRODUTO não encontrado deseja cadastrar !!', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
                    begin
                      FRMCADPROD.limpar();
            ...
            until (sai <> False);
              ...
              ShowMEssage('gravação dos Itens');
              Except
...
POPULANFENTRADAS();
      if (Alterar = True ) then
          ...
          ShowMessage('ERRO ao na ALTERAÇÂO da nota verifique as informações e tente novamente');
