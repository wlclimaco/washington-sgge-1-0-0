...
(function ($) {
	$.date = (function() {
		...
		var fnFormatterTimeZone = function(oDate) {
			...
			var iDateMilliseconds  = oUtc + (3600000 * parseFloat(sensus.settings.TIME_ZONE_HOURS));

			return new Date(iDateMilliseconds);
		};

		/**
		 * Apply timeZone of tenant on the date
		 *
		 * @return Object Date in the timeZone
		 */
		var fnFormatterTenantTimeZone = function(oDate) {

			var oUtc               = oDate.getTime();
			var iDateMilliseconds  = oUtc + (3600000 * parseFloat(sensus.settings.userContext.tenant.timezoneRawOffset));
		...
		var fnGetDate = function(sDate, applyTimeZone, applyTenantTimeZone) {
		//console.log(sDate);
			...
			if(applyTenantTimeZone){
				...
				dDate = fnFormatterTenantTimeZone(dDate);
		 ...
		 * 				Date formated
		 */
		var fnFormatSimpleDay = function(sDateFormated, sDate, sElseFormat, bApplyTimeZone, bApplyTenantTimeZone) {
			...
			if(sDateFormated === dToday) {
				...
				if(sDateFormated === dYesterday) {
					...
					sDateFormated = (sElseFormat === "empty") ? "" : $.datepicker.formatDate(sElseFormat, fnGetDate(sDate, bApplyTimeZone, bApplyTenantTimeZone));
		...
		return {
			 ...
			 * 			Formated date
			 */
			dateFormat : function(sDate, sFormat, sElseFormat, bApplyTimeZone, bApplyTenantTimeZone) {
				...
				if(sFormat === "simpleDay")	{
					...
					sFormat     = fnSimpleDay();
					sDateFormat = $.datepicker.formatDate(sFormat, fnGetDate(sDate,bApplyTimeZone,bApplyTenantTimeZone));
					sDateFormat = fnFormatSimpleDay(sDateFormat,sDate,sElseFormat,bApplyTimeZone, bApplyTenantTimeZone);
					//Format Hour
			 ...
			 * 			string date in json format
			 */
			fnTenantTimeToUTC : function(dDate) {
