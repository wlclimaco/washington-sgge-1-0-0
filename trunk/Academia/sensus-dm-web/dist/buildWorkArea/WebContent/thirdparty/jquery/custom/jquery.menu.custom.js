...
function Menu(caller, options) {
	...
	this.create = function() {
		...
		if (options.actionCallback) {
			menu.actionCallback = options.actionCallback;
		}
	...
	this.chooseItem = function(item) {
		var itemObj = $(item);
		var option = itemObj.attr('id').split("-");
		menu.kill();
		if (option[0] === "applyScheduledEvent" || option[0] === "applyScheduledEventToGroup") {
			sensus.util.page.scheduledEventName = $(item).text();
			sensus.util.page.scheduledEventId = $(item).attr("name");

			sensus.util.page.smartpointId = $(".meterId").text();
		}
		if (option[0] === "applyDevicesToAction") {
			sensus.util.page.actionName = $(item).text();
			sensus.util.page.actionId = $(item).attr("name");
			//sensus.util.page.smartpointId = $(".meterId").text();
		}
		// edit this for your own custom function/callback:
		// action menu events for
		menu.actionCallback(itemObj.attr('id'),itemObj.attr('name'));
		$('#menuSelection').text(itemObj.text());
		// location.href = itemObj.attr('href');
		return false;
	};
};
	...
	container.addClass('fg-menu-flyout').find('li:has(ul)').each(
			function() {
				...
				$(this).find('a:eq(0)').addClass('fg-menu-indicator').html(
						'<span>' + $(this).find('a:eq(0)').text() + '</span><span class="ui-icon ' + options.nextMenuLink + '"></span>').hover(function() {
				...
				$(this).addClass('fg-menu-indicator').each(function() {
					$(this).html('<span>' + $(this).text() + '</span><span class="ui-icon ' + options.nextMenuLink + '"></span>');
							...
							// or initialize top breadcrumb
								...
								var newCrumb = $('<li class="fg-menu-current-crumb"><a href="javascript: return false;" class="fg-menu-crumb">' + crumbText + '</a></li>');
								newCrumb.appendTo(breadcrumb).find('a').click(function() {
									if ($(this).parent().is('.fg-menu-current-crumb')) {
										...
										// make this the current crumb, delete
										// all breadcrumbs after this one, and
...
jQuery.fn.getTotalWidth = function() {
	return $(this).width() + parseInt($(this).css('paddingRight')) + parseInt($(this).css('paddingLeft')) + parseInt($(this).css('borderRightWidth'))
			+ parseInt($(this).css('borderLeftWidth'));
};

jQuery.fn.getTotalHeight = function() {
	return $(this).height() + parseInt($(this).css('paddingTop')) + parseInt($(this).css('paddingBottom')) + parseInt($(this).css('borderTopWidth'))
