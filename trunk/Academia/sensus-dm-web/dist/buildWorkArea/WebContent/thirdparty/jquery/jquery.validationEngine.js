    ...
    var methods = {
        ...
        _required: function(field, rules, i, options) {
            switch (field.prop("type")) {
                ...
                case "file":
                default:
                    if (!field.val() || !field.val().replace(/\s/g, '').length) {
                        return options.allrules[rules[i]].alertText;
                    } else {
                    	// remove white space after and before the text
                    	var sValue = field.val().replace(/\s{2,}/g, '');
                    	field.val(sValue);
                    }
                    break;
                case "radio":
        ...
        _past: function(field, rules, i, options) {
            ...
            var oDate = null 
            try {
            	oDate = $.datepicker.parseDate(sMask,field.val());
            } catch (ex) {}
            
            var vdate = methods._parseDate($.datepicker.formatDate('yy-mm-dd', oDate));
            
            if (vdate > pdate ) {
                var rule = options.allrules.past;
                if (rule.alertText2) return rule.alertText + methods._dateToString(pdate) + rule.alertText2;
                return rule.alertText + $.datepicker.formatDate(sMask, new Date(pdate));
            }
        },
        /**
