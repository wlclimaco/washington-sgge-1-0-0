...
import com.sensus.lc.base.validation.util.LCValidationUtil;
import com.sensus.lc.base.validation.util.LightConfigurationValidationUtil;
import com.sensus.lc.base.validation.util.LightLastOpDateValidationUtil;
import com.sensus.lc.base.validation.util.LightLocationValidationUtil;
...
public class MlcServerValidator implements IValidator
{
	...
	/** The Constant SENSUS_MLC_LIGHTVALIDATOR_SUNRISEOFFSET_REQUIRED. */
	...
	private void validateAppCode86Data(List<MessageInfo> list, AppCode86Info lightBindingDetail)
	{
		...
		isNull(lightBindingDetail.getDeviceVersionMajor(), SENSUS_MLC_LIGHTVALIDATOR_FIRMWARE_REQUIRED, list);
		isNull(lightBindingDetail.getDeviceVersionMinor(), SENSUS_MLC_LIGHTVALIDATOR_FIRMWARE_REQUIRED, list);
		isNull(lightBindingDetail.getDeviceVersionPatch(), SENSUS_MLC_LIGHTVALIDATOR_FIRMWARE_REQUIRED, list);
	}
	...
	private void validateAppCode11Data(List<MessageInfo> list, AppCode11Info appCode11Data)
	{
		...
		LightLastOpDateValidationUtil.validateCurrent(appCode11Data.getCurrent(), list);
		LightLastOpDateValidationUtil.validateVoltage(appCode11Data.getVoltage(), list);
		LightConfigurationValidationUtil.validateTemperature(appCode11Data.getTemperature(), list);
	...
	{
		isNull(lightState, SENSUS_MLC_LIGHTVALIDATOR_LIGHT_STATE_REQUIRED, list);
	}
	...
	private void validateAppCode85Data(List<MessageInfo> list, AppCode85Info appCode85Info)
	{
		...
		isNull(appCode85Info.getSunriseOffset(), SENSUS_MLC_LIGHTVALIDATOR_SUNRISEOFFSET_REQUIRED, list);
		isNull(appCode85Info.getSunsetOffset(), SENSUS_MLC_LIGHTVALIDATOR_SUNSETOFFSET_REQUIRED, list);
	}
	...
	private void validateAppCode90Data(List<MessageInfo> list, AppCode90Info appCode90Info)
	{
		...
		isNull(appCode90Info.getLightState(), SENSUS_MLC_LIGHTVALIDATOR_LIGHT_STATUS_REQUIRED, list);
	...
	private void validateAppCode90DataReadLight(List<MessageInfo> list, AppCode90Info appCode90Info)
	{
		...
		// Operational Data
		LightLastOpDateValidationUtil.validateCurrent(appCode90Info.getCurrent(), list);
		LightLastOpDateValidationUtil.validateVoltage(appCode90Info.getVoltage(), list);
		LightLastOpDateValidationUtil.validateConsumption(appCode90Info.getConsumption(), list);
	}
	...
	private void validateInstallationDetail(InstallationInfo installationDetail, List<MessageInfo> list)
	{
		...
		{
			list.add(MessageInfo
					.createFieldValidationError(SENSUS_MLC_LIGHTVALIDATOR_INSTALLATION_INFO_REQUIRED,
							installationDetail));
			return;
		}

		LightLocationValidationUtil.validateTimeZoneRegion(installationDetail.getTimezoneRegion(), list);
	}
	...
	private void validateAppCode94Data(List<MessageInfo> list, AppCode94Info appCode94Data)
	{
		...
		{
			list.add(MessageInfo.createFieldValidationError(SENSUS_MLC_LIGHTVALIDATOR_SERIAL_NUMBERS_REQUIRED));
			return;
		}

		LightConfigurationValidationUtil.validateLowerCaseSerial(appCode94Data.getLowerCaseSerial(), list);
		LightConfigurationValidationUtil.validateUpperCaseSerial(appCode94Data.getUpperCaseSerial(), list);
		LightConfigurationValidationUtil.validateBulbSerial(appCode94Data.getBulbSerial(), list);
		LightConfigurationValidationUtil.validateBallastSerial(appCode94Data.getBallastSerial(), list);
		LightConfigurationValidationUtil.validateProductNumber(appCode94Data.getProductNumber(), list);
