	...
	$.moduleController = (function() {
		...
		*/
		...
		return {
			...
			* Initial Function
			*/
			init : function(oParameter, sDynamicTabId, oResponse) {
				...
				if (oParameter)
				{
					...
					$.ajax({
						url: "lightDetail/openlightdetailComponent",
						type: "POST",
						data:  oProperties,
						async : false,
						success: function(result) {
							...
							if (!sensus.commons.modules.ajaxCache.length && $.sc.isValidPreLoad(oResponse, true)) {
								sensus.commons.modules.ajaxCache.push({
									oResponse:	oResponse.lightDetailResponse,
									url: "api/light/fetchById/" + $.address.parameter('id'),
									bPreLoad: true
								});
								sensus.commons.modules.ajaxCache.push({
									oResponse:	oResponse.groupResponse,
									url: "api/group/fetchall",
									bPreLoad: true
								});
								sensus.commons.modules.ajaxCache.push({
									oResponse:	oResponse.tagResponse,
									url: "api/tag/fetch",
									bPreLoad: true
								});
								sensus.commons.modules.ajaxCache.push({
									oResponse:	oResponse.scheduleResponse,
