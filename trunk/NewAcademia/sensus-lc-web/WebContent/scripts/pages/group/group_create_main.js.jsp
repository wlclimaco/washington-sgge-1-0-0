...
sensus.pages.groupInsert = {
	...
	ajaxCall : function (sUrlAdress,sMessage)
	{
		...
		var iGroupIdFromPath = $.address.path().split('/').pop();
		var iGroupId = (!isNaN(iGroupIdFromPath))? iGroupIdFromPath : $.address.parameter('groupId');


		//Create request of creating or update
		if (iGroupId)
		{
			var request = new groupRequest(sGroupName, sGroupDescription,sGroupNameOld,iGroupId);
		}
		else
		{
			var request = new groupRequest(sGroupName, sGroupDescription,sGroupNameOld);
		}
