...
sensus.pages.analytics = {
		...
		graphicCategories		: [],
		oPreLoadResponseChart	: null,
		oPreLoadResponseTable	: null,
		...
		setDateInterval : function(e, iPage) {
			...
			if (e.length)
			{
				...
				{
					dInitDate = dInitDate.addMonths(-1);
				}
				else if (oOption == '3m')
				{
					dInitDate = dInitDate.addMonths(-3);
				}
				else if (oOption == '365')
				{
					dInitDate = dInitDate.addMonths(-12);
				}
				else
				{
				...
				{
					if ((iPage == null) && (oOption == 'ytd' || oOption == '3m' || oOption == '7' || oOption == '365'))
					{
						dEndDate.setDate((dEndDate.getDate()) - 1);
					}

					if ((iPage==1 || iAnalyticsType == 5 || iAnalyticsType == 6) && $.address.parameter('dt')!='1d' && oOption != 'ytd')
					{
				...
				if ((iAnalyticsType == 5 || iAnalyticsType == 6 ) && oOption == '0')
				{
					dInitDate = new Date();
					dEndDate  = dInitDate;
				}
				if ($.address.parameter("dt") == '0' && $.sc.isNullOrUndefined($.address.parameter("og")))
				{
					dInitDate = $.address.parameter("di");
					dEndDate  = $.address.parameter("de");
				}
		...
		fillChartParameters : function() {
			...
			//set chart type
			if($.address.parameter("chart") == 'normal')
			{
				sensus.pages.analytics.highChart.plotOptions.column.stacking = "";
			}
			else
			{
				sensus.pages.analytics.highChart.plotOptions.column.stacking = "normal";
			}
		...
		searchGraphic : function(hash) {
					...
					for( c in data.columns)
					{
						if (data.columns.hasOwnProperty(c))
						{
							...
							else
							{
								...
								if(data.columns[c].description == 'sensus.mlc.alert_subtype.all')
								{
									bVisible = false;
								}
								...
								if(data.columns[c].description == "sensus.mlc.ecomode_consumption.baseline")
								{
									aType.push(parseFloat(data.columns[c].value));
								}
								else
								{
							...
							var sDate = $.sc.dateFormat(data.columns[c].date, sFormatDate, null, false, true);
			...
			if(sensus.pages.analytics.bPreload && $.sc.isValidPreLoad(sensus.pages.analytics.oPreLoadResponseChart, true))
			{
				fnCallBack(sensus.pages.analytics.oPreLoadResponseChart);
			 ...
			 series : {
					...
					events : {
						legendItemClick : function(event) {
							...
							if (current.name == $.sc.locale("sensus.mlc.alert_subtype.all"))
							{
							...
							{
								$.each(series, function(i, e){
									if (e.name == $.sc.locale("sensus.mlc.alert_subtype.all"))
									{
										e.hide();
									}
								});
		...
		getTableData : function(){
				...
				if($.sc.isValidArray(data.groups))
				{
					if(data.groups[0].columns != null && data.groups[0].columns != undefined)
					{
						...
						if(sensus.pages.analytics.analyticsTable)
						{
							...
							sensus.pages.analytics.analyticsTable.data().config.bPreLoad = false;
			...
			if(sensus.pages.analytics.bPreload && $.sc.isValidPreLoad(sensus.pages.analytics.oPreLoadResponseTable, true))
			{

				fnCallBack(sensus.pages.analytics.oPreLoadResponseTable);
