...
sensus.pages.smartpoint.detail.dialogSettings = {
		 ...
		 */
		 baselineEcoMode : {
			 ...
			 * The dialog title.
			 */
			title : $.sc.locale("smartpoint.actions.ecoModeBaseLine.title"),
			...
			buttons : [{
					...
					text : $.sc.locale("smartpoint.actions.ecoModeBaseLine.save"),
					click : function() {
			...
			action : function(actionDialog) {
				...
				var sCurrentType = '';
				var sReplaced = '';
				if(oResponse.light.ecoModeBaseline && oResponse.light.ecoModeBaseline.replacedType){

					sCurrentType = oResponse.light.ecoModeBaseline.replacedType;
					sReplaced = oResponse.light.ecoModeBaseline.replacedWattage;

				}
				sensus.commons.modules.bForceReload = false;
				var sHtml = '<form id="baseline-detail"  name="baseline-detail" method="post" action="#"><fieldset class="edit-only"><ul><li><label for="">'+$.sc.locale('smartpoint.actions.replacedType')+':</label><select id="light-type">';
				var aType = sensus.settings.enums.LightEcoModeTypeEnum;
				var sSelected = '';
				...
				sHtml += '</select></li><li><label for="">'+$.sc.locale('smartpoint.actions.replacedWattage')+':</label><input type="text" id="baseline-wattage" class="validate[required, custom[integer, min[0], max[5000]]] short" value="'+sReplaced+'"></li>';
				sHtml += '<li class="note"><p>'+$.sc.locale('smartpoint.actions.ecoModeBaseLine.note')+'</p></li></ul></fieldset></form>';
				...
				$("#baseline-detail").validationEngine('attach', {
					...
					onValidationComplete   : function(form, status) {
						...
						if (status) {
							...
							aEcoModeBaseline.push(new ecoModeBaseline (sType, nWattage));
							var oLight = new Light(null, parseInt($.address.parameter("id")), $("#pole-id").text().trim());
							oLight.ecoModeBaseline = new ecoModeBaseline(sType, nWattage);
							var oParams = {
								lights : [oLight],
							};

							$.sc.getJson("api/ecomode/upsert", new ecoModeRequest(oParams), false, null, $.sc.locale("action.longRunningProcessDialog.confirm"));
							sensus.commons.modules.bForceReload = true;
							var oResponse = sensus.commons.modules.summaryData.lightInformation.fnGetResponse();
							sensus.commons.modules.bForceReload = false;
							actionDialog.dialog('close');
							sensus.widgets.datatable.reloadTable(sensus.pages.smartpoint.detail.ecoModeTable);
							$('#ecomode_baseline').html($.sc.locale("smartpoint.detail.ecoMode.baseline", nWattage, sType));

							var sEcoModeLabel;
							var nPercent;

							if(oResponse.light.ecoMode){
								var nValue = Math.round(oResponse.light.ecoMode);
								nPercent = nValue + '%';

								sEcoModeLabel = $.sc.locale('smartpoint.detail.ecoMode.Economy');
