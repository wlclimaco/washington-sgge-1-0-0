...
public class ExercicioViewController extends BaseViewController
{
	...
	/** The Constant LOG. */
	private static final Logger LOG = LoggerFactory.getLogger(ExercicioViewController.class);
	...
	public ModelAndView loadList(HttpServletRequest request)
	{
		...
		InquiryExercicioRequest inquiryPaginationRequest = new InquiryExercicioRequest();
		...
		InquiryExercicioResponse inquiryExercicioResponse =
				getExercicioBCF().fetchAllExercicios(inquiryPaginationRequest);

		try
		{
			modelAndView.addObject(RESPONSE, getMapper().writeValueAsString(inquiryExercicioResponse));
		}
		catch (IOException e)
		{
