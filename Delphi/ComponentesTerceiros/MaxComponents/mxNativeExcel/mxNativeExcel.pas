...
// ****************************************************************************
// * Copyright 2001-2008, Bitvadász Kft. All Rights Reserved.
// ****************************************************************************
...
Begin
    ...
    Begin
          FFileName := Value;
          if CompareText( ExtractFileExt( FFileName ), EXCEL_EXTENSION ) <> 0 Then
          Begin
...
Var
    ...
    StringLength: Byte; // Integer
    InternalValue:PAnsiChar;
Begin
    InternalValue:=PAnsiChar(AnsiString(Value));

    If StrLen( InternalValue ) > 255 Then
    Begin
          StrLCopy( Buffer, InternalValue, SizeOf( Buffer ) );
          StringLength := SizeOf( Buffer );
    End
    Else
    Begin
          StrCopy( Buffer, InternalValue );
          StringLength := StrLen( InternalValue );
    End;
...
Var
    Buffer: Array[ 0..1 ] Of Word;
    TempByte: Byte;
    InternalFontName:AnsiString;
Begin
    If ( Value = Nil ) Or ( FFontNumber = 4 ) Then Exit;
    Inc( FFontNumber );

    InternalFontName:=AnsiString(Value.Name);

    FRecordType := EXCEL_FONT;
    FDataLength := 5 + Length( InternalFontName );
    WriteRecordHeader;
    FDataLength := Length( InternalFontName );
    ...
    TempByte := Length( InternalFontName );
    FExcelStream.Write( TempByte, SizeOf( TempByte ) );
{$WARNINGS OFF}
    FExcelStream.Write( InternalFontName[ 1 ], Length( InternalFontName ) );
{$WARNINGS ON}
End;
...
Var
    ...
    StringLength: Byte;//Byte;
    RecalcFlag: Byte;
    StringField: Boolean;
    TempStr: String;
Begin
    // TODO: to check for unicode in/compatibility
    raise Exception.Create('Formula not supported.');
