...
{$endif}
  ...
  public
    ...
    // be reused (it will therefore spare resulting pdf file space)
    // - if ForceCompression property is set, the picture will be stored as a JPEG
    // - the low-level DIB data can be specified in bmi/bits, to avoid recreating
    // it for calculating the binary TPdfImageHash values
    function CreateOrGetImage(B: TBitmap; DrawAt: PPdfBox=nil;
      bmi: PBitmapInfo=nil; bits: pointer=nil): PDFString;
...
{$endif}
  ...
  public
    ...
    procedure ConcatToCTM(a, b, c, d, e, f: Single);                  {  cm  }
    ...
    procedure SetDash(const aarray: array of Byte; phase: integer=0); {  d   }
...
{$endif}
    ...
    function SetFont(const AName: RawUTF8; ASize: Single; AStyle: TFontStyles;
      ACharSet: integer=-1): TPdfFont; overload;
    /// set the current font for the PDF Canvas
...
{$endif}
  ...
  protected
    ...
    // low level adding of a glyph (return the real glyph index found)
    function AddGlyph(aGlyph: word): word;
  public
    ...
    // - return NIL if the font is itself the Unicode version, OR if the
    // Unicode font has not yet been created by the CreateUnicodeFont method
...
uses
  WinSpool,
  SynZip; // Synopse implementation of ZLib, used for the FlateDecode filter
...
begin
  {$ifdef UNICODE}
  result := PDFString(Value); // Delphi 2009/2010/XE will do the conversion as expected
  {$else}
...
begin
  ...
  while Ansi<>0 do begin
    ...
    Assert(TTF<>nil);
    if (Canvas.FPage.Font<>TTF.UnicodeFont) then begin
    ...
    Add('<');
    repeat
      AddHex4(TTF.fUsedWide[TTF.FindOrAddUsedWideChar(PW^)].Glyph);
      inc(PW);
      Ansi := WideCharToWinAnsi(cardinal(PW^));
      if Ansi=160 then
        Ansi := 32;
    until (Ansi>=0) and (Ansi<>32); // we allow space inside Unicode text
    Add('>').Add(SHOWTEXTCMD[NextLine]);
    NextLine := false; // MoveToNextLine only once
  end;
end;
...
begin
  ...
  if StandardFontsReplace then // fonts width are for WinAnsi encoding only
    for i := 0 to high(STANDARDFONTS) do
      if SameTextU(STANDARDFONTS[i].Name,FontName) then begin
        result := TPdfFontType1.Create(FXref,PDFString(FontName),STANDARDFONTS[i].Widths);
        RegisterFont(result);
        Exit;
      end;
  result := nil;
...
function TPdfDocument.CreateOrGetImage(B: TBitmap; DrawAt: PPdfBox;
  bmi: PBitmapInfo; bits: pointer): PDFString;
var J: TJpegImage;
    Img: TPdfImage;
    Hash: TPdfImageHash;
    HeaderSize: dword;
    ImageSize: dword;
    bmiArray, bitsArray: array of byte;
begin
  // if no DIB data specified -> recreate from TBitmap object
  GetDIBSizes(B.Handle,HeaderSize,ImageSize);
  if (bmi=nil) or (bits=nil) or
     (bmi^.bmiHeader.biWidth<>B.Width) or
     (bmi^.bmiHeader.biHeight<>B.Height) then begin
    SetLength(bmiArray,HeaderSize);
    SetLength(bitsArray,ImageSize);
    bmi := pointer(bmiArray);
    bits := pointer(bitsArray);
    GetDIB(B.Handle, B.Palette, bmi^, bits^);
  end;
  // "4 algorithms to rule them all"
  Hash[0] := Hash32(bmi,HeaderSize) xor Hash32(bits,ImageSize);
  Hash[1] := HashOf(pointer(bmi),HeaderSize) xor HashOf(bits,ImageSize);
  Hash[2] := crc32(crc32(0,pointer(bmi),HeaderSize),bits,ImageSize);
  Hash[3] := adler32(adler32(0,pointer(bmi),HeaderSize),bits,ImageSize);
...
begin
  if fsItalic in AStyle then
    if fsBold in AStyle then
      result := AName+'-BoldOblique' else
      result := AName+'-Oblique' else
    if fsBold in AStyle then
      result := AName+'-Bold' else
      result := AName;
end;
...
begin
  ...
  result := fDoc.GetRegisteredNotTrueTypeFont(PDFString(ABaseFont));
  if result=nil then
...
var AFont: TLogFontW;
    FontIndex, i: integer;
begin
  // use embedded fonts for standard fonts
  result := nil;
...
procedure TPdfCanvas.ConcatToCTM(a, b, c, d, e, f: Single);
begin
  if FContents<>nil then
    FContents.Writer.AddWithSpace(a).AddWithSpace(b).AddWithSpace(c).AddWithSpace(d).AddWithSpace(e).AddWithSpace(f).Add('cm'#10);
end;
...
procedure TPdfCanvas.SetDash(const aarray: array of Byte; phase: integer);
var i: integer;
begin
  if FContents=nil then
    exit;
...
begin
  ...
  if PW<>nil then
    if FPage.FFont.FTrueTypeFontsIndex=0 then begin
      Ansi := PDFString(PW); // only work with current codepage/charset
      i := 1;
  ...
  try
    if Unicode then begin
      ...
      if fDoc.PDFA1 or (fDoc.EmbeddedTTF and
        (fDoc.fEmbeddedTTFIgnore.IndexOf(fDoc.FTrueTypeFonts[fTrueTypeFontsIndex-1])<0)) then begin
        old := SelectObject(fDoc.FDC,fHGDI);
...
{  if E.fInLined and
  ...
  EMR_SELECTOBJECT:
    if integer(PEMRSelectObject(R)^.ihObject)<0 then begin // stock object?
      ...
      case Kind of
        0: result := false; // invalid EMF content -> stop enumeration
        OBJ_PEN: begin
          if E.fInLined and
