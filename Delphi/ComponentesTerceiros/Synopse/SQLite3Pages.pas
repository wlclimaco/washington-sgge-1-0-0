...
(*
 ...
 Notes:
  ...
  Enhanced for the freeware Synopse SQLite3 database framework:
 ***************************************************************
...
{$ifdef GDIPLUSDRAW}
  SynGdiPlus,
{$endif}
  Graphics, Controls, Dialogs, Forms,
...
type
  ...
  // - returned AnsiString is NOT an ansi string, but an unicode-encoded string
  // mapped into an AnsiString (i.e. a RawByteString for Delphi 2009/2010/XE) 
  TOnStringToUnicodeEvent = function(const Text: string):
    {$ifdef UNICODE}RawByteString{$else}AnsiString{$endif} of object;
  ...
  public
    ...
    constructor Create(Report: TGDIPages; doubleline: boolean;
      const aText: string=''; IsText: boolean=false);
  end;
...
{$endif}
    ...
    procedure PrintFormattedLine(s: string; flags: integer;
...
{$endif}
    ...
    fInternalUnicodeString: {$ifdef UNICODE}RawByteString{$else}AnsiString{$endif};
    PreviewForm: TForm;
    ...
    procedure InternalUnicodeString(const s: string;
...
{$endif}
    ...
    procedure DrawTitle(const s: string; DrawBottomLine: boolean=false; OutlineLevel: Integer=0;
      const aBookmark: string=''; const aLink: string='');
    /// draw one line of text, with the current alignment
    procedure DrawTextAt(s: string; XPos: integer; const aLink: string='');
...
{$endif}
    ...
    procedure AddColumnHeaders(const headers: array of string;
    ...
    procedure AddColumnHeadersFromCSV(var CSV: PChar;
    ...
    procedure DrawTextAcrossCols(const StringArray: array of string;
      BackgroundColor: TColor=clNone);
    /// draw some text, split across every columns
    // - this method expect the text to be separated by commas
    // - if BackgroundColor is not clNone (i.e. clRed or clNavy or clBlack), the
    // row is printed on white with this background color (e.g. to highlight errors)
    procedure DrawTextAcrossColsFromCSV(var CSV: PChar; BackgroundColor: TColor=clNone);
...
resourcestring
  ...
  sReportPopupMenu1 = 'Next page,Previous page,Go to Page...,Zoom...,'+
    'Bookmarks,Copy Page as Text,Print,PDF Export,Page fit,Page width';
  /// used to create the pages browsing menu of the report
  sReportPopupMenu2 = 'Pages %d to %d,Page %d';

const
  ReportPopupMenuNextPage = 1;
  ReportPopupMenuPreviousPage = 2;
  ReportPopupMenuGoToPage = 3;
  ReportPopupMenuZoom = 4;
  ReportPopupMenuBookmarks = 5;
  ReportPopupMenuPageAsText = 6;
  ReportPopupMenuPrint = 7;
  ReportPopupMenuPDFExport = 8;  
...
begin
  result := S;
  i := Length(s);
  while (i > 0) and (ord(result[i])<=32) do dec(i);
  SetLength(result,i);
end;
...
function GetTextExtentExPointNoPartials(DC: HDC; p2: PChar; p3, p4: Integer;
  var p5: Integer; const p6: integer; var p7: TSize): BOOL; stdcall;
    external gdi32 name
    {$ifdef UNICODE}'GetTextExtentExPointW';{$else}'GetTextExtentExPointA';{$endif}
...
var
  ...
  begin
    result := pchar(pointer(s)); // result := next #0 ended string in s
...
procedure TGDIPages.PrintFormattedLine(s: string; flags: integer;
  const aBookmark: string; const aLink: string);
var i: integer;
...
procedure TGDIPages.LeftOrJustifiedWrap(const s: string);
...
procedure TGDIPages.RightOrCenterWrap(const s: string);
  ...
  repeat
    ...
    case falign of
      ...
      taCenter: Offset := leftOffset +
        (rightOffset-leftOffset-fCanvas.TextWidth(leftstring)) shr 1;
...
procedure TGDIPages.HandleTabsAndPrint(const leftstring: string;
  var rightstring: string; leftOffset, rightOffset: integer);
const
...
begin
  ...
  while tabPos > 0 do begin // and still room to print
    ...
    GetTextExtentPoint32(fCanvas.Handle,pointer(ls),tabPos,size);
  ...
  if (falign = taLeft) or (rightstring = '') then begin // left aligned
    ...
    Windows.ExtTextOutW(fCanvas.Handle, leftOffset, fCurrentYPos,
      fCanvas.TextFlags, nil, PW, PWlen, nil);
...
{$ifdef GDIPLUSDRAW} // anti aliased drawing:
      if not ForceNoAntiAliased then
        DrawEmfGdip(PreviewSurfaceBitmap.Canvas.Handle,
          TMetaFile(Pages.Objects[Page-1]),R,ForceInternalAntiAliased) else
  ...
  if s = '' then
    ...
    while P^ <> #0 do begin
      ...
      if not fInHeaderOrFooter then
        fCanvasText := fCanvasText+tmpStr+#13#10;
...
begin
  DrawText(UTF8ToString(s));
end;

procedure TGDIPages.DrawTitle(const s: string; DrawBottomLine: boolean=false;
...
begin
  ...
  try
    if not fInHeaderOrFooter then
      fCanvasText := fCanvasText+s+#13#10; // copy as text
    PrintFormattedLine(s,TitleFlags,aBookMark,aLink);
    if UseOutlines then
      AddOutline(s,OutlineLevel,fCurrentTextTop,fCurrentTextPage);
...
procedure TGDIPages.DrawTextAt(s: string; XPos: integer; const aLink: string='');
var i: integer;
    R: TRect;
    Size: TSize;
begin
  ...
  R.Top := fCurrentYPos;
  fCanvas.TextOut(R.Left,R.Top,s);
  if not fInHeaderOrFooter then
...
procedure TGDIPages.DrawAngledTextAt(const s: string; XPos, Angle: integer);
var
  lf: TLogFont;
...
var R: TRect;
    H: Integer;
begin
  if (Self=nil) or (bmp=nil) then exit; // avoid GPF
  ...
  R.Bottom := (bmp.Height*bWidth) div bmp.Width;
  if Legend<>'' then
    H := LineHeight else
    H := 0;
...
function GetNextItem(var P: PChar): string; // work with generic string type
var S: PChar;
begin
  if P=nil then
    result := '' else begin
...
begin
  ...
  if PopupMenu=nil then // caller may have created a TAdvPopupMenu instance
    PopupMenu := PopupMenuClass.Create(Self) else
    PopupMenu.Items.Clear;
  PopupMenu.OnPopup := PopupMenuPopup;
  PC := pointer(string(sReportPopupMenu1));
  // 'Next,Previous,GotoPage,Zoom,Bookmarks,CopyasText,Print,PDFExport,Pagefit,Pagewidth'
  for i := 1 to ReportPopupMenuPDFExport do 
    NewPopupMenuItem(GetNextItem(PC),-i).Enabled :=
      (i<ReportPopupMenuPrint) or
      ( (i=ReportPopupMenuPrint) and fHasPrinterInstalled) or
      ( (i=ReportPopupMenuPDFExport)
        {$ifdef USEPDFPRINTER}and fHasPDFPrinterInstalled{$endif});
  M := PopupMenu.Items[ReportPopupMenuZoom-1];
  NewPopupMenuItem(GetNextItem(PC),-1000-PAGE_FIT,M);
  NewPopupMenuItem(GetNextItem(PC),-1000-PAGE_WIDTH,M);
  for i := 0 to high(MenuZoom) do
    NewPopupMenuItem(format('%d %%',[MenuZoom[i]]),-1000-MenuZoom[i],M);
  Root := PopupMenu.Items[ReportPopupMenuBookmarks-1];
...
procedure TGDIPages.AddTextToHeader(const s: string);
begin
  if Self=nil then exit; // avoid GPF
  fHeaderLines.Add(THeaderFooter.Create(Self,false,s,true));
end;

procedure TGDIPages.AddTextToHeaderAt(const s: string; XPos: integer);
...
procedure TGDIPages.AddTextToFooter(const s: string);
begin
  if Self=nil then exit; // avoid GPF
  if fFooterLines.Count = 0 then
    CalcFooterGap;
...
procedure TGDIPages.AddTextToFooterAt(const s: string; XPos: integer);
...
procedure TGDIPages.AddColumnHeaders(const headers: array of string;
  WithBottomGrayLine: boolean=false; BoldFont: boolean=false;
  RowLineHeight: integer=0; flags: integer=0);
var i: integer;
    zStr: TZStrings;
...
function CSVToArray(var CSV: PChar; n: integer): TStringDynArray;
var i: integer;
begin
  SetLength(result,n);
  for i := 0 to n-1 do
    result[i] := GetNextItem(CSV);
end;

procedure TGDIPages.AddColumnHeadersFromCSV(var CSV: PChar;
...
procedure TGDIPages.DrawTextAcrossColsFromCSV(var CSV: PChar; BackgroundColor: TColor=clNone);
begin
  if Self<>nil then // avoid GPF
...
procedure TGDIPages.DrawTextAcrossCols(const StringArray: array of string;
  BackgroundColor: TColor = clNone);
...
function WrapText(s: string; MaxWidth: integer; Lines: TStringList): integer;
var j,k,sp: integer;
begin
  result := 0; // returns the line count
  repeat
    if HasCRLF(s) or (fCanvas.TextWidth(s)>MaxWidth) then begin
      j := 1;
      k := 1;
      sp := 0;
      while (j<length(s)) and (fCanvas.TextWidth(copy(s,1,j))<MaxWidth) do begin
        ...
        if SysLocale.FarEast then
          j := NextCharIndex(s,j) else // MBCS ready
        {$endif}
        inc(j);
      end;
      if sp=0 then
...
begin
  ...
  if WordWrapLeftCols then begin // check if stay on current page after word wrap
    ...
    if (ColAlign=caLeft) and (ColRight>ColLeft) and
       (HasCRLF(StringArray[j]) or
        (fCanvas.TextWidth(StringArray[j])>ColRight-ColLeft)) then begin
  ...
  for i := 0 to max do begin
    s := StringArray[i];
    line := line+s+#9; // add column content + tab for report text
    if s<>'' then
    ...
    if ColRight>ColLeft then begin
          ...
          if WordWrapLeftCols and (ColRight>ColLeft) and
            ...
            try
              ...
              for j := 0 to Lines.Count-1 do begin
                ...
                Windows.ExtTextOutW(Handle,X,RowRect.Top,Options,@RowRect,PW,PWLen,nil);
              end;
              RowRect.Top := fCurrentYPos;
            finally
              Lines.Free;
      ...
      dec(RowRect.Left,ParenthW);
      Windows.ExtTextOutW(Handle,RowRect.Left+ParenthW,fCurrentYPos,Options,@RowRect,PW,PWLen,nil);
...
function TGDIPages.TextWidth(const Text: string): integer;
begin
  if Self=nil then
    result := 0 else begin
    if fCanvas=nil then
      result := Canvas.TextWidth(Text) else
  ...
  try
    ...
    with PaperSize do begin
      if cy=0 then
        y := 1 else
        y := cy;
      PreviewForm.Width := (cx*PreviewForm.Height) div y+64;
    end;
    OldParent := Parent;
    Parent := PreviewForm;
    Align := alClient;
...
begin
  ...
  if (M.Tag=0) or (M.Tag>PageCount) then
     OnPopupMenuClick(Sender); // only notify custom events
  case -M.Tag of
    0: exit;
    ReportPopupMenuNextPage:
      Page := Page+1;
    ReportPopupMenuPreviousPage:
      Page := Page-1;
    ReportPopupMenuPageAsText:
      if Page>0 then
        Clipboard.AsText := Pages.Strings[Page-1];
    ReportPopupMenuPrint:
      if PrintPages(-1,-1) then
       if PreviewForm<>nil then
         PreviewForm.Close;
    ReportPopupMenuPDFExport:
      ExportPDF('',true);
    ...
    2000..4000: begin // allow -2000-OutlineIndex
      ...
      if Cardinal(M.Tag)<=Cardinal(PageCount) then
        Page := M.Tag;
  end;
end;
...
begin
  with PopupMenu.Items do
    while Count>ReportPopupMenuPDFExport do
      Delete(ReportPopupMenuPDFExport); // delete after "PDF Export" entry
  PopupMenu.Items[ReportPopupMenuNextPage-1].Enabled := Page<PageCount;
  PopupMenu.Items[ReportPopupMenuPreviousPage-1].Enabled := Page>1; 
  M := PopupMenu.Items[ReportPopupMenuGoToPage-1];
  while M.Count>0 do
    M.Delete(0);
  ...
  if PageCount>=1 then begin // add 'Go to Page' sub menus (group by 10 pages) 
    ...
    PageFromTo := GetNextItem(P); // Pages %d to %d  
    PageN := GetNextItem(P);      // Page %d
