...
sensus.pages.smartpoint.applySmartPointToSchedule = function(url, urlInclude, title) {
	...
	var apply = {
		...
		buttons : [{
				...
				text : sensus.locale.get("action.addtoschedule.submit"),
				click : function() {
					...
					if (sScheduleName.length == 0 || sScheduleName == sensus.locale.get("widgets.combobox.prompt")) {
						$("#add-schedule-form #add-schedule-select_input").validationEngine('showPrompt', sensus.locale.get("validation.addtoschedule.noschedule"), 'red', '', true);
						return false;
					}
					
					if (url == "addToEventScheduleUrl") {
						sUrl = sensus.settings.addToEventScheduleUrl;
						sScheduleType = 'EVENT'; 

					} else if (url == "addToOffsetScheduleUrl") {
						sUrl = sensus.settings.addToOffsetScheduleUrl;
					...
					var oRequest = { 'scheduleRequest': new scheduleRequest(nId, null, null, null, null, null, {}, sScheduleType, sensus.widgets.datatable.isAllRows, aIds, oSearchLight, sScheduleName) } ;

					$.ajaxValidator.fnMonitor(sUrl, oRequest, false, sensus.pages.smartpoint.fnClearTableSelects, sensus.locale.get("action.longRunningProcessDialog.confirm"), false);
		...
		action : function(actionDialog) {
			...
			$('#action-dialog').empty().load(sUrlIncludePage, function() {
				...
				$("#add-schedule-select_input").attr("value", sensus.locale.get("widgets.combobox.prompt.typetosearch"));
...
sensus.pages.smartpoint.resetSchedule = function(sParamURL) {
	 ...
	 var reset = {
		 ...
		 * The dialog title.
		 */
		title : sensus.locale.get("action.removefromschedule.title"),
		...
		buttons : [{
				...
				text   : sensus.locale.get("action.removefromschedule.submit"),
				click  : function() {
					...
					if (sParamURL == "removeFromEventScheduleUrl") {
						...
						sUrl = sensus.settings.removeFromEventScheduleUrl;
						sScheduleType = 'EVENT';

					} else if (sParamURL == "removeFromOffsetScheduleUrl") {
						
						sUrl = sensus.settings.removeFromOffsetScheduleUrl;
						sScheduleType = 'OFFSET';
					...
					var oRequest = { 'scheduleRequest': new scheduleRequest(null, null, null, null, null, null, {}, sScheduleType, sensus.widgets.datatable.isAllRows, aIds, oSearchLight, null) } ;
				...
				text   :sensus.locale.get("action.removefromschedule.cancel"), 
				click  : function() {
		...
		action : function(actionDialog) {
			...
			$('#action-dialog').empty().load(sensus.settings.removeFromEventScheduleInclude, function() {
...
sensus.pages.smartpoint.dialogSettings = {
	...
	addToGroup : {
		...
		buttons : [{
				...
				text : sensus.locale.get("action.addtogroup.submit"),
				click : function() {
					...
					if (sGroupName == sensus.locale.get("action.addgroup.label.addnewgroup")) {
						...
						$('#group-add-select').find('option').each(function() {
							...
							if ($(this).text() == $('#control-group-name-create').val()) {
								...
								$("#add-group-form #control-group-name-create").validationEngine('showPrompt', sensus.locale.get("validation.addtogroup.uniquegroupname"), 'red', '', true);
						...
						for (var i = 0; i < sNewGroupName.length ; i++) {
							...
							if (sChars.indexOf(sNewGroupName.charAt(i)) != -1) {
								...
								$("#add-group-form #control-group-name-create").validationEngine('showPrompt', sensus.locale.get("validation.specialCharsCustom"), 'red', '', true);
								bValidGroupName = false;
						...
						if (sNewGroupName.length == 0 || sNewGroupName.length > 100) {
							...
							if (sNewGroupName.length == 0) {
								...
								$("#add-group-form #control-group-name-create").validationEngine('showPrompt', sensus.locale.get("validation.addtogroup.nonewgroupname"), 'red', '', true);
								
							} else {
							
								$("#add-group-form #control-group-name-create").validationEngine('showPrompt', sensus.locale.get("groupcreate.form.name.maxlength"), 'red', '', true);
							...
							if (sGroupDescription.length <= 150) {
								...
								sensus.util.ajaxaction.sendActionDefault(sensus.locale.get("action.addsgroup.success", sNewGroupName), sensus.locale.get("action.addgroup.error"));
								...
								$("#control-group-description").validationEngine('showPrompt', sensus.locale.get("groupcreate.form.description.maxlength"), 'red', '', true);
							
							}
							
						}						
						
					} else if (sGroupName != sensus.locale.get("widgets.combobox.prompt") && sGroupName.length != 0 && sensus.util.combobox.getOption("#group-add-select") != undefined) {
						...
						if (sensus.util.combobox.getOption("#group-add-select") == undefined) {
							...
							$("#add-group-form .ui-autocomplete-input").validationEngine('showPrompt', sensus.locale.get("smartpoint.actions.addToGroup.not.exists", $('#group-add-select_input').val()), 'red', '', true);
						
						} else {
						
							$("#add-group-form .ui-autocomplete-input").validationEngine('showPrompt', sensus.locale.get("validation.addtogroup.nonewgroupname"), 'red', '', true);
					...
					if (sGroup) {
						...
						sensus.util.ajaxaction.sendActionDefault(sensus.locale.get("smartpoint.actions.addgroup.success", sNewGroupName), sensus.locale.get("action.addgroup.error"));
				...
				text :sensus.locale.get("action.addtogroup.cancel"), 
				click : function() {
		...
		action : function(actionDialog) {
			...
			$('#action-dialog').empty().load(sensus.settings.addToGroupInclude, function() {
				...
				$('.ui-corner-all', actionDialog).live('click', function() {
					...
					if ($('#group-add-select_input').val() != sensus.locale.get('widgets.combobox.prompt') 
							&& $('#group_add_select_input').val() != sensus.locale.get('action.addgroup.label.addnewgroup')) {
				...
				$('#action-dialog a').click(function() {
					...
					if ($('#add-group-form .add-group-control').attr('class') == 'add-group-control s') {
						...
						$('#group-add-select_input').val(sensus.locale.get('action.addgroup.label.addnewgroup'));
						$('#group-add-select option[value="0"]').text(sensus.locale.get('action.addgroup.label.addnewgroup'));
				...
				$("#group-add-select_input").attr("value", sensus.locale.get("widgets.combobox.prompt2"));
				$('.smartpoint-count').html($('.checked-count').text().split(' ')[0]);
			});
	...
	removeFromGroup : {
		...
		buttons : [{
				...
				text : sensus.locale.get("action.removefromgroup.submit"),
				click : function() {
					...
					var sStringFiltersURL = sensus.pages.smartpoint.getStringFiltersURL();
					var sNoGroup = sGroupName.length == 0 || sGroupName == sensus.locale.get("widgets.combobox.prompt");
					if (!sNoGroup && (sensus.widgets.datatable.isAllRows || sIds.length > 0)) {
						...
						sensus.util.ajaxaction.sendActionDefault(sensus.locale.get("smartpoint.actions.removegroup.success", GroupName), sensus.locale.get("action.addgroup.error"));
						...
						if (sIds.length == 0) {
							...
							$("#add-group-form .ui-autocomplete-input").validationEngine('showPrompt', sensus.locale.get("validation.removefromgroup.nogroup"), 'red', '', true);
				...
				text :sensus.locale.get("action.removefromgroup.cancel"), 
				click : function() {
	...
	addTag : {
		...
		buttons              : [{
				id     : "add-tag-submit", 
				text   : sensus.locale.get("action.addtag.submit"),
				click  : function() {

					var aIdTags  = [];

					if ( ($('.search-field input').val() == sensus.locale.get('widgets.combobox.prompt.selecttags') 
							|| $('.search-field input').val().length == 0) && $(".chzn-choices li.search-choice").length == 0) {

						$('.chzn-choices').validationEngine('showPrompt', sensus.locale.get("validation.addtag.notag"), 'red', '', true);
						return;
					...
					$(".chzn-choices li.search-choice, .search-field input").each(function() {
						...
						if (!oOption && sLiText 
							...
							sensus.util.ajaxaction.sendActionDefault(sensus.locale.get("action.inserttag.success", [sLiText, sensus.settings.smartpointUrl]));
				...
				id: "add-tag-cancel", 
				text :sensus.locale.get("action.addtag.cancel"), 
				click : function() {
					$(this).dialog('close');
				}
		}],
	...
	removeTag : {
		...
		buttons : [{
				...
				text : sensus.locale.get("action.removetag.submit"),
				click : function() {
					...
					if (aIdTags.length == 0) {
						$('.chzn-choices').validationEngine('showPrompt', sensus.locale.get("validation.removetag.notag"), 'red', '', true);
						return;
					} 
				...
				text :sensus.locale.get("action.removetag.cancel"), 
				click : function() {
	 ...
	 */
	applyEventSchedule : sensus.pages.smartpoint.applySmartPointToSchedule("addToEventScheduleUrl", 'addToScheduleEventInclude', sensus.locale.get("action.applyeventschedule.title")),
	
	/**
	 * Configuration for the "applyOffsetSchedule" dialog.
	 */
	applyOffsetSchedule : sensus.pages.smartpoint.applySmartPointToSchedule("addToOffsetScheduleUrl", 'addToScheduleOffsetInclude', sensus.locale.get("action.applyoffsetschedule.title")),
	...
	saveSearch : {
		 ...
		 * The dialog title.
		 */
		title : sensus.locale.get("action.savesearch.title"),
		...
		buttons : [{
				...
				text : sensus.locale.get("action.savesearch.submit"),
				click : function() {
					...
					if (!oSearchName.val().replace(/\s/,"").length) {
						...
						$("#save-search-form #saved-search-name").validationEngine('showPrompt', sensus.locale.get("validation.savesearch.noname"), 'red', '', true);
						sensus.widgets.datatable.clearSelectsFunction.call(sensus.pages.smartpoint.smartpointTable);
						return false;
					...
					if (sSearchDescription.length > 200) {
						...
						$("#save-search-form #saved-search-description").validationEngine('showPrompt', sensus.locale.get("validation.savesearchdescription.maxlength"), 'red', '', true);
						return false;
					...
					$.ajax({
						...
						success     : function(oResponse) {
							...
							if (oResponse && oResponse.operationSuccess) { 
								...
								sensus.util.page.showMessage("messaging-main", sensus.locale.get("action.savesearch.success", sSearchName), "confirm");
								return true;
							...
							if (oResponse.messageList[0].text) {
								...
								sensus.util.page.showMessage("messaging-main", sensus.locale.get("action.savesearch.error"), "error");
		 ...
		 */
		action : function(actionDialog) {
			$('#action-dialog').empty().load(sensus.settings.saveSearchInclude, function() {
				actionDialog.removeClass('waiting');
