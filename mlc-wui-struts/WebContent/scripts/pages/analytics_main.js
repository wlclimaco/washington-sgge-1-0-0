...
sensus.pages.analytics = {
		...
		monthNames           : [   // Array with name of months
              	sensus.locale.get('analytics.month.short.jan')
              , sensus.locale.get('analytics.month.short.feb')
              , sensus.locale.get('analytics.month.short.mar')
              , sensus.locale.get('analytics.month.short.apr')
              , sensus.locale.get('analytics.month.short.may')
              , sensus.locale.get('analytics.month.short.jun')
              , sensus.locale.get('analytics.month.short.jul')
              , sensus.locale.get('analytics.month.short.aug')
              , sensus.locale.get('analytics.month.short.sep')
              , sensus.locale.get('analytics.month.short.oct')
              , sensus.locale.get('analytics.month.short.nov')
              , sensus.locale.get('analytics.month.short.dec')
        ],
		...
		getGroupId : function() {
			...
			if (nId == undefined || sName == sensus.locale.get('analytics.page.allLights')) {
		...
		setGroup : function() {
			...
			if (hashObj.db) {
				...
				$("#group_select_input").val(sensus.locale.get("analytics.page.allLights"));
		...
		searchGraphic : function(hash) {
			...
			$.ajax({
				url          : sensus.settings.searchGraphic,
				dataType     : 'json',
				type         : "POST",
				data         : $.toJSON({'analyticsRequest' : new analyticsRequest()}),
				async        : false,
				...
				success      : function(data) {
					...
					if(data.columns){
						...
						for( c in data.columns) {
							...
							if (data.columns.hasOwnProperty(c)) {
								...
								if($.grep(aSeries, function(e,i){ aPosition = i; return e.name == sensus.locale.get(data.columns[c].description); }).length){
									...
									if($.address.parameter('ty') == 5){
										...
										if(data.columns[c].description == "sensus.mlc.ecomode_consumption.baseline"){
											...
											aSeries[oControl[sensus.locale.get(data.columns[c].description)]].data.push(parseFloat(data.columns[c].value) / 1000);
										
										} else {
																					
											aSeries[oControl[sensus.locale.get(data.columns[c].description)]].data.push(data.columns[c].value);
									...
									if(data.columns[c].description == "sensus.mlc.ecomode_consumption.baseline"){
										...
										var nIndex = aSeries.push({
											'name'    : sensus.locale.get(data.columns[c].description),
											'data'    : [],
											'type'    : "column",
											'visible' : bVisible
										});
									
										oControl[sensus.locale.get(data.columns[c].description)] = parseInt(nIndex -1 );
								...
								var sDate = $.date.dateFormat(data.columns[c].date, sFormatDate, null, true);
								
								if ($.address.parameter('ty') == 5 || $.address.parameter('ty') == 6){
								
									sDate = $.date.dateFormat(data.columns[c].date, sFormatDate, null, false);
					...
					if(aType.length){
						...
						var oType = { type: 'spline', color: '#cccccc', name: sensus.locale.get("analytics.table.analytics_group_baseline_consumption"), data: aType	};
		...
		datePickerFormat : function() {
			...
			return sensus.settings.dateFormatMask.toLowerCase();
			 ...
			 series : {
					...
					events : {
						legendItemClick : function(event) {
							...
							if (current.name == sensus.locale.get("sensus.mlc.status_subtype.all")) {
								...
								$.each(series, function(i, e){
									...
									if (e.name == sensus.locale.get("sensus.mlc.status_subtype.all")){
		...
		isValidDate : function(value, n, mask) {
			...
			$("#analytics-filter-date #datepicker" + n).validationEngine('showPrompt', sensus.locale.get("analytics.page.date.error"), 'red', '', true);
			return false;
		...
		isTodayDate : function(value, n) {
			...
			var iAnalyticsType  = sensus.pages.analytics.getAnalyticsType(),
				dateFormat      = sensus.settings.dateFormatMask.substring(0, 8),
				dt              = new Date(),
			...
			if ( (iAnalyticsType == 5 || iAnalyticsType == 6)
					...
					$("#analytics-filter-date #datepicker" + n).validationEngine('showPrompt', sensus.locale.get("analytics.page.date.yesterdayDate"), 'red', '', true);
					return false;
			}
		...
		isGreaterDate : function(value, n, mask) {
			...
			$("#analytics-filter-date #datepicker" + n).validationEngine('showPrompt', sensus.locale.get("analytics.page.date.error.greater"), 'red', '', true);
			return false;
		...
		getTableData : function(){
			...
			$.ajax({
				url			: sensus.pages.analytics.getAjaxSource(''),
				dataType    : 'json',
				type        : "POST",
				...
				success : function(data) {
					...
					if($.ajaxValidator.fnValidArray(data.groups)){
