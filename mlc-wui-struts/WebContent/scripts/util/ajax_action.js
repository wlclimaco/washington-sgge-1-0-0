		...
		$.ajax( {
			...
			success  : function (e) {
				...
				if (e.result === sensus.constants.ajax.ok) {
					...
					sensus.util.ajaxaction.sendActionDefault(sensus.locale.get("smartpoint.actions.addgroup.success", groupName), sensus.locale.get("action.longRunningProcessDialog.error"));
					sensus.util.ajaxaction.actionResult = true;
		...
		$.ajax({
			...
			success      : function(response) {
				...
				if (response.operationSuccess || response.result == "success" || response.result == sensus.constants.ajax.ok) {
					...
					sensus.util.page.showMessage("messaging-main", sensus.locale.get("action.longRunningProcessDialog.confirm"), "confirm");
					...
					sensus.util.page.showMessage("messaging-main", sensus.locale.get("action.longRunningProcessDialog.error") + sensus.util.page.getMessageList(""), "error");
		...
		$.ajax({
			...
			success      : function(oData) {
				...
				if (oData && oData.operationSuccess && oData.currentAlarmWarningMessages[0]) {
					...
					$("#light-status").text( sensus.locale.get('smartpointdetail.status.' +  textLightStatus ) );
