sensus.widgets.datatable = {
	...
	reloadTable : function(table, iStart) {
		...
		table.fnSettings()._iDisplayLength = iLength || parseInt(sensus.settings.pageSize, 10);
	...
	savePageSize: function() {
		...
		sensus.util.page.fnSavePropertyProfile( { "PAGE_SIZE" : $(".dataTables_length").find("option:selected").val() } );
	...
	setTable : function(config) {
		...
		} else if (sensus.settings.pageSize != undefined) {
	...
	fnCreateRequestModel : function(sSource, oTable, config, aoData, aoCustomColumns, fnCallback) {
		...
		var oRequest      = new config.oSettings.oRequest(config.oSettings.fnRequest(oTable)),
			sRequestName  = (config.oSettings.oRequest.valueOf().toString().match(/function (.{1,})\(/)[1]).trim(),
		...
		$.ajax({
			...
			success     : function(json) {
				...
				if (oTable.dateRefresh != undefined) {
					...
					var sDate = $.date.dateFormat(oTable.dateRefresh, sensus.settings.dateFormatMask + ' - h:i:s A');

					$(".stamp").text(sensus.locale.get('table.refreshed', [sDate + " " + " " + sensus.settings.timezoneShort ]))
							   .wrapInner("<em/>");
				}
	...
	addDefaultHeader : function(sName, bVisible, sClass, sColumnId, sId) {
		...
		if (!sName || sName == "" || sName == "null") {
			...
			sName = sensus.locale.get("smartpoint.table.header." + sColumnId.replace(/\_/g, ""));
		...
		if (iHeadRows > 1) {
			...
			if (datatable.aColumns.columns.length != oTable.find("th").length - (iSecondRowLength - 1)) {
				...
				if (datatable.oSettings.iMapCol) {
					$("<th rowspan=" + iSecondRowLength +">" + sensus.locale.get("dashboard.page.mapIt") + "</th>").insertBefore(oTable.find("thead tr")
			...
			//Add map
			if (datatable.oSettings.iMapCol) {
				$("<th>" + sensus.locale.get("dashboard.page.mapIt") + "</th>").insertBefore(oTable.find("thead tr")
																			  .find("th:eq("+ datatable.oSettings.iMapCol +")"));
			}
		}
	},
	...
	rowCallback : function(nRow, aData, iDisplayIndex) {
		...
		//Abort
		abort = $('<td><a class="delete abort remove_dialog" href="#">'+ sensus.locale.get("commons.pages.abort")+'</a></td>').click(function(e) {
			e.preventDefault();
		...
		//Summary
		summary = $('<td><a class="button small" href="#">'+ sensus.locale.get("table.summary")+'</a></td>').click(function(e) {
			e.preventDefault();
		...
		if (datatable.oSettings.process && datatable.oSettings.process.oAction) {
			...
			if (datatable.oSettings.process.oAction.edit) {
				...
				if (url.length > 1) {
					oActionEdit  = $('<li><a id="menu-bt-edit" href="'
											...
											+ sensus.locale.get("commons.pages.edit")
					...
					oActionEdit  = $('<li><a id="menu-bt-edit" href="'
							...
							+ '"><span class="icon-small icn-edit"></span> '+ sensus.locale.get("commons.pages.edit") +'</a></li>').click(function(e) {e.preventDefault()
								sensus.util.session.removeSessionCustomize("util/removeSessionCustomize.action", "sessionCustomizeType=SelectedFiltersSchedule");
								sensus.util.session.createSession("util/createSessionCustomize.action", "sessionCustomizeType=SelectedFiltersSchedule&sessionCustomize=" + $.address.queryString().split("&").join("&sessionCustomize="));
							});
				}
			...
			if (datatable.oSettings.process.oAction.clone) {
				...
				oActionClone = $('<li><a id="menu-bt-clone" href="'
						...
						+ sensus.locale.get("commons.pages.clone")
			...
			// button [PAUSE]
			if (datatable.oSettings.process.oAction.pause && aData[datatable.aColumns.colIndexAll.status] == sensus.locale.get("table.type.scheduled")) {
			...
			// button [RESUME]
			if (datatable.oSettings.process.oAction.resume && aData[datatable.aColumns.colIndexAll.status] == sensus.locale.get("table.type.paused")) {
			...
			if (datatable.oSettings.process.oAction.deleteBt) {
				...
				oActionDelete = $('<li><a id="menu-bt-delete" title="'+ datatable.oSettings.process.oAction.deleteBt.text(aData, iDisplayIndex) +'" class="delete-dialog last" href=""><span class="icon-small icn-delete"></span> '+ sensus.locale.get("commons.pages.delete") +'</a></li>').click(function(e) {
					e.preventDefault();
		...
		if (datatable.oSettings.iMapCol) {
			...
			$("<td class='map-col'><a id='map_" + iDisplayIndex + "' href='#'>"+ sensus.locale.get("dashboard.page.map") +"</a></td>").insertBefore($('td:eq(' + mapCol + ')', nRow)).click(
					function(evt, ui) {
						sensus.util.page.startProgressBar();
						evt.preventDefault();
		...
		if (datatable.oSettings.remove) {
			...
			if (aData[datatable.aColumns.colIndexAll.status] == false) {
			...
			} else if (aData[datatable.aColumns.colIndexAll.status] == sensus.locale.get("table.type.scheduled")) {
				...
				$('td:eq(' + datatable.aColumns.colIndexAll.status + ')', nRow).html("<span>"+ sensus.locale.get("process.page."+aData[datatable.aColumns.colIndexAll.status]) +"</span>");
	...
	fnFooterCallback : function(nRow, aaData, iStart, iEnd, aiDisplay, iRecordsDisplay) {
		...
		$(".dataTables_length").find("select").change(function() {
			...
			if (sensus.settings.pageSizeShowDialog == "3") {
		...
		if (datatable.oSettings.order) {
			...
			$.each($tr, function(i, e) {
				...
				if (datatable.oSettings.orderType === 'date') {
					orderComparable = $.date.dateFormat(order) || order;
				} else {
	...
	abortDialog : function(datatable) {
		this.oTableSettings = datatable;
		sensus.util.actiondialog.launchActionDialog("abort", this.dialogSettings["abort"]);
	},
	...
	pageSizeDialog: function(datatable) {
		this.oTableSettings = datatable;
		sensus.util.actiondialog.launchActionDialog("pageSize", this.dialogSettings["pageSize"]);
	}
};
...
sensus.widgets.datatable.dialogSettings = {
		...
		remove : {
			...
			buttons              : (function() {
				...
				buttons[sensus.locale.get("commons.pages.delete")] = function() {
					...
					oNewRequest[sRequestName] = settings.fnRequest(sensus.widgets.datatable.currentRow,  sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll);
					...
					$.ajax({
						...
						success      : function(data) {
							...
							if (data.messageList && data.messageList.length) {
								...
								sensus.util.page.showMessage("messaging-main", data.messageList[0].text , "error"); // sensus.util.page.getMessageList(data.messages)
								return;
	...
	abort : {
		 ...
		 * The dialog title.
		 */
		title : sensus.locale.get("action.abortprocess.title"),
	...
	cancel : {
		...
		buttons : (function() {
			var buttons = {};
			buttons[sensus.locale.get("action.cancel.submit")] = function() {
				...
				$.ajax({
					...
					success  : function(data) {
						...
						if (data.result == sensus.constants.ajax.ok) {
							...
							sensus.util.page.showMessage("messaging-main", success, "error");
	...
	summary : {
		...
		buttons : (function() {
			var buttons = {};
			buttons[sensus.locale.get("action.deleteschedule.submit")] = function() {
				
				
				console.log($('form:last').attr('id'));
				var oFormSubmit = $('form:last').attr('id');
//				
//				sensus.pages.scheduleEvent.sRepeatStartDate = $("#repeat_start_date").val();
//				
//				oFormSubmit.validate(sensus.pages.scheduleEvent.validateRepeat);
//				
				//$("#"+oFormSubmit).submit();
				sensus.util.page.initCreateDate($("#"+oFormSubmit).serialize(),$('form:last').attr('action'));
			
			};
			buttons[sensus.locale.get("commons.pages.cancel")] = function() {
				$("#action-dialog").dialog('close');
				$("#add-Repeat-Definition-Event").attr('checked', false);	
	...
	mapIt : {
		...
		action : function(actionDialog) {
			...
			if (sensus.widgets.datatable.oTableSettings.id == "group-table") {
				dialogTitle = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.groupName] + ' - ' + sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.groupSmartPointCount] + ' '+sensus.locale.get('commons.pages.smartPoints');
			}
			...
			if (sensus.widgets.datatable.oTableSettings.id == "group-table") {
				...
				dialogTitle = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.scheduleName] + ' - ' + sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.scheduleSmartPointCount] + ' '+sensus.locale.get('commons.pages.smartPoints');
				...
				sensus.widgets.datatable.objLatLon.push({
					...
					dialogTitle = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.id] + ' - ' + sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.userSmartPointCount] + ' '+sensus.locale.get('commons.pages.smartPoints');					
			...
			sensus.util.mapopen.mapIt('detail-map-container', sensus.widgets.datatable.objLatLon, iGroupId, sName, iScheduleId, iUserId, aGroupIds);			
	...
	pageSize : {
		 ...
		 * The dialog title.
		 */
		title : sensus.locale.get("action.pagesize.title"),
		/**
		 * Whether this dialog requires a smartpoint list.
		 */
		...
		buttons : [
           {
				id     : "page-size-submit",
				text   : sensus.locale.get("action.pagesize.submit"),
				click  : function() {

					var nPageSize = $(".dataTables_length").find("option:selected").val();
					if (($("#page-size-checkbox").attr("checked") == "checked") || (sensus.settings.pageSizeShowDialog == "1")) {
			...
			{
				id     : "page-size-cancel",
				text   : sensus.locale.get("action.pagesize.cancel"),
				click  : function() {
					...
					if ($("#page-size-checkbox").attr("checked") == "checked") {
						...
						sensus.settings.pageSizeShowDialog = "2";
						sensus.util.page.fnSavePropertyProfile( { "PAGE_SIZE_SHOW_DIALOG" : sensus.settings.pageSizeShowDialog } );
		...
		action : function(actionDialog) {
			...
			$('#action-dialog').load(sensus.settings.pageSizeInclude, function() {
				actionDialog.removeClass('waiting');
