sensus.commons.modules.content.lightGroups = {
		...
		for (g in aGroups) {
			...
			if (aGroups.hasOwnProperty(g)) {
				...
				if (sensus.settings.userContext.userRole == "ROLE_Role.Admin"){
					...
					sHtmlGroups += '<td><a id="remove-groups" class="delete right" href="#"><span class="hide">'+aGroups[g].id+'</span>'+sensus.locale.get("smartpointdetail.actiongroups.remove")+'</a></td>';
	...
	init : function () {
		...
		var oRequest = {'inquiryPaginationRequest':new inquiryPaginationRequest()};
		$.ajaxValidator.fnDoCall("/slc/group/search.action", oRequest, false, sensus.commons.modules.content.lightGroups.fnLoadGroupList,null);
...
$(".edit", oGroups).click(function() {
	$('input#group-add-select_input').val(sensus.locale.get('widgets.combobox.prompt'))
...
$(".submit-short-form", oGroups).click(function() {
	...
	if (sensus.util.combobox.getOption("#group-add-select") != undefined 
		...
		var oRequest = { 'groupRequest': new groupRequest(sGroupName, null, null, sIdGroup, sAction, false, adUrl) };
		$.ajaxValidator.fnDoCall(sensus.settings.addToGroupUrl, oRequest, false, sensus.commons.modules.content.lightGroups.fnReloadGroups, null);
...
$(oGroups).on('click', 'a.delete', function() {
	...
	// Submit Ajax Action
	var oRequest = { 'groupRequest': new groupRequest(sGroupName, null, null, sIdGroup, sAction, false, adUrl) };
	$.ajaxValidator.fnDoCall(sensus.settings.removeFromGroupUrl, oRequest, false, sensus.commons.modules.content.lightGroups.fnReloadGroups, null);
