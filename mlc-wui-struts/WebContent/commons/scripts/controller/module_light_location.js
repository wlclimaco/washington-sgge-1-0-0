...
$('#address .edit').click(function() {
	var lat = $("#address .read-only dd #latitude").text();
	$("[name='latitude']").val(lat.substring(0, lat.length - 1));
	var lon = $("#address .read-only dd #longitude").text();
...
$('#update-coordinates-form').validationEngine('attach', {
	...
	onValidationComplete    : function(form, status) {
		...
		if (status) {
			...
			var oRequest = {"lightRequest" : new lightRequest(parseInt($.address.parameter('id')), null, null, null, null, null, null, null, null, null, null, null, null, aParam)};
			var fnCallBack = sensus.commons.modules.content.lightLocation.fnUpdateLocation;
			$.ajaxValidator.fnMonitor(sensus.settings.updateLightProperty, oRequest, false, fnCallBack, sensus.locale.get("action.longRunningProcessDialog.confirm"), true);
