(function($) {
	...
	function fnIsRniOn (bShowErrorMessage) {
		var oRniResponse = fnGetResponse(sensus.settings.checkRni, {'processRequest' : new processRequest()}, false);
	...
	function fnCheckResponseStatus (oRequest, oWaitResponseDeferred, sSuccessMessage) {
		...
		(function fnVerify(){
			...
			var oResponse = $.ajaxValidator.fnDoCall("/slc/smartpoint/checkUpdatedLight.action", oRequest, false, null, sSuccessMessage, true);
	...
	function fnMonitor (sUrl, oRequest, bAsync, fnCallback, sSuccessMessage, bCheckResponseReturn) {
		...
		switch (sensus.settings.monitor) {
			case "1" :
				...
				fnDoCall(sUrl, oRequest, bAsync, fnCallback, sSuccessMessage, bBlockProgressBar);
				break;
			case "2" :
