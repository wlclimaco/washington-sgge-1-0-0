var funcionarioModule = (function () {
    ...
    function FUNCIONARIO(funcionario) {
        try {
            ...
            self.estadoCivil 			= ko.observable( funcionario ? (funcionario.estadoCivil || "" ) : "");
            self.tipoPessoa 			= ko.observable( funcionario ? (funcionario.tipoPessoa || 0 ) : 0);
        	...
        	self.bancos 				= ko.observableArray([new bancoModule.BANCO(funcionario.bancos)]);
        	...
        	self.beneficios 			= ko.observableArray([new beneficioModule.BENEFICIO(funcionario.beneficios)]);
        	self.eventosList 			= ko.observableArray([new eventoModule.EVENTO(funcionario.eventosList)]);
