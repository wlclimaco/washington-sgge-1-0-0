...
<c:choose>
    <c:when test="${empty cnaeResponse}">
			viewLoadedObject = null;
    </c:when>
    <c:otherwise>
			viewLoadedObject = ${cnaeResponse};
    </c:otherwise>
</c:choose>
	...
	//		rest_post_call('qat-webdaptive/cidade/api/insertBAS', oData, fill_data, process_error);
		...
		function callPagedFetchWS(_iPageSize, _iStartPage)
		{
			...
			if (viewLoadedObject == null)
			{
				...
				rest_post_call('qat-sysmgmt-sample/services/rest/CidadeService/fetchCnaeByRequest', {}, fill_data, process_error);
			}
			else
			{
		...
		{
			data = reuse_fill_data(procResponse,data,"cnae");
