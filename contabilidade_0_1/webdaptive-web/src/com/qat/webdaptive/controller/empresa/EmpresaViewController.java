package com.qat.webdaptive.controller.empresa;
...
public class EmpresaViewController extends EmpresaBaseController
{
	...
	private static final String FETCH_LIST_REGIME = "regime";

	private static final String FETCH_LIST_CNAE = "cnae";
	...
	public ModelAndView loadListRegime(HttpServletRequest request)
	{
		...
		try
		{
			...
			modelAndView.addObject(RESPONSE, getMapper()
					.writeValueAsString(fetchRegimeByRequest(pagedInquiryRequest)));
	...
	public ModelAndView loadListCnae(HttpServletRequest request)
	{
		...
		CnaeInquiryRequest pagedInquiryRequest = new CnaeInquiryRequest();
		pagedInquiryRequest.setStartPage(START_PAGE_NUMBER);
		pagedInquiryRequest.setPageSize(INITIAL_PAGE_SIZE);
		pagedInquiryRequest.setPreQueryCount(true);
		pagedInquiryRequest.addSortExpressions(new SortExpression("ID",
		...
		try
		{
			...
			modelAndView.addObject(RESPONSE, getMapper()
					.writeValueAsString(fetchCnaeByRequest(pagedInquiryRequest)));
