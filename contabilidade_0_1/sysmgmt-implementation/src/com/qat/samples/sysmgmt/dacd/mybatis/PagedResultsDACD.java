package com.qat.samples.sysmgmt.dacd.mybatis;
...
public final class PagedResultsDACD extends SqlSessionDaoSupport
{
	...
	// public static void fetchObjectsByRequestCadastro(SqlSession sqlSession, CadastroInquiryRequest request,
	// String countStatement, String fetchPagedStatement,
	// InternalResultsResponse<?> response)
	// {
	//
	// // If the user requested the total rows/record count
	...
	// // remember if the count was not requested the TotalRowsAvailable will be false because the assumption
	// // is that you your own logic to handle this.
	// if (response.getResultsSetInfo().getTotalRowsAvailable() > (response.getResultsSetInfo().getPageSize() *
	// startPage))
	// {
	// response.getResultsSetInfo().setMoreRowsAvailable(true);
	// }
	...
	public static void fetchObjectsByRequestCidade(SqlSession sqlSession, CidadeInquiryRequest request,
			String countStatement, String fetchPagedStatement,
			InternalResultsResponse<?> response)
	{
