 ...
 */
public final class ImagemDACD extends SqlSessionDaoSupport
{
	...
	public static Integer maintainImagemAssociations(List<Imagem> tabPrecoList,
			InternalResultsResponse<?> response, Integer parentId, TypeEnum type, AcaoTypeEnum insert,
			TabelaEnum tabelaEnum, IImagemDAC tabPrecoDAC, IStatusDAC statusDAC, IHistoricoDAC historicoDAC,
			Integer empId,
			String UserId, Integer processId)
	{
		...
		for (Imagem tabPreco : tabPrecoList)
		{
			...
			{
				case INSERT:
					count = tabPrecoDAC.insertImagem(tabPreco,
							"insertImagem", response);
					if (count > 0)
					{
					...
					break;
				case UPDATE:
					count = tabPrecoDAC.updateImagem(tabPreco, response);
					if (count > 0)
					{
						count =
								StatusDACD
