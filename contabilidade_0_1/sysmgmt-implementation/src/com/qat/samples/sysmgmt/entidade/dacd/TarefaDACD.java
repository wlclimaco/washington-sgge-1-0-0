...
public final class TarefaDACD extends SqlSessionDaoSupport
{
	...
	public static Integer maintainTarefaAssociations(List<TarefaEmpresa> tarefaList,
			InternalResultsResponse<?> response, Integer parentId, TypeEnum type, AcaoEnum acaoType,
			TabelaEnum tabelaEnum, ITarefaDAC tarefaDAC, IStatusDAC statusDAC, IHistoricoDAC historicoDAC, Integer empId,
			String UserId, Integer processId, Integer historicoId)
	{
		...
		// For Each Contact...
		for (TarefaEmpresa tarefa : tarefaList)
		{
			...
			switch (tarefa.getModelAction())
			{
				case INSERT:
					...
					count = tarefaDAC.insertTarefaEmpresa(tarefa);
					if (count > 0)
					{
				...
				case UPDATE:
					...
					count = tarefaDAC.updateTarefaEmpresa(tarefa);
					if (count > 0)
					{
						count =
								StatusDACD
					...
					break;
				case DELETE:
					count = tarefaDAC.deleteTarefaEmpresa(tarefa);
					Status status = new Status();
