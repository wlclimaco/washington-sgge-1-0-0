$.widget("sc.filters", {
	...
	_create : function() {
		...
		var isOpen;
		var type;
		var deviceType = $.address.parameter("device_type");

		this._parameters = {};
		this._objFiltersCustomize = [];

		this.defaultSearchNameLabel = qat.model.locale.get(this.options.defaultSearchNameLabel);
		...
		for (i in filters) {
			...
			filter = filters[i];
				...
				if($.ajaxValidator.fnIsNullOrUndefined(deviceType)){
					filter.id = i;
					...
					if (i != "search") {
						...
						this._objFiltersCustomize.push({
							name : qat.model.locale.get(filter.title),
							session : i,
							type : type
						});
				...
				}else if((deviceType.toLowerCase() == 'han_device|')||(deviceType.toLowerCase() == 'lcm|')){
					filter.id = i;
	...
	_createCombobox : function () {
		...
		$(this.element).find("input.ui-autocomplete-input").each(function() {
			...
			$(this).val(iSelectSize + ' ' + qat.model.locale.get('filter.more'));
		});
	},
	...
	_createCustomizeButton : function () {
		...
		return '<div id="customFilter" class="filter-input advanced ui-widget">'
			...
			+ '<span class="ui-button-text">' + qat.model.locale.get('filter.customize') + '</span></a></div>';
	},
	...
	_createOptionFilters : function(flt, isOpen, filterId) {
		...
		// Filter Title
		$filter.push(this._createFilterTitle(qat.model.locale.get(flt.title), isOpen));

		$filter.push('<div class="collapse checkBoxUl" ' + (isOpen ? "" : 'style="display:none;"') + '>');

		//Verify All Checked
		if (qat.model.util.filter.oDefaultCheckboxEvent[flt.id] != null && qat.model.util.filter.oDefaultCheckboxEvent[flt.id] != 0) {
		...
		if ($.address.parameter("pg") == "device/history") {
			for (;x < length; x = x + 1) {
				if (dataList[x][0] == "qat.model.dm.action.category.export") {
					dataList.splice(x,1);
	...
	_createAllCheckbox : function(flt, isChecked) {
		...
		return '<ul class="all-checked"><li class="checkbox">'
				...
				+ '<strong>' + qat.model.locale.get(flt.allLabel) + '</strong></label></li></ul>';
	},
	...
	_selectFilterEvent : function($checkbox, $filter, bReload) {
		...
		if (value == "ALL") {
			...
			if (qat.model.util.filter.oDefaultCheckboxEvent[id] != null && qat.model.util.filter.oDefaultCheckboxEvent[id] != 0) {
			...
			if ($parent.find(".ui-listcombobox input:checked").length == 0 && qat.model.util.filter.oDefaultCheckboxEvent[id] == null) {
	...
	_startDatePicker : function () {
		...
		var sDateFormat = qat.model.settings.userSettings.dateFormatMask.replace('yyyy','yy');
	...
	_autocomplete : function () {
		...
		$(this.element).find(".ui-autocomplete-input").on("autocompleteclose", function(event, ui) {
			...
			var title;
			var value = $this.val();
			var sHint = qat.model.locale.get('filter.more');
	...
	_createMapTag : function () {
		...
		var label = qat.model.locale.get("map.tag.shape", [$.sc.map.getVectorIndex(), value, "<sup>2</sup>"]);
		var typeLabel = qat.model.locale.get("map.tag.drawing");
	...
	loadOptionFilter : function ($filter, parameter) {
		...
		if (values) {
			...
			if (length > 0) {
				...
				if (qat.model.util.filter.oDefaultCheckboxEvent[typeId] != null && values.indexOf(qat.model.util.filter.oDefaultCheckboxEvent[typeId].toString()) != -1) {
				...
				for (i = 0; i < length; i = i + 1) {
					...
					if ($input.length) {
						...
						if(!$.ajaxValidator.fnIsNullOrUndefined(deviceType)){
							if((deviceType.toLowerCase() == 'han_device|')||(deviceType.toLowerCase() == 'lcm|')){
								this.addTag(typeId, typeLabel, label, $input.val(), title, $li ? $li.find("input") : $input);
							}else{
								if(typeId!="description"){
									this.addTag(typeId, typeLabel, label, $input.val(), title, $li ? $li.find("input") : $input);
								}else{
									this.addTag("meter_type","meter_type", label, $input.val(), title, $li ? $li.find("input") : $input);
								}
							}
						}else{
							this.addTag(typeId, typeLabel, label, $input.val(), title, $li ? $li.find("input") : $input);
	...
	addTag : function (typeId, typeLabel, label, value, title, $input) {
		...
		if (typeId == "drawing") {
			...
			if ($tag.length) {
				...
				if (qat.model.util.page.fnCheckXSS(value))
				{
					var sInvalidTag = qat.model.locale.get("qat.model.filter.validation.invalidTag");
					qat.model.util.page.showMessage(this.options.sMessageId, qat.model.locale.get("qat.model.filter.validation.error"), "error");
					value = sInvalidTag;
					label = sInvalidTag;
					title = sInvalidTag;
				}
