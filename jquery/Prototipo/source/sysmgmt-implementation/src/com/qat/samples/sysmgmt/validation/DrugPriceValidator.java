...
public class DrugPriceValidator implements IValidator
{
	...
	{
		if (ValidationUtil.isNull(drugPrice.getEffectiveDate()))
		{
			list.add(new MessageInfo(SYSMGMT_BASE_DRUGPRICEVALIDATOR_DATE_REQUIRED, Message.MessageSeverity.Error,
					Message.MessageLevel.Field));
		}
		else
		{
			drugPrice.setEffectiveDate(QATDateUtil.truncateTime(drugPrice.getEffectiveDate()));
		}
	}
	...
	private void validatePriceSexInd(List<MessageInfo> list, DrugPrice drugPrice)
	{
		...
		{
			list.add(new MessageInfo(SYSMGMT_BASE_DRUGPRICEVALIDATOR_SEXIND_REQUIRED, Message.MessageSeverity.Error,
					Message.MessageLevel.Field));
		}
	}
