...
//    // Cria widget da Basket
//    //$('.page-basket').basket();
//	//$('#DeliveryPostalCode').setMask('99999-999'); Nunca fazer isso! cada widget deve se inicializar!!!!!!
//});

; (function ($, window, document, undefined) {

// Objeto que representa o basket
$.widget("ui.basket", $.wd.widget, {
    ...
    _create: function () {
        ...
        this._bindLineButtons();

        if (this.options.datasource.Response != null && this.options.datasource.Response.Warnings.length > 0)
        {
            this._showWarnings(this.options.datasource.Response, false);
        }

    },

    _init: function () {
        this._initLineButtons();
        this._bindShippingCommands();
        this._bindCouponCommands();
        this._bindActionButtons();
        
        var $self = this;
        $.subscribe('/wd/checkout/basket/refresh', function(e, response){
    ...
    // Chama uma action por ajax para buscar o HTML do carrinho e faz um refresh
    _refresh: function () {
        var $self = this,
            TemplateResult='',
            _json='';

        var data_js = $self.element.attr('data-widget-js');
        if(data_js){
            _json = jQuery.parseJSON( data_js );
        }
        if(_json){
            TemplateResult='&Template='+_json.Template;
        }

        //**
        var baseUrl = this.options.datasource.Urls.ParentBaseUrl;
        
        this._ajax({
            //url: '/widget/checkout_basket?c=' + new Date().getMilliseconds(),
            url: baseUrl + 'widget/checkout_basket?c=' + new Date().getMilliseconds()+TemplateResult,
            type: 'GET',
            cache: false,
            headers: {
                Accept : "text/html",
            ...
            success: function (data, textStatus, jqXHR) {
                if (data !== null) {
                    //$self.element.closest('.js-basket-holder').empty().html(data);
                    $self.element.html($(data).html());
                    $self.element.basket();


                    $self.publish(baseUrl + 'checkout/basket/changed', { });
    ...
    // Ajusta as configurações do box de qty e botões
    _initLineButtons: function () {
        var $self = this;

        //Movido para o document.ready.js
        // $self.element.find('.js-qty').setMask({ mask: '9999' });
        $self.element.find('.js-qty').each(function () {
            var minusBt = $(this).parent().find('.js-qty-less');
            if (parseInt($(this).val()) === 1) {
                minusBt.attr('disabled', 'disabled');
                minusBt.attr('title', '');
            }
        });
    ...
    // Atrela o comportamento nos botões de [-] e [+]
    _bindLineButtons: function () {

        var $self = this;

        $self.element.find('.js-remove').live('click', function () {
            var basketitemid = $(this).parents(".item").find('.js-basketitemid:first').val();
            $self._remove(basketitemid);
        });

        $self.element.find('.js-qty-more').live('click', function () {
            $input = $(this).parent().find('.js-qty');
            var productid = $(this).parents(".item").find('.js-basketitemid:first').val();
            var qty = parseInt($input.val()) + 1;
            $self._updateQuantity(productid, qty);
        });

        $self.element.find('.js-qty-less').live('click', function () {
            $input = $(this).parent().find('.js-qty');
            var productid = $(this).parents(".item").find('.js-basketitemid:first').val();
            var qty = parseInt($input.val()) - 1;
            if (qty > 0)
        ...
        $self.element.find('.js-qty').live('change', function () {
            var productid = $(this).parents(".item").find('.js-basketitemid:first').val();
            var qty = parseInt($(this).val());
            $self._updateQuantity(productid, qty, 'onchange');
        });

    },

    // Atrela os comportamentos referentes aos meios de entrega
    _bindShippingCommands: function () {
        var $self = this, $w = this.getContext();

        var eventUpdatePostalCode = function(){
            var postalcode = $w('#DeliveryPostalCode').val().replace('-', '');
            $self._updatePostalCode(postalcode);
        };
        $w('#DeliveryPostalCode').keydown(function(e){
                if ({9:1, 13:1}[e.which || e.keyCode])
                    eventUpdatePostalCode();
            })
            .keypress(function(e){
                var $keyCode = e.which || e.keyCode;
                //console.log($keyCode);
                if (!{8:1, 37:1, 39:1, 46:1}[$keyCode] && !/^[0-9]$/.test(String.fromCharCode($keyCode)))
                    e.preventDefault();
                else {
                    if ($(this).val().length == 7 && !~$(this).val().indexOf('-')) {
                        var $val = $(this).val() + String.fromCharCode($keyCode);
                        $val = $val.split('');
                        $val.splice(5,0,'-');
                        $(this).val($val.join(''));
                    }
        ...
        $self.element.find('.js-shipping-calc').click(eventUpdatePostalCode);

        $self.element.find('.js-delivery-choice').bind('click', function () {
            var radio = $(this).find('input:radio');
            radio.attr('checked', 'checked');
            $self._setDeliveryOption(radio.val());
        });
        
        //Corrige o bug no Chrome, o radio não fica checked quando retorna o ajax.
        ...
        $self.element.find('.open-schedule-shifts').bind('click', function () {
            var $this = $(this),
                $parent = $this.closest('.js-delivery-choice');
            $self.modal({
                element: $parent.find('.schedule-shifts-modal'),
                width: 940,
                'class': 'schedule-shifts-modal'
            });
        });
    ...
    _bindCouponCommands: function (){
        var $self = this;

        $self.element.find('.js-coupon-calc').click(function () {
            var couponCode = $self.element.find('#CouponCode').val();
            
              $self._addCoupon(couponCode);
            
        });
        
        $self.element.find('.js-coupon-remove').click(function () {
            var couponCode = $(this).parent().find("span").text();
            if(couponCode.length)
                $self._removeCoupon(couponCode);
        });
    },

    // Atrela os comportamentos referentes aos botões de ação do carrinho
    _bindActionButtons: function () {
        ...
        $self.element.find('.bt-keep-buying').click(function () {
            //$(location).attr('href', "/");
        ...
        $self.element.find('.bt-clear-basket').click(function () {
            $self._ajax({
                //url: '/carrinho/remover-todos',
                url: baseUrl + 'carrinho/remover-todos',
                success: function (data) {
                    if (!data.IsValid) {
                        $self._showErrors(data);
                    } else {
                        $self._refresh();
                    }
                }
        ...
        $self.element.find('.bt-checkout').click(function () {
            var checkoutUrl = $self.options.datasource.Urls.Shopping.Checkout.Start;
            if (checkoutUrl === null)
                //checkoutUrl = '/checkout/identificacao';
                checkoutUrl = baseUrl + 'checkout/identificacao';
    ...
    _showErrors: function (data, refresh, extraMessage) {
        var $self = this;
        refresh = refresh || false;
		extraMessage = extraMessage || '';

		var message = data.Errors[0].ErrorMessage;
		if (extraMessage)
			message += extraMessage;
        $self._showMessage(message, refresh, 'error');
    },

    _showWarnings: function (data, refresh) {
        var $self = this;
        refresh = refresh || false;
        $self._showMessage(data.Warnings[0].Message, refresh, 'warning');
    },

    _showMessage: function (message, refresh, type) {
        var $self = this;
        refresh = refresh || false;
        $self.alert(message, type, {
            'class': 'wd-checkout-basket-message',
            onClosed: $self._refresh
        });
    },

    //Atualiza o cep do meio de entrega do carrinho
    _updatePostalCode: function (postalcode) {
        var $self = this;
        ...
        this._ajax({
            //url: '/carrinho/definir-cep',
            url: baseUrl + 'carrinho/definir-cep',
    ...
    //Adiciona um coupon
    _addCoupon: function (couponCode) {
        var $self = this;
        ...
        this._ajax({
            //url: '/carrinho/adicionar-cupom',
            url: baseUrl + 'carrinho/adicionar-cupom',
            data: 'couponCode=' + couponCode,
            success: function (response) {
                if (!response.IsValid) {
					var extraMessage = '';
					if (response.Url && data.Url != '')
						extraMessage = "<br /><a href='" + response.Url + "'>Clique aqui para ser redirecionado para tela de login</a>";
                    $self._showErrors(response, 'refresh', extraMessage);
                } else {
                    if (response.hasOwnProperty('Warnings') && response.Warnings.length) {
                        $self._showWarnings(response);
                    } else {
                        $self._refresh();
                    }
                }
    ...
    //Remove o coupon
    _removeCoupon: function (couponCode) {
        var $self = this;
        ...
        this._ajax({
            //url: '/carrinho/remover-cupom',
            url: baseUrl + 'carrinho/remover-cupom',
            data: 'couponCode=' + couponCode,
            success: function (data) {
                if (!data.IsValid) {
                    $self._showErrors(data, 'refresh');
                } else {
                    $self._refresh();
                }
            }
    ...
    _setDeliveryOption: function (deliveryoptionid) {
        var $self = this;
        ...
        this._ajax({
            //url: '/carrinho/definir-opcao-entrega',
            url: baseUrl + 'carrinho/definir-opcao-entrega',
        ...
        this._ajax({
            ...
            success: function (data) {
                if (!data.IsValid) {
                    ...
                    if($('.wd-checkout-basket-grid tr.item').length>1){
                        $('#basket-item-' + basketItemID + ' td > div').animate({opacity:0},'fast').slideUp('fast',function () {
                            $self._refresh();
                        });
                    }else {
                        $self._refresh();
    ...
    // Chama o serviço para atualizar a quantidade no carrinho
    _updateQuantity: function (basketitemid, qty, e) {
        var $self = this;
        ...
        this._ajax({
            //url: '/carrinho/alterar-quantidade',
            url: baseUrl + 'carrinho/alterar-quantidade',
            data: 'BasketItemID=' + basketitemid + '&Quantity=' + qty,
            success: function (data) {
                if (!data.IsValid) {
                	//$self._showErrors(data, 'refresh');
                	$.colorbox({
                    	html: '<div class="alert alert-error"><div class="message">' + data.Errors[0].ErrorMessage + '</div></div>',
						onClosed: function(){
							$('#basket-item-'+basketitemid+', #basket-item-'+basketitemid+' .product').effect( 'highlight', {}, 1140 )
						    //$('.js-qty, .qty-masked').mask('?9999',{placeholder:""});
                            setMasks();
                        }
                    ...
                    $self._showWarnings(data, 'refresh');
                    setMasks();
                }
                else {
                    $self._refresh();
    ...
    // Ajax para métodos do carrinho. Através desse metodo será controlado os erros, loading, etc.
    ...
    _startLoading: function () {
        var $self = this;
        $('.wd-checkout-basket .loading').fadeIn('fast');
        $self.element.find(':input, :submit, :button, .js-basket-holder a').attr('disabled', 'disabled');
    },


    _stopLoading: function () {
        var $self = this;
         $('.wd-checkout-basket .loading').fadeOut('fast');
        $self.element.find(':input, :submit, :button, .js-basket-holder a').removeAttr('disabled');
    }
