...
type
  ...
  TXPProgressBar = class(TGraphicControl)
  private
    FColorBorder: TColor;
...
procedure Register;
begin
  RegisterComponents('Active Controls', [TXPProgressBar]);
end;

constructor TXPProgressBar.Create(AOwner: TComponent);
begin
  inherited;
  Width := 200;
  Height := 16;
...
procedure TXPProgressBar.Paint;
var
  ScrBmp: TBitmap;
  Pos: Integer;
begin
...
procedure TXPProgressBar.SetColors(Index: Integer; Value: TColor);
begin
  case Index of
...
procedure TXPProgressBar.StepIt;
begin
  Inc(FPosition, FStep);
  Paint;
end;

procedure TXPProgressBar.SetPosition(Value: Integer);
begin
...
procedure TXPProgressBar.SetMin(Value: Integer);
begin
...
procedure TXPProgressBar.SetMax(Value: Integer);
begin
  if Value < FMin then Exit;
  FMax:= Value;
  Paint;
...
procedure TXPProgressBar.SetStep(Value: Integer);
begin
  FStep:= Value;
  Paint;
end;

procedure TXPProgressBar.SetOrientation(Value: TProgressBarOrientation);
begin
  FOrientation:= Value;
  Invalidate;
end;

procedure TXPProgressBar.SetColorScheme(Value: TColorScheme);
begin
  FColorScheme:= Value;
...
procedure TXPProgressBar.GradientFillRect(Canvas: TCanvas; Rect: TRect;
                StartColor, EndColor: TColor; Direction: TFillDirection);
var
  Steps: Integer;
