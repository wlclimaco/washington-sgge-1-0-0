 ...
 *
 * This program is licensed under the Alias-i Royalty Free License * Version 1 WITHOUT ANY WARRANTY, without even the implied warranty of
...
public class JaroWinkler
{
	 ...
	 * sequences. The distance is symmetric and will fall in the
	 ...
	 * in the range <code>0</code> (no match) to <code>1</code> (perfect match)inclusive. See the class definition above
	 * for an exact definition of Jaro-Winkler string comparison.
	 *
	 * <p>
	 * The method {@link #distance(CharSequence,CharSequence)} returns a distance measure that is one minus the
	 * comparison value.
	 *
	 * @param cSeq1 First character sequence to compare.
	 * @param cSeq2 Second character sequence to compare.
	 ...
	 * would be returned by the nullary constructor <code>JaroWinklerDistance()</code>.
	 *
	 * <p>
	 * Instances are thread safe, so this single distance instance may be used for all comparisons within an
	 * application.
	 */
	 ...
	 * returned by the nullary constructor <code>JaroWinklerDistance(0.7,4)</code>.
	 *
	 * <p>
	 * Instances are thread safe, so this single distance instance may be used for all comparisons within an
	 * application.
