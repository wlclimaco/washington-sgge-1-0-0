	...
	<script type="text/javascript">
		pgsi.pages.deposito.form = {
			...
			ajaxCall : function(sUrlAdress, mainModelAction) {
				...
				// Validate the form
				var bValidForm = pgsi.pages.deposito.form.validator.form();
				...
				var request = pgsi.pages.deposito.form.fnFillRequestObject(mainModelAction);

				var fnCallback = function (oResponse){
					if (oResponse.operationSuccess == true) {
						$(this).dialog('close');
						if (!$.pgsi.isNullOrUndefined(oResponse.depositoList[0]) && !$.pgsi.isNullOrUndefined(oResponse.depositoList[0].id)){
							var nLocationId = oResponse.depositoList[0].id;
							$.pgsi.pageLoader.load({
								url: "deposito/view?locationId=" + nLocationId,
								$content: $("#load"),
			...
			fnFillRequestObject : function(sModelAction){
				...
				request.deposito = pgsi.pages.deposito.form.fnFillEmpresa(sModelAction);
