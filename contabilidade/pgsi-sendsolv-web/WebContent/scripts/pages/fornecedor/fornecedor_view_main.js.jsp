...
pgsi.pages.fornecedor.view = {
	...
	fnFillFornecedor : function(oResponse) {

		var oFornecedor = oResponse.fornecedorList[0];

console.log(oFornecedor)

	//	<span class="label">Nome/Fantasia</span>
				$("#nome-field").text(oFornecedor.nome);

	//			<span class="label">CNPJ</span>

				for(var i=0;i<oFornecedor.documentos.length;i++){
					if(oFornecedor.documentos[i].type == "CNPJ"){
						$("#cnpj-field").text(oFornecedor.documentos[i].numero);
					}else if(oFornecedor.documentos[i].type == "IM") {
						$("#im-field").text(oFornecedor.documentos[i].numero);
					}else if(oFornecedor.documentos[i].type == "IE") {
						$("#IE-field").text(oFornecedor.documentos[i].numero);
					}
				}

				$("#regime-field").text(oFornecedor.regime);

				pgsi.pages.phone.view.fillFields(oFornecedor.telefones);

				pgsi.pages.address.view.fillFields(oFornecedor.enderecos);
				var sEmail = "";

				for (var i = 0; i < oFornecedor.emails.length; i++) {
					sEmail = sEmail + oFornecedor.emails[i].description + " " +oFornecedor.emails[i].email +"<br>"
				}
				$('#phone-container').append(sEmail);

				pgsi.pages.fornecedor.view.fillCnae(oFornecedor.cnaes)
