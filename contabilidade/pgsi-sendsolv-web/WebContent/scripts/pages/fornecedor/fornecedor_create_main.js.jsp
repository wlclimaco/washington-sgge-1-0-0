	...
	<script type="text/javascript">
		pgsi.pages.fornecedor.form = {
			...
			ajaxCall : function(sUrlAdress, mainModelAction) {
				...
				// Validate the form
				var bValidForm = pgsi.pages.fornecedor.form.validator.form();
				...
				var request = pgsi.pages.fornecedor.form.fnFillRequestObject(mainModelAction);

				var fnCallback = function (oResponse){
					if (oResponse.operationSuccess == true) {
						$(this).dialog('close');
						if (!$.pgsi.isNullOrUndefined(oResponse.fornecedorList[0]) && !$.pgsi.isNullOrUndefined(oResponse.fornecedorList[0].id)){
							var nLocationId = oResponse.fornecedorList[0].id;
							$.pgsi.pageLoader.load({
								url: "fornecedor/view?locationId=" + nLocationId,
								$content: $("#load"),
			...
			fnFillRequestObject : function(sModelAction){
				...
				request.fornecedor = pgsi.pages.fornecedor.form.fnFillFornecedor(sModelAction);
