...
$(document).ready(function()
{
	...
	$.pgsi.listener.wait({
		eventName 	: "clienteList",
		arguments 	: ["table"],
		fnCallback 	: $.pgsi.progressBar.stopGlobal
	});

	/** * jQuery dataTable setup ** */
	pgsi.pages.cliente.clienteTable = $('#data_list').dataTable($.pgsi.table.setTable(
	{
		id 			: "#data_list",
		...
		[
		{
			headerData 		: "CNPJ",
			order			: "name",
			mRender         : pgsi.pages.cliente.fnCreateEmpresaNameLink,
			sDefaultContent : "",
			bSortable 		: false,
			sClass          : "name-col"
		},
		{
			...
			mRender 		: pgsi.pages.cliente.fnCreateNomeLink,
			sDefaultContent : "",
			bSortable 		: false
		},
		{
			headerData 		: "Cnae",
			order			: "city_column",
			mRender 		: pgsi.pages.cliente.fnCnae,
			mData	 		: "null",
			sDefaultContent : "",
			bSortable 		: false
		},
		{
			headerData 		: "Email",
			order			: "state_column",
			mRender 		: pgsi.pages.cliente.fnEmail,
			sDefaultContent : "",
			bSortable 		: false
		},
		{
			...
			mRender 		: pgsi.pages.cliente.fnTelefone,
			sDefaultContent : "",
			bSortable 		: false
		},
		{
			...
			mRender 		: pgsi.pages.cliente.fnRegime,
			sDefaultContent : "",
			bSortable 		: false
		},
		{
			headerData 		: "Endereco",
			order			: "phone_column",
			mRender 		: pgsi.pages.cliente.fnEndereco,
			sDefaultContent : "",
			bSortable 		: false
		},
		{
			...
			order			: "phone_column",
			mRender 		: pgsi.pages.cliente.fnDocumento,
			sDefaultContent : "",
			bSortable 		: false
		},
		],
		...
		rowCallback : function(nRow, aData, iDisplayIndex, oColumn) {
			...
			oActionSummary = $('<div><div><a href="cliente/view?tab=info&clienteId=' + aData.id + '" title="View '+aData.name+'" id="update" class="icon-nav icon-pencil alist icon-small-button"></a>'
				+ sButtonStatus
				+ sButtonDelete
				+pgsi.util.page.fnInsertButtonSDNSAR(aData,"cliente")+"</div></div>");
			...
			oActionSummary.find('a.deleteDialog, a.active, a.deactivate ,a.sarDialog').click(function (e) {
				...
				var fnCallBack = function(oResponse) {
					...
					if (oResponse.operationSuccess == true) {
						...
						var iStart;
						var oSettings = pgsi.pages.cliente.clienteTable.fnSettings();
						...
						$.pgsi.table.reloadTable({
							table 		: pgsi.pages.cliente.clienteTable,
							iStart 		: iStart
						});
					}else{
				...
				if($(this).hasClass('deleteDialog'))
				{
					...
					var oRequest = new LocationMaintenanceRequest({cliente : {id : aData.id, name: aData.name }});
	...
	$("#clear-all").on("click", function(e)
	{
		...
		$.address.parameter("cliente","");
		pgsi.util.page.fnReloadTable(pgsi.pages.cliente.clienteTable);
	});
