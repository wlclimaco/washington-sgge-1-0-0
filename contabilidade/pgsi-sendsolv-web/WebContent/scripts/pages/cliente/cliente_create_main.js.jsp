	...
	<script type="text/javascript">
		pgsi.pages.cliente.form = {
			...
			ajaxCall : function(sUrlAdress, mainModelAction) {
				...
				// Validate the form
				var bValidForm = pgsi.pages.cliente.form.validator.form();
				...
				var request = pgsi.pages.cliente.form.fnFillRequestObject(mainModelAction);

				var fnCallback = function (oResponse){
					if (oResponse.operationSuccess == true) {
						$(this).dialog('close');
						if (!$.pgsi.isNullOrUndefined(oResponse.clienteList[0]) && !$.pgsi.isNullOrUndefined(oResponse.clienteList[0].id)){
							var nLocationId = oResponse.clienteList[0].id;
							$.pgsi.pageLoader.load({
								url: "cliente/view?locationId=" + nLocationId,
								$content: $("#load"),
			...
			fnFillRequestObject : function(sModelAction){
				...
				request.cliente = pgsi.pages.cliente.form.fnFillEmpresa(sModelAction);
