...
/**
 * @namespace pgsi.pages.historico
...
$(document).ready(function()
{
	...
	$.pgsi.listener.wait({
		eventName 	: "historicoList",
		arguments 	: ["table"],
		fnCallback 	: $.pgsi.progressBar.stopGlobal
	});

	/** * jQuery dataTable setup ** */
	pgsi.pages.historico.historicoTable = $('#data_list').dataTable($.pgsi.table.setTable(
	{
		id 			: "#data_list",
		sAjaxSource : "api/historico/fetchall",
		bPreLoad	: true,
		...
		},
		{
			...
			mRender 		: pgsi.pages.historico.fnCreateData,
			sDefaultContent : "",
			bSortable 		: false
		},
		{
		...
		},
		{
			headerData 		: "Açoes",
			order			: "sdn_status_column",
			mRender 		: pgsi.pages.historico.fnCreateHistoricoItens,
			sDefaultContent : "",
			bSortable 		: false
		},
		],
		...
		rowCallback : function(nRow, aData, iDisplayIndex, oColumn) {
			...
			oActionSummary = $('<div><div><a href="historico/view?tab=info&historicoId=' + aData.id + '" title="View '+aData.name+'" id="update" class="icon-nav icon-pencil alist icon-small-button"></a>'
				+ sButtonStatus
				+ sButtonDelete
				+pgsi.util.page.fnInsertButtonSDNSAR(aData,"historico")+"</div></div>");
			...
			oActionSummary.find('a.deleteDialog, a.active, a.deactivate ,a.sarDialog').click(function (e) {
				...
				var fnCallBack = function(oResponse) {
					...
					if (oResponse.operationSuccess == true) {
						...
						var iStart;
						var oSettings = pgsi.pages.historico.historicoTable.fnSettings();
						...
						$.pgsi.table.reloadTable({
							table 		: pgsi.pages.historico.historicoTable,
							iStart 		: iStart
						});
					}else{
	...
	$("#clear-all").on("click", function(e)
	{
		...
		$.address.parameter("historico","");
		pgsi.util.page.fnReloadTable(pgsi.pages.historico.historicoTable);
	});
