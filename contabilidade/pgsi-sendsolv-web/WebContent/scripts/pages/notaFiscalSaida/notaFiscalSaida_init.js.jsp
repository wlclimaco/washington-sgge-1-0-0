...
$(document).ready(function()
{
	...
	$.pgsi.listener.wait({
		eventName 	: "notaFiscalSaidaList",
		arguments 	: ["table"],
		fnCallback 	: $.pgsi.progressBar.stopGlobal
	});

	/** * jQuery dataTable setup ** */
	pgsi.pages.notaFiscalSaida.notaFiscalSaidaTable = $('#data_list').dataTable($.pgsi.table.setTable(
	{
		id 			: "#data_list",
		...
		rowCallback : function(nRow, aData, iDisplayIndex, oColumn) {
			...
			oActionSummary = $('<div><div><a href="notaFiscalSaida/view?tab=info&notaFiscalSaidaId=' + aData.id + '" title="View '+aData.name+'" id="update" class="icon-nav icon-pencil alist icon-small-button"></a>'
				+ sButtonStatus
				+ sButtonDelete
				+pgsi.util.page.fnInsertButtonSDNSAR(aData,"notaFiscalSaida")+"</div></div>");
			...
			oActionSummary.find('a.deleteDialog, a.active, a.deactivate ,a.sarDialog').click(function (e) {
				...
				var fnCallBack = function(oResponse) {
					...
					if (oResponse.operationSuccess == true) {
						...
						var iStart;
						var oSettings = pgsi.pages.notaFiscalSaida.notaFiscalSaidaTable.fnSettings();
						...
						$.pgsi.table.reloadTable({
							table 		: pgsi.pages.notaFiscalSaida.notaFiscalSaidaTable,
							iStart 		: iStart
						});
					}else{
	...
	$("#clear-all").on("click", function(e)
	{
		...
		$.address.parameter("notaFiscalSaida","");
		pgsi.util.page.fnReloadTable(pgsi.pages.notaFiscalSaida.notaFiscalSaidaTable);
	});
