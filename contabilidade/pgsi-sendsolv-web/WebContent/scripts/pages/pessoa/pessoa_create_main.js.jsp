	...
	<script type="text/javascript">
		pgsi.pages.pessoa.form = {
			...
			ajaxCall : function(sUrlAdress, mainModelAction) {
				...
				// Validate the form
				var bValidForm = pgsi.pages.pessoa.form.validator.form();
				...
				var request = pgsi.pages.pessoa.form.fnFillRequestObject(mainModelAction);

				var fnCallback = function (oResponse){
					if (oResponse.operationSuccess == true) {
						$(this).dialog('close');
						if (!$.pgsi.isNullOrUndefined(oResponse.pessoaList[0]) && !$.pgsi.isNullOrUndefined(oResponse.pessoaList[0].id)){
							var nLocationId = oResponse.pessoaList[0].id;
							$.pgsi.pageLoader.load({
								url: "pessoa/view?locationId=" + nLocationId,
								$content: $("#load"),
			...
			fnFillRequestObject : function(sModelAction){
				...
				request.pessoa = pgsi.pages.pessoa.form.fnFillEmpresa(sModelAction);
