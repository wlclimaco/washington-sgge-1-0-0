...
pgsi.util.page = {
	...
	fnIsSDNFlagged : function(sSDNStatus) {
		if (!$.pgsi.isNullOrUndefined(sSDNStatus) && sSDNStatus !== "UNKNOWN" && sSDNStatus !== "NEUTRAL" && sSDNStatus != "FALSE_POSITIVE") {
		 	return true;
		}
	...
	fnReloadTable     : function(sNameSpace)
	{
		...
		$.pgsi.table.reloadTable({table : sNameSpace});
	...
	fnInsertButtonSDNSAR : function(aData,sPage){
		...
		if ((aData.sdnstatus === "POSITIVE")||(aData.sdnstatus === "PENDING_INVESTIGATION")||(aData.sdnstatus === "PENDING_NEUTRAL")||(aData.sdnstatus === "PENDING_FEDERAL_INVESTIGATION")){
			sButtonSDN = '<a href="sdn/'+sPage+'/view?type='+sPage+'&id=' + aData.id + '" title="'+ $.pgsi.locale.get("pages.sdn.view.table.SDN") + ' ' + aData.firstName + ' ' + aData.lastName + '" id="update" class="alist"><span class="icon-nav icon-security icon-shield82 icon-small-button sarDialog"></span></a>'
		}
		sButtonSAR = '<a href="#" class="sarDialog"><span class="icon-nav icon-security icon-eye-1 icon-small-button sarDialog icon-small-button" title="' + $.pgsi.locale.get("commons.title.table.button.SAR") + ' ' + aData.firstName + ' ' + aData.lastName + '"></span></a>';
	...
	fnInsertMask : function(sMask, iValue){
		var sReturn;
		if((!$.pgsi.isNullOrUndefined(sMask))&&(!$.pgsi.isNullOrUndefined(iValue))){
	...
	fnValidadeSAR : function(iId){
		...
		{
			oResponse = pgsi.pages.sar.common.ajaxCallSearch(iId);
			 ...
			 fnCallback : function(oResponse){
				...
				var fnCallBackActive = function(oReturn) {
						...
						if (oReturn.operationSuccess == true) {
							...
							if(sStatusValue == 2){
								var sStatusName = $.pgsi.enums.internationalizeByLabel("com.prosperitasglobal.sendsolv.model.StatusEnum","ACTIVE");
								$("#status-value").val(1);
								sTextButton = "pages.view.activate";
							}else{
								var sStatusName = $.pgsi.enums.internationalizeByLabel("com.prosperitasglobal.sendsolv.model.StatusEnum","INACTIVE");
								$("#status-value").val(2);
							...
							if (!$.pgsi.isNullOrUndefined(fnRefreshPage)) {
								if(bList){
							...
							pgsi.pages.sendsolv.fnDialogMessageError("",{},oReturn,null,$.pgsi.locale.get("commons.dialog.error.title"),true);
						}
						$.pgsi.progressBar.stop();
					}
	...
	getSortExpression : function(table, aaSorting, config) {
		...
		if (!$.pgsi.isNullOrUndefined(oSpan.attr('id'))) {
			if (oSpan.length > 0) {
	...
	form : {
		fnInitForm : function($container) {
			...
			if (!$.pgsi.isNullOrUndefined($container)) {
				pgsi.util.page.form.fnInitSelectmenu($container.find('select'));
				pgsi.util.page.form.fnInitTolltip($container.find('.error'));
				$container.find(".btn").button();
			}

			else {
				pgsi.util.page.form.fnInitSelectmenu();
		...
		fnInitSelectmenu : function($elem) {
			...
			if ($.pgsi.isNullOrUndefined($elem)) {
				$elem = $("select");
			}
			$elem.selectmenu({
		...
		fnInitTolltip : function(sSelectors) {
			...
			if (!$.pgsi.isNullOrUndefined(sSelectors)) {
				aItens = sSelectors;
			}
			...
			$(document).tooltip({
				...
				content : function() {
				 	if ($(this).siblings("label.error").length) {
						...
						return "<span class='icon-nav icon-exclamation-triangle'></span>" + $.pgsi.locale.get("pages.form.required");
					}
	...
	business : {
		form : {
			...
			maskFields: {
				...
				fnMask : function() {
					if( $.isFunction( $.fn.mask) ) {
						$("#ein").mask(pgsi.util.page.business.form.maskFields.aMasks["ein"]);
					}
				},
			...
			fillFormFields : function(oData) {
				...
				if (!$.pgsi.isNullOrUndefined(oBusiness.numberOfEmployees)) {
					// Number Of Employees
				...
				if (!$.pgsi.isNullOrUndefined(oBusiness.numberOfMigrantWorkers)) {
					// Number of Imigrants
		...
		view : {
			fillFields : function(oData) {
				...
				if (!$.pgsi.isNullOrUndefined(oBusiness.employerIdentificationNumber)) {
					var oEin = document.createElement("input");
					oEin.type= "text";
					oEin.value = oBusiness.employerIdentificationNumber;
					var $ein = $(oEin);
				...
				if (oBusiness.businessTypeValue === 1 ) {
					$('#status-template .newline').text($.pgsi.locale.get("commons.organization.view.organizationId"))
				}

				else {
					$('#status-template .newline').text($.pgsi.locale.get("pages.location.locationId"))
				}

				$('#status-template #status').text($.pgsi.enums.internationalizeByLabel("com.prosperitasglobal.sendsolv.model.StatusEnum",oBusiness.status));

				$('#status-template #status-id').text(pgsi.util.page.fnInsertMask("***-99999",oBusiness.key) || oBusiness.id);

				// SDN Status
				pgsi.util.page.status.view.fnFillStatusFlags(oBusiness);
			...
			form : {
				...
				fnInit : function($container, iBusinessId) {
					...
					$('#add-another-name').on('click', function(e) {
						...
						$container.append(pgsi.util.page.person.otherNames.form.fnCreateNodes($container, iBusinessId));
				...
				fnFill : function(aOtherNames, $container) {
					...
					var aOtherName = pgsi.util.page.person.otherNames.form.fnCreateNodes($container);
				    var aOtherNameCopy = [];
					...
					for (var i=0; i < aOtherNames.length; i++) {
						...
						if (i === 0) {
							aOtherNameCopy[0].text($.pgsi.locale.get('pages.member.form.label.otherNames'));
			...
			fillObject : function() {
				...
				$("#email-template").find(".row-form").each(function() {
					...
					if (sModelAction.toUpperCase().indexOf("DELETE") != -1) {
						...
						if (!$.pgsi.isNullOrUndefined(sEmailId) && sEmailId.length > 0) {
			...
			fillFormFields : function(oContactList) {
				...
				for (var i = 0; i < oContactList.length; i++) {
					...
					if (oContactList[i].contactTypeValue == 3
						...
						if ($.pgsi.isNullOrUndefined(oEmail)) {
				...
				for (var i=0; i < oEmailList.length; i++) {
					...
					var $copy = $(pgsi.util.page.email.form.createNewNode());
					...
					pgsi.util.page.form.fnInitSelectmenu($email.find("select"));
	...
	status : {
		view : {
			fnFillStatusFlags : function (oBusiness) {
				if (pgsi.util.page.fnIsSDNFlagged(oBusiness.sdnstatus)) {
					var sSDNStatus = $.pgsi.enums.internationalizeByLabel("com.prosperitasglobal.sendsolv.sdn.model.SDNStatusEnum",oBusiness.sdnstatus);
	...
	reports : {
		sar : {
			fnInit : function($container) {
				$container.find(".reports-SARSummaryReport").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fSARSummaryReport&rs:Command=Render");
				$container.find(".reports-SARSummaryByEmployeeReport").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fSARSummaryByEmployeeReport&rs:Command=Render");
				$container.find(".reports-SARSummaryByOrganizationReport").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fSARSummaryByOrganizationReport&rs:Command=Render");
				$container.find(".reports-SARGeneratedAgainstOrganizationsReport").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fSARGeneratedAgainstOrganizationsReport&rs:Command=Render");
				$container.find(".reports-SARGeneratedAgainstIndividuals").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fSARGeneratedAgainstIndividuals&rs:Command=Render");
			}
		},

		operation : {
			fnInit : function($container) {
				$container.find(".reports-OperationalOrganizationReport").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fOperationalOrganizationReport&rs:Command=Render");
				$container.find(".reports-OperationalMemberReport").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fOperationalMemberReport&rs:Command=Render");
				$container.find(".reports-OperationalRevenueReport").prop('href', pgsi.settings.sendsolvReportServerUrl + "/Pages/ReportViewer.aspx?%2fPGSiReports%2fOperationalRevenueReport&rs:Command=Render");
			}
		}
	}
};
...
$.fn.fnLoadDropDownListWithInternationalization = function(aList) {
		if (aList) {
			...
			var sOptions = "<option></option>";
			for(var i = 0; i < aList.length; i++ ) {
				sOptions = sOptions + "<option value='" + aList[i].key + "'>" +  $.pgsi.locale.get(aList[i].value) + "</option>";
			}
