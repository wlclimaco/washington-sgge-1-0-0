								...
								fnRowCallback : function(nRow, aData, iDisplayIndex, col) {
									...
									if (nSmartpointCount < 1 || aData[col.ActionId] == '7') {
										...
										if (nSmartpointCount > 0) {
											...
											$("td:eq(" +col.smartpoint+ ")", nRow).html('<a href="light?'+ sType +'=' + sActionValue +'" class="launch">' + nSmartpointCount + '</a>');

										} else {

											$("td:eq(" +col.smartpoint+ ")", nRow).html('<a href="lightDetail?id='+ nId +'" class="launch">' + nSmartpointCount + '</a>');
								...
								fnDrawCallback : function(setting, col) {
									...
									$("#process-table").on("click", ".spindown", function(e) {
											...
											if (!orowNow.hasClass('childOn')) {
													...
													for (i in aProcesses) {
														...
														if (aProcesses.hasOwnProperty(i)) {
															...
															if (aProcesses[i].isProcessComplete == false) {
																...
																ochildProcess+= "<td class='processing'>" + $.sc.locale("table.type.processing") + "</td>"
															...
															ochildProcess+= '<td><a id="delete_' + aProcesses[i].id + '" class="delete delete_dialog delete_col" href="#">' + $.sc.locale("table.action.delete") + '</a></td></tr>';
												...
												$.sc.getJson("process/fetch", new processRequest(aProcessList,null,null,null), false, fnCallback, null, null, null, null);
