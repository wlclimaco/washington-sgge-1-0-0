...
sensus.pages.longrunningprocess = {
		...
		checkRni   : function() {
			...
			$.when(this.checkRniAjax()).then(function(e) {
				...
				sensus.pages.longrunningprocess.isRniOn = e.rniOnline;


				if (e.rniOnline == false) {

					$("#system-messaging").show();
					$("#rni-offline").show();
					$.sc.stopProgressBar(null, true);

				} else if (e.rniOnline == true) {

					$("#rni-offline").hide();

				}

				if (sensus.pages.longrunningprocess.lrpSize == 0 && e.rniOnline == true) {
		...
		monitor : function(url, data) {
			...
			$.when(this.checkRniAjax()).then(function(e) {
				...
				if (($.isArray(e.operationSuccess) && (e.operationSuccess != false)) || url == "api/process/update") {
				//if (($.isArray(e.operationSuccess) && (e.operationSuccess != false)) || url == "api/process/update" "process/updateMonitorProcess.action") {
		...
		checkRniAjax : function () {
			return $.ajax({
				url   		 : "api/process/checkRNIStatus",
				dataType     : 'json',
				type         : "POST",
				data         : $.toJSON(new processRequest()),
				async        : false,
		...
		longRunningProcessSystemMessaging : function () {
			...
			$.sc.getJson("api/process/fetch", {action: "count"}, false, fnCallBack);
...
alert(1);
			$.ajax({
				...
				success      : function(response) {
					...
					if (response.result == sensus.constants.ajax.ok || response.result == "success" || response.operationSuccess) {
						...
						sensus.util.page.showMessage("messaging-main", $.sc.locale("action.longRunningProcessDialog.confirm") + sensus.util.page.getMessageList(sMessage), "confirm");
						...
						sensus.util.page.showMessage("messaging-main", $.sc.locale("commons.pages.error") + sensus.util.page.getMessageList(response.messages), "error");
						sensus.util.page.stopProgressBar(null, true);
					}
				...
				error: function(e) {
					...
					if (e.operationSuccess == false) {
						...
						sensus.util.page.showMessage("messaging-main", $.sc.locale("commons.pages.error"), "error");
			...
			(function fnVerify(){
				...
				if (iCount == 10) {
					...
					if(!bIsError) {
						...
						sensus.util.page.showMessage('messaging-main',$.sc.locale('commons.pages.exportmsg'),'confirm')
						sensus.util.page.stopProgressBar(null, true);
	...
	callPromptMonitorAction : function(hasBoxUpdating) {
		...
		if (sensus.pages.longrunningprocess.isRniOn == true) {
			...
			sensus.util.page.stopProgressBar(null, true);
			sensus.util.page.showMessage("messaging-main", $.sc.locale("action.longRunningProcessDialog.error") + $.sc.locale("action.longRunningProcessDialog.rnioffline"), "error");
