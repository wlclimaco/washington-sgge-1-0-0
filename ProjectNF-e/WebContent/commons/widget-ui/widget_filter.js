...
$.fn.wFilters = function (arrayObj, objTable, fnCallBack, sPage) {
	...
	sensus.pages.objFilters = {
		...
		filtersConf :{
			...
			groups : {name : $.sc.locale('commons.pages.groups'), type : 'checkBox',	session : 'groups'}
			...
			for (k in aData.fields) {
				...
				if (aData.fields.hasOwnProperty(k)) {
					...
					if (aData.fields[k].type == 'text') {
						...
						/** html template **/
						sHtml = $('<li id="textFilterLi'+k+'" ><label for="'+aData.fields[k].id+'">' + $.sc.locale('commons.pages.'+aData.fields[k].id) + '</label><input id="'+aData.fields[k].id+'" class="standard '+sClass+'" type="text" /></li>');
					...
					/** for select filters **/
						...
						sHtml = $('<li id="textFilterLi'+k+'" ><label for="'+aData.fields[k].id+'">'+$.sc.locale('commons.pages.'+aData.fields[k].id)+'</label><select id="'+aData.fields[k].id.toUpperCase()+'" class="standard '+validationClass+'"><option>ALL</option></select> </li>');
						/** append to filter dom **/
						$('#textFilterUl', $sNewDiv).append(sHtml);
	...
	for (x in objFilters) {
		...
		if (objFilters.hasOwnProperty(x)) {
			...
			if ((sDivToClone == '#searchFilter')||(sDivToClone == '#dateFilter')) {
				...
				/** set filter text **/
				$($sNewDiv + ' .toggle').html($.sc.locale('commons.pages.'+objFilters[x].session));
				/** set ALL text **/
				var allText = $.sc.locale('filter.all.'+objFilters[x].session);
				...
				if (sDivToClone == '#customFilter') {
					...
					/** html template **/
					$('<div id="customFilter" class="filter-input advanced ui-widget"><a class="button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" id="custom-filter-action" href="" role="button" aria-disabled="false"><span class="ui-button-text">'+$.sc.locale('filter.customize')+'</span></a></div>').appendTo('#filters');
