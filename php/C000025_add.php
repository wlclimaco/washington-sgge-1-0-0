...
if($inlineadd == ADD_INLINE)
	$templatefile = "C000025_inline_add.htm";
else
	$templatefile = "C000025_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: C000025_add.php');
...
//	processing CODBARRA - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CODBARRA = $pageObject->getControl("CODBARRA", $id);
		$control_CODBARRA->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CODBARRA - end
//	processing PRODUTO - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_PRODUTO = $pageObject->getControl("PRODUTO", $id);
		$control_PRODUTO->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing UNIDADE - start
	...
	if($inlineAddOption)
	{
		...
		$control_UNIDADE->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing UNIDADE - end
//	processing NOTAFISCAL - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_NOTAFISCAL = $pageObject->getControl("NOTAFISCAL", $id);
		$control_NOTAFISCAL->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing NOTAFISCAL - end
//	processing VALIDADE - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_VALIDADE = $pageObject->getControl("VALIDADE", $id);
		$control_VALIDADE->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing VALIDADE - end
//	processing CODGRUPO - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CODGRUPO = $pageObject->getControl("CODGRUPO", $id);
		$control_CODGRUPO->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CODGRUPO - end
//	processing CODSUBGRUPO - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CODSUBGRUPO = $pageObject->getControl("CODSUBGRUPO", $id);
		$control_CODSUBGRUPO->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CODSUBGRUPO - end
//	processing CODFORNECEDOR - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CODFORNECEDOR = $pageObject->getControl("CODFORNECEDOR", $id);
		$control_CODFORNECEDOR->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CODFORNECEDOR - end
//	processing CODMARCA - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CODMARCA = $pageObject->getControl("CODMARCA", $id);
		$control_CODMARCA->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'C000025_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'C000025_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: C000025_".$pageObject->getPageType().".php");
