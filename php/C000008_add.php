...
if($inlineadd == ADD_INLINE)
	$templatefile = "C000008_inline_add.htm";
else
	$templatefile = "C000008_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: C000008_add.php');
...
//	processing NOME - end
//	processing ENDERECO - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_ENDERECO = $pageObject->getControl("ENDERECO", $id);
		$control_ENDERECO->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing RG - start
	...
	if($inlineAddOption)
	{
		...
		$control_RG->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing RG - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'C000008_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'C000008_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: C000008_".$pageObject->getPageType().".php");
