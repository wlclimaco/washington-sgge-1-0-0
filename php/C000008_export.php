...
$layout->blocks["top"][] = "export";$page_layouts["C000008_export"] = $layout;
...
$xt->display("C000008_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=C000008.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=C000008.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=C000008.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["FUNCAO"] = $pageObject->showDBValue("FUNCAO", $row);
			$values["DATA_ADMISSAO"] = $pageObject->showDBValue("DATA_ADMISSAO", $row);
			$values["DATA_DEMISSAO"] = $pageObject->showDBValue("DATA_DEMISSAO", $row);
			$values["SITUACAO"] = $pageObject->showDBValue("SITUACAO", $row);
			$values["COMISSAO"] = $pageObject->showDBValue("COMISSAO", $row);
			$values["NASCIMENTO"] = $pageObject->showDBValue("NASCIMENTO", $row);
			$values["SALARIO"] = $pageObject->showDBValue("SALARIO", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=C000008.csv");
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"DATA_ADMISSAO\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"DATA_DEMISSAO\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"SITUACAO\"";
	if($outstr!="")
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["FUNCAO"] = $pageObject->getViewControl("FUNCAO")->showDBValue($row, "");
			$values["DATA_ADMISSAO"] = $pageObject->getViewControl("DATA_ADMISSAO")->showDBValue($row, "");
			$values["DATA_DEMISSAO"] = $pageObject->getViewControl("DATA_DEMISSAO")->showDBValue($row, "");
			$values["SITUACAO"] = $pageObject->getViewControl("SITUACAO")->showDBValue($row, "");
			$values["COMISSAO"] = $row["COMISSAO"];
			$values["NASCIMENTO"] = $pageObject->getViewControl("NASCIMENTO")->showDBValue($row, "");
			$values["SALARIO"] = $row["SALARIO"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["FUNCAO"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DATA_ADMISSAO"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DATA_DEMISSAO"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["SITUACAO"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["COMISSAO"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["NASCIMENTO"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["SALARIO"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Função").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data de Admissão").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data de Demissão").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Situação").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Comissão").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data de Nascimento").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Salário").'</td>';	
	...
	else
	{
		...
		echo "<td>"."RG"."</td>";
		echo "<td>"."Função"."</td>";
		echo "<td>"."Data de Admissão"."</td>";
		echo "<td>"."Data de Demissão"."</td>";
		echo "<td>"."Situação"."</td>";
		echo "<td>"."Comissão"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["DATA_DEMISSAO"] = $pageObject->getViewControl("DATA_DEMISSAO")->showDBValue($row, "");
					$values["SITUACAO"] = $pageObject->getViewControl("SITUACAO")->showDBValue($row, "");
					$values["COMISSAO"] = $pageObject->getViewControl("COMISSAO")->showDBValue($row, "");
					$values["NASCIMENTO"] = $pageObject->getViewControl("NASCIMENTO")->showDBValue($row, "");
					$values["SALARIO"] = $pageObject->getViewControl("SALARIO")->showDBValue($row, "");
					$values["OBS1"] = $pageObject->getViewControl("OBS1")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["FUNCAO"]);
					else
						echo $values["FUNCAO"];
			echo '</td>';
							echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DATA_ADMISSAO"]);
					else
						echo $values["DATA_ADMISSAO"];
			echo '</td>';
							echo '<td>';
			
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DATA_DEMISSAO"]);
					else
						echo $values["DATA_DEMISSAO"];
			echo '</td>';
							echo '<td>';
