...
$layout->blocks["top"][] = "search";$page_layouts["C000046_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = "C000046_search.htm";
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("CODNOTA"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("CODNOTA")] = GetTableURL($pageObject->pSet->getLookupTable("CODNOTA"));
	
	$pageObject->fillFieldToolTips("CODNOTA");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("CODNOTA");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "CODNOTA";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("CODNOTA_label","<label for=\"".GetInputElementId("CODNOTA", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("CODNOTA_label", true);
	
	$xt->assign("CODNOTA_fieldblock", true);
	$xt->assignbyref("CODNOTA_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("CODNOTA_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("CODNOTA_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_CODNOTA", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("CODNOTA");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CODNOTA", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"CODNOTA", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
