...
$layout->blocks["top"][] = "export";$page_layouts["C000044_export"] = $layout;
...
$xt->display("C000044_export.htm");

function ExportToExcel_old($cipherer)
{
	global $cCharset;
	header("Content-Type: application/vnd.ms-excel");
	header("Content-Disposition: attachment;Filename=C000044.xls");
...
function ExportToWord($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=C000044.doc");
...
function ExportToXML($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=C000044.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["CODOPERADOR"] = $pageObject->showDBValue("CODOPERADOR", $row);
			$values["DATA"] = $pageObject->showDBValue("DATA", $row);
			$values["SAIDA"] = $pageObject->showDBValue("SAIDA", $row);
			$values["ENTRADA"] = $pageObject->showDBValue("ENTRADA", $row);
...
function ExportToCSV($cipherer)
{
	...
	header("Content-Disposition: attachment;Filename=C000044.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["CODOPERADOR"] = $pageObject->getViewControl("CODOPERADOR")->showDBValue($row, "");
			$values["DATA"] = $pageObject->getViewControl("DATA")->showDBValue($row, "");
			$values["SAIDA"] = $row["SAIDA"];
			$values["ENTRADA"] = $row["ENTRADA"];
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["CODOPERADOR"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["DATA"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["SAIDA"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["ENTRADA"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Código do Caixa").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Código do Operador").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Saída").'</td>';	
	...
	{
		echo "<td>"."Código"."</td>";
		echo "<td>"."Código do Caixa"."</td>";
		echo "<td>"."Código do Operador"."</td>";
		echo "<td>"."Data"."</td>";
		echo "<td>"."Saída"."</td>";
		echo "<td>"."Entrada"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
					...
					$values["CODOPERADOR"] = $pageObject->getViewControl("CODOPERADOR")->showDBValue($row, "");
					$values["DATA"] = $pageObject->getViewControl("DATA")->showDBValue($row, "");
					$values["SAIDA"] = $pageObject->getViewControl("SAIDA")->showDBValue($row, "");
					$values["ENTRADA"] = $pageObject->getViewControl("ENTRADA")->showDBValue($row, "");
					$values["CODCONTA"] = $pageObject->getViewControl("CODCONTA")->showDBValue($row, "");
					$values["MOVIMENTO"] = $pageObject->getViewControl("MOVIMENTO")->showDBValue($row, "");
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["CODOPERADOR"]);
					else
						echo $values["CODOPERADOR"];
			echo '</td>';
							echo '<td>';
