...
if($inlineadd == ADD_INLINE)
	$templatefile = "C000009_inline_add.htm";
else
	$templatefile = "C000009_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: C000009_add.php');
...
//	processing NOME - end
//	processing FANTASIA - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_FANTASIA = $pageObject->getControl("FANTASIA", $id);
		$control_FANTASIA->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing FANTASIA - end
//	processing UF - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_UF = $pageObject->getControl("UF", $id);
		$control_UF->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing UF - end
//	processing CNPJ - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$control_CNPJ = $pageObject->getControl("CNPJ", $id);
		$control_CNPJ->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing CNPJ - end
...
//	processing FANTASIA - start
			...
			if($inlineAddOption)
			{
				$control_FANTASIA->afterSuccessfulSave();
			}
//	processing FANTASIA - end
//	processing UF - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_UF->afterSuccessfulSave();
			}
//	processing UF - end
//	processing CNPJ - start
			$inlineAddOption = true;
			...
			if($inlineAddOption)
			{
				$control_CNPJ->afterSuccessfulSave();
			}
//	processing CNPJ - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'C000009_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'C000009_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: C000009_".$pageObject->getPageType().".php");
...
//	IE
	...
	{	
		...
		$showRawValues["FANTASIA"] = substr($data["FANTASIA"],0,100);
		...
		$showRawValues["CONTATO1"] = substr($data["CONTATO1"],0,100);
		$showRawValues["CONTATO2"] = substr($data["CONTATO2"],0,100);
		$showRawValues["CELULAR1"] = substr($data["CELULAR1"],0,100);
		$showRawValues["CELULAR2"] = substr($data["CELULAR2"],0,100);
		$showRawValues["EMAIL"] = substr($data["EMAIL"],0,100);
		$showRawValues["HOMEPAGE"] = substr($data["HOMEPAGE"],0,100);
		$showRawValues["CNPJ"] = substr($data["CNPJ"],0,100);
		$showRawValues["IE"] = substr($data["IE"],0,100);
		$showRawValues["OBS1"] = substr($data["OBS1"],0,100);
		$showRawValues["IM"] = substr($data["IM"],0,100);
