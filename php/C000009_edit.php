...
$templatefile = ($inlineedit == EDIT_INLINE) ? "C000009_inline_edit.htm" : "C000009_edit.htm";
...
//	processing NOME - end
//	processing FANTASIA - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_FANTASIA = $pageObject->getControl("FANTASIA", $id);
		$control_FANTASIA->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
//	processing UF - begin
	...
	if($condition)
	{
		...
		$control_UF->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing UF - end
//	processing CNPJ - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$control_CNPJ = $pageObject->getControl("CNPJ", $id);
		$control_CNPJ->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: C000009_".$pageObject->getPageType().".php?".$keyGetQ);
...
if($readevalues)
{
	...
	$data["FANTASIA"] = $evalues["FANTASIA"];
	$data["UF"] = $evalues["UF"];
	$data["CNPJ"] = $evalues["CNPJ"];
}
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='C000009_view.php?".$viewlink."'\"");
