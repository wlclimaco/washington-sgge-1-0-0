sensus.widgets.datatable = {
	 ...
	 * 				The table object
	 */
	reloadTable : function(table, iStart, bResetSelects) {
		...
		if(bResetSelects || bResetSelects == undefined)
		{
				...
				for (var k in aRowColumn) {
					...
					if( aRowColumn[k].match(/fn\(/)){
						...
						if (aChildren[1]) {
							...
							if(aChildren.length == 3)
							{
								if (!data[obj][i][aChildren[0]][aChildren[1]]) {
								...
								aData.push(data[obj][i][aChildren[0]][aChildren[1]][aChildren[2]]);
								continue;
			...
			if (data[obj].hasOwnProperty(i)) {
				...
				for (var k in aRowColumn) {
					...
					if( aRowColumn[k].propertyRequest.match(/fn\(/)){
						...
						var args = (aaChildren[0] != "") ? data[obj][i][aaChildren[0]] : data[obj][i];
						aData.push(sensus.widgets.datatable.fnExecuteFunctionByName(aaChildren[1].replace(')]', '').replace('fn(',''), window, args));
						continue;
	...
	fnCreateRequestModel : function(sSource, oTable, config, aoData, aoCustomColumns, fnCallback)
	{
		...
		var fnSuccessCallBack = function(json)
		{
			...
			var hours = dateNow.getTimezoneOffset() / 60;

			dateNow.setHours(dateNow.getHours() + hours);

			oTable.dateRefresh = dateNow;
			...
			if (oTable.dateRefresh != undefined)
			{
				...
				$(".stamp").text($.sc.locale('table.refreshed', [sDate + " " + " " + sensus.settings.TIME_ZONE_SHORT ]))
						   .wrapInner("<em/>");
			}
		...
		{
			if($.sc.isValidPreLoad(config.oPreLoadResponse))
			{
		...
		else
		{
			...
			if (oTable.dataTableSettings[0].aaSorting.length)
			{
				...
				var oSorted = oTable.find(".sorting_asc, .sorting_desc");

				if($(oSorted).children('span').attr('id')){

					sEnumValue = $(oSorted).children('span').attr('id').replace('id=\"','').replace('\"','').replace(/-/g,'_').toUpperCase();

				} else if (oTable.attr('id') == 'smartpoint-table' && $.address.parameter('sort')){

					sEnumValue = $.address.parameter('sort').split('|');
					sDirection = sEnumValue[1];
					sEnumValue = sEnumValue[0];

				}

				if(!sDirection)
				{
					sDirection = oDataSettings[0].aaSorting[0][1];
				}

				if (!$.sc.isNullOrUndefined(sEnumValue))
				{
	...
	addCustomColumns : function(oTableSettings, oTable) {
		...
		if (oTableSettings.customColumns == "analyticslist") {
			...
			oCustomDataResponse  = $.wCustomize && $.wCustomize.getData(oTableSettings.customColumns, 'Columns', null, oTableSettings.oFilterPreLoad);

		}

		if ( oCustomDataResponse && oCustomDataResponse.listColumn) {

			this.oTableSettings.customColumns = oTableSettings.customColumns;
		...
		$.each(columns, function(iColumns) {
			...
			$.each(oCustomData, function(iCustomIndex) {
				...
				if (bAddDefaultHeader) {
					...
					} else if (oCustomData[iCustomIndex].sId == "ALERTS") {
						sColumnClass = "active";
					...
					if (oCustomData[iCustomIndex].sId.toUpperCase() == 'MAP_IT') {
						...
						for (var iKey in columns) {
							if (columns[iKey].sId.toUpperCase() == oCustomData[iCustomIndex].sId.toUpperCase()) {
	...
	fnFormatColumnResponse : function(e, oTable) {
		...
		if(this.oTableSettings.customColumns == "smartpointlist")
		{
			$.each(e.listColumn, function(iColumnIndex) {
				...
				oCurrentColumn.sName           = $.sc.locale("smartpoint.table.header."+e.listColumn[iColumnIndex].columnEnum.replace(/_/g,"").toLowerCase());
	...
	rowCallback : function(nRow, aData, iDisplayIndex) {
		...
		//Remove
		remove = $('<td id="buttons"><a id="delete_' + iDisplayIndex + '" class="delete delete_dialog delete_col a" href="#">'+ $.sc.locale("table.action.delete") +'</a>   <a id="edit_' + iDisplayIndex + '" class="edit a edit_dialog edit_col" href="#">'+ $.sc.locale("smartpointdetail.location.edit") +'</a></td>').click(function(e) {

			var $this = $(this);

			e.preventDefault();

			$('#buttons a').live('click', function(event){
				...
				{
					sensus.widgets.datatable.currentRow = aData;
					sensus.widgets.datatable.editDialog(datatable);
				}else{
					sensus.widgets.datatable.currentRow = aData;
	...
	fnDrawCallback : function(oSettings) {
		...
		var datatable             = $(this).data("table"),
			...
			$paginationLast       = $("span a:last", $paginationContainer),
			$paginationFirst      = $("span a:first", $paginationContainer);
			$oPaging              = $('.paging', $table.siblings('.table-footer'));

		var fnBind = function (o, p) {
			oSettings.oApi._fnBindAction( o, {"page": (p)?p:parseFloat(o.text())-1}, function(e) {
		...
		$(".dataTables_paginate span a").each(function() {
		...
		if ((iPaginatePosition < 7) && (iTotalPagination > 12))
		{
			...
			if (iCurrent < iTotalPagination - 2)
			{
				...
				{
					$paginationLast.text(iTotalPagination).unbind("click");
					fnBind($paginationLast, "last");
					$paginationLast.prev().text(iTotalPagination - 1).unbind("click");
					fnBind($paginationLast.prev());
			...
			else if (iCurrent < iTotalPagination - 1)
			{
				$paginationLast.text(iTotalPagination).unbind("click");
				fnBind($paginationLast, "last");
				$("<a class='paginate_button'>...</a>").insertBefore($paginationLast);
			}
		}
		...
		{
			//Add first button
			var buttonFirst = $('a.paginate_button:first').clone(true);
			buttonFirst.text('1').unbind("click");

			var tablePaginate = $('a:eq(2)','.dataTables_paginate');
			fnBind($(buttonFirst).prependTo(tablePaginate));
		...
		else if ((iCurrent > 7) && (iTotalPagination > 12))
		{
			...
			if (iCurrent > 3)
			{
				$paginationFirst.text("1").unbind("click");
				fnBind($paginationFirst, "first");
				$paginationFirst.next().text("2").unbind("click");
				fnBind($paginationFirst.next());
				$("<a class='paginate_button'>...</a>").insertAfter($paginationFirst.next());
			}
			else if (iCurrent > 2)
			{
				$paginationFirst.text("1").unbind("click");
				fnBind($paginationFirst, "first");
				$("<a class='paginate_button'>...</a>").insertAfter($paginationFirst);
			}
		}
	...
	editDialog : function(datatable) {
		console.log(datatable);
		this.oTableSettings = datatable;
		$.sc.launchActionDialog("academiaDialog", sensus.pages.academia.dialogSettings.academiaDialog);
	},
	...
	pageSizeDialog: function(datatable) {
		this.oTableSettings = datatable;
		var oPosition = {my : "top", of : $(".content-inner")};
		$.sc.launchActionDialog("pageSize", this.dialogSettings["pageSize"], null, oPosition);
	}
};
...
sensus.widgets.datatable.dialogSettings = {
		...
		remove : {
			...
			action : function(actionDialog) {
				var settings = sensus.widgets.datatable.oTableSettings.oSettings.remove || sensus.widgets.datatable.oTableSettings.oSettings.process.remove;
				$('#action-dialog').load('pages/datatable_remove.jsp', function() {
	...
	mapIt : {
		...
		action : function(actionDialog) {
			...
			if (sensus.widgets.datatable.oTableSettings.id == "group-table") {
				dialogTitle = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.groupName] + ' - ' + sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.groupLightCount] + ' '+$.sc.locale('commons.pages.smartPoints');
				iGroupId = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.Id];
				oRequest = new lightRequest(new LightCriteria(), null, new GroupCriteria([iGroupId]));
				objLatLon.push({
					centerLat:sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.groupLatitude],
					centerLon:sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.groupLongitude]
				});
			} else if (sensus.widgets.datatable.oTableSettings.id == "schedule-table") {
				dialogTitle = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.scheduleName] + ' - ' + sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.scheduleLightCount] + ' '+$.sc.locale('commons.pages.smartPoints');
				...
				oRequest = new lightRequest(new LightCriteria(), null, null, null, null, null, new ScheduleCriteria(null, [iScheduleId]));
				objLatLon.push({
					centerLat:sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.centerLatitude],
					centerLon:sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.centerLongitude]
				});
			} else if (sensus.widgets.datatable.oTableSettings.id == "user-table") {
				dialogTitle = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.id] + ' - ' + sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.userLightCount] + ' '+$.sc.locale('commons.pages.smartPoints');
				...
				for (var i=0; i<aGroups.length; i++){
					aGroupIds.push(aGroups[i].id);
				}
				oRequest = new lightRequest(new LightCriteria(), null, new GroupCriteria(aGroupIds));
				sName = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.userName];
				objLatLon.push({
					centerLat:sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.userLatitude],
					centerLon:sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.userLongitude]
				});
				...
				dialogTitle = sName;
				objLatLon.push({
					centerLat:sensus.pages.user.nLat,
					centerLon:sensus.pages.user.nLng
				});

			} else {

				var sStatus = sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.PRECEDENCE];

				if(isNaN(sStatus)){

					var status = sStatus.toLowerCase();
				...
				objLatLon.push({
					//centerLat : parseFloat(sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.LATITUDE]),
					latitudeAvg : parseFloat(sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.LATITUDE]),
					//centerLon : parseFloat(sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.LONGITUDE]),
					longitudeAvg : parseFloat(sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.LONGITUDE]),
					currentLightStatus : sStatus,
					lightIds : [sensus.widgets.datatable.currentRow[sensus.widgets.datatable.oTableSettings.aColumns.colIndexAll.Id]],
			...
			actionDialog.dialog('open');
			$.sc.maps('detail-map-container', objLatLon, "api/maps/fetch", oRequest, 'small', 1);
		...
		buttons : [
           {
				...
				click  : function() {
					...
					$.sc.savePropertyProfile({ "PAGE_SIZE" : nPageSize });

					if ($("#page-size-checkbox").is(":checked"))
					{
						$.sc.savePropertyProfile( { "PAGE_SIZE_SHOW_DIALOG" : "2" } );
					}
