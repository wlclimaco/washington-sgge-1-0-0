...
sensus.pages.user = {
	...
	renderDeleteMessage : function() {
		...
		if (nUsers.length == 1)
		{
			$('#selected-users').append($.sc.locale("user.validation.deleteUser", sUserName));
		}
		else
		{
			$('#selected-users').append($.sc.locale("user.validation.deleteUsers", $('.checked-count').text().split(' ')[0]));
		}
	}
