sensus.commons.modules.content.lightGroups = {
	...
	fnReloadGroups : function () {
		...
		sensus.commons.modules.content.lightGroups.fnLoadGroups(oResponse);
	...
	init : function () {
		...
		var obj = function() {
			return {
					url : "api/group/fetchall",
					data : new inquiryPaginationRequest()
				};
		};

		$.when(sensus.commons.modules.util.getData(obj)).done(function(oGroupResponse) {
			sensus.commons.modules.content.lightGroups.fnLoadGroupList(oGroupResponse);
		});
...
$(".submit-short-form", oGroups).click(function() {
				...
				&& $('#group-add-select_input').val() != $.sc.locale("widgets.combobox.prompt"))
	{
		...
		var oRequest = new groupRequest(sGroupName, null, null, sIdGroup, false, aIds);
		oRequest.lightRequest = new lightTableRequest(sensus.util.page.getSearchCriteria());
		oRequest.lightRequest = sensus.pages.smartpoint.setLightIdsOnLightCriteria(oRequest.lightRequest, aIds);
