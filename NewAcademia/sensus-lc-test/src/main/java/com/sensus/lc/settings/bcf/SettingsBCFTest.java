...
public class SettingsBCFTest extends AbstractTestBaseBusiness
{
	...
	public void testUpsertSystemSettings()
	{
		// Validation
		SettingsRequest request = new SettingsRequest();
		SettingsResponse response = getSettingsBCF().upsertSystemSettings(request);
		assertFalse(response.isOperationSuccess());
		assertMessages(response, SENSUS_MLC_VALIDATOR_ID_REQUIRED, SENSUS_MLC_VALIDATOR_ID_REQUIRED,
				SENSUS_MLC_VALIDATOR_REQUIRED);

		request = TestBaseUtil.createSettingRequest();
		response = getSettingsBCF().upsertSystemSettings(request);
		assertFalse(response.isOperationSuccess());
		assertMessages(response, SENSUS_MLC_SETTINGSVALIDATOR_SETTINGS_REQUIRED);
		...
		// Error situation
		setSituation(getSettingsBCF(), SituationsEnum.ERROR, ISettingsBCL.class, UPSERT_SYSTEM_SETTINGS);
		request = TestBaseUtil.createSettingRequest();
		request.setActualPassword(ACTUAL_PASSWORD);
		request.setNewPassword(NEW_PASSWORD);
		response = getSettingsBCF().upsertSystemSettings(request);
		...
		// Exception situation
		setSituation(getSettingsBCF(), SituationsEnum.EXCEPTION, ISettingsBCL.class, UPSERT_SYSTEM_SETTINGS);
		request = TestBaseUtil.createSettingRequest();
		request.setActualPassword(ACTUAL_PASSWORD);
		request.setNewPassword(NEW_PASSWORD);
		response = getSettingsBCF().upsertSystemSettings(request);
